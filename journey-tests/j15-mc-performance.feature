...
Feature: j15 — Selma's singing room with live gifts
  ...
  Background:
    ...
    Given Selma [P-15] is signed in on the app physical with userType=MC_SINGER, beans=10000
    Given Alice [P-02] is signed in on Web Chromium with shyCoins=5000
    Given Theo [P-10] is signed in on the app with shyCoins=1500
  ...
  Scenario: Selma opens a "Singing" template room with the "MC Singer" badge on the host seat
    When Selma on the app opens the "rooms" tab
    When Selma on the app taps "main_createRoomFab"
    When Selma on the app picks template "Singing" and title "Selma's Saturday Sing-along"
    When Selma on the app taps "createRoom_confirmButton"
    ...
    Then Selma's app UI shows the room screen with host seat occupied + "MC Singer" badge
  ...
  Scenario: Selma's open room surfaces on Theo's "Following" rail with the tier badge
    ...
    When Theo on the app refreshes the "rooms" tab
    Then within 5000ms Theo's app UI shows Selma's room in the "Following" rail
    Then Theo's app UI shows the "MC Singer" tier badge on the room card
  ...
  Scenario: Alice (Web) and Theo (Android) join Selma's room — participants list updates
    ...
    When Theo on the app taps Selma's room
    Then within 5000ms the database has document "rooms/{roomId}" with field "participantIds" containing [50000010, 50000060]
    Then within 3000ms Selma's app UI shows the participants list with Alice + Theo
  ...
  Scenario: Alice sends a rose mid-performance — coins debit, beans credit, animation on all 3 UIs
    ...
    Then within 3000ms Selma's app UI shows the rose gift animation overlay with sender "Alice"
    Then within 3000ms Alice's Web UI shows the rose gift animation
    Then within 3000ms Theo's app UI shows the rose gift animation
  ...
  Scenario: Theo's crown + Alice's diamond escalations — beans accrue, tier-appropriate animations play
    ...
    When Theo on the app sends "crown" to Selma
    ...
    Then within 3000ms Selma's app UI shows the diamond animation with celebratory effect
  ...
  Scenario: Real-time top-contributor banner in the room — Alice tops the list after her diamond
    ...
    Then within 3000ms Selma's app UI shows "Top contributor: Alice" banner
  ...
  Scenario: Selma closes the room — summary panel + global leaderboard reflect 755 beans
    ...
    When Selma on the app taps "room_endRoomButton"
    When Selma on the app confirms
    Then within 5000ms the database has document "rooms/{roomId}" with field "state" equal to "CLOSED"
    Then within 5000ms Selma's app UI shows the room-closed summary panel
    Then Selma's app UI shows total beans earned this session = (5 + 250 + 500) = 755
    Then Selma's app UI shows the list of contributors with amounts
    When Selma on the app opens the "leaderboard" screen
    Then within 5000ms Selma's app UI shows her rank in the MC_SINGER leaderboard
  ...
  Scenario: Selma's room is auto-featured on the home tab for her tier
    ...
    Then within 3000ms Adam's app UI shows Selma's room in the "Featured MC_SINGER rooms" rail
    Then the rail card shows the "MC Singer" badge

  @android-physical @ios-physical @cross-cohort
  Scenario: Selma is adult — her singing room does NOT appear to minor users
    Given Marcus [P-04] (minor) opens the "home" tab
    When Marcus on the app refreshes the rooms list
    Then Marcus's app UI does not show Selma's room
