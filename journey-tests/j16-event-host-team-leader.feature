...
Feature: j16 — Tariq's multi-singer event
  ...
  Background:
    ...
    Given Tariq is also signed in on the app (same Firebase identity) for hosting
    Given Tariq's user doc has teamRoster=[50000080] (Selma is on his roster)
    Given Selma [P-15] is signed in on the app with userType=MC_SINGER
    Given Alice [P-02] is signed in on Web Chromium
    Given Theo [P-10] is signed in on the app
  ...
  Scenario: Tariq schedules a Saturday Showcase, Selma is invited and accepts
    ...
    Then within 3000ms Selma's app UI shows an in-app banner "You are scheduled in Tariq's event"
  ...
  Scenario: Audience tips Selma — beans split + event-level gift ledger gets two entries
    ...
    When Theo on the app sends "rose" (10 coins) to Selma
  ...
  Scenario: Tariq closes the event — all four UIs see the appropriate closing summary
    ...
    Then within 5000ms Tariq's app UI shows the event summary panel: total gifts, total beans, per-MC breakdown
    Then within 5000ms Selma's app UI shows her individual earnings for this event (255 beans)
