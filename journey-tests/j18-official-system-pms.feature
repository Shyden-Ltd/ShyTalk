...
Feature: j18 — Officia's system PMs
  ...
  Background:
    ...
    Given Adam [P-01] is signed in on the app with cohort=adult and locale=en
    Given Hayato [P-06] is signed in on the app with cohort=minor (post-j04 state) and locale=ja
  ...
  Scenario: Age-up welcome PM is sent in recipient's locale + has official badge
    ...
    Then within 5000ms Adam's app UI shows a new PM thread with sender "ShyTalk Official"
    Then Adam's app UI shows the official badge on the sender avatar
    Then Adam's app UI shows the welcome PM body in English

  @blocker @android-physical @ios-physical @locale-cjk
  Scenario: Age-down PM is sent in Japanese (recipient locale) regardless of admin locale
    ...
    Then within 5000ms Hayato's app UI shows the system PM with Japanese body
    Then Hayato's app UI shows the official badge

  @blocker @android-physical @ios-physical
  Scenario: System PMs are unblockable
    Given Adam received a system PM from Officia
    When Adam on the app opens Officia's profile from the PM
    Then Adam's app UI shows "Official" badge and a disabled block button
  ...
  Scenario: Officia can send to a minor user (cohort gate exemption for SHYTALK_OFFICIAL)
    Given Marcus [P-04] (minor) is signed in on the app
    ...
    Then within 5000ms Marcus's app UI shows the PM with the official badge
  ...
  Scenario: PM template with unknown key falls back to the English value, with telemetry warning
    ...
    Then within 5000ms Adam's app UI shows the PM (does not silently fail to deliver)
  ...
  @android-physical @ios-physical @perf-budget:5000
