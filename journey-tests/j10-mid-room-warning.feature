...
Feature: j10 — Admin warning lands during an active voice room
  ...
  Scenario: Pre-state — Theo hosts "r1" with mic open and no active warning
    ...
    Then Theo's app UI shows the room screen with mic-on indicator
  ...
  @blocker @android-physical
  Scenario: Theo's app UI navigates to the warning screen with reason + duck + acknowledge
    Given Theo has hasActiveWarning=true, warningReason="Inappropriate language in voice room"
    Then within 5000ms Theo's app UI navigates to the warning screen
    Then Theo's app UI shows the warning reason "Inappropriate language in voice room"
    Then Theo's app UI shows the police duck image
    Then Theo's app UI does not show the voice room UI
    Then Theo's app UI does not show the element with tag "main_roomsTab"
    Then Theo's app UI shows the element with tag "warning_acknowledgeButton"
  ...
  Scenario: Ines's iOS Sim sees Theo's seat with mic-off and may receive the room-closed cascade
    ...
    Then within 5000ms Ines's app UI shows Theo's seat with mic-off indicator
    Then within 6000ms either Ines's app UI is still in the room with host muted
    Then OR within 6000ms Ines's app UI shows a "Room closed by host warning" toast and navigates back to "/rooms"
  ...
  Scenario: Theo cannot bypass the warning — back-press and relaunch both keep him on the warning screen
    ...
    When Theo on the app attempts to navigate via the back button
    Then Theo's app UI does not navigate away
    Then Theo's app UI still shows the warning screen
    When Theo on the app attempts to kill and relaunch the app
    Then within 5000ms Theo's app UI shows the warning screen again on next launch

  @blocker @android-physical
  Scenario: Theo acknowledges — warning cleared, ack timestamp recorded, back at rooms tab
    Given Theo is on the warning screen with hasActiveWarning=true
    When Theo on the app taps "warning_acknowledgeButton"
    ...
    Then within 3000ms Theo's app UI shows the element with tag "main_roomsTab"
    Then Theo's app UI does NOT navigate back into room "r1" automatically (he must rejoin)
  ...
  Scenario: Theo rejoins room "r1" — seat is empty and his mic defaults to muted
    ...
    When Theo on the app taps the room "r1" card
    Then within 5000ms the database has document "rooms/r1" with field "participantIds" containing 50000060
    Then within 3000ms Theo's app UI shows the room screen
    Then Theo's app UI shows his seat as available (he is NOT auto-seated as host on rejoin — must request seat)
    Then Theo's app UI mic indicator shows "muted"
  ...
  Scenario: Warning while listening (not seated) — banner instead of screen
    ...
    Then within 5000ms Theo's app UI shows the warning banner overlay on top of the room
    Then within 5000ms Theo's app UI is still in the room but unable to interact
    Then within 5000ms Theo's app UI shows the acknowledge button in the banner
    When Theo on the app taps acknowledge
    Then within 3000ms Theo's app UI continues normally in the room

  @blocker @ios-sim
  Scenario: Warning with seated minor in a minor-cohort room — same cascade
    Given Marcus [P-04] is on the app seated in a minor-cohort room with mic open
    When Greta on Web Admin issues a warning to Marcus
    Then within 5000ms Marcus's app UI shows the warning screen
