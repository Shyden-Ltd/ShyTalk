...
Feature: j07 — Adam discovers + messages Alice
  As an age-verified adult on the app
  ...
  Background:
    ...
    Given Adam [P-01] is signed in on the app with cohort=adult and ageVerified=true (post-j01 state)
  ...
  Scenario: Adam discovers Alice via search and lands on her profile
    ...
    When Adam on the app types "Alice" into the search field
    Then within 3000ms Adam's app UI shows Alice's user card
    When Adam on the app taps Alice's user card
    Then within 2000ms Adam's app UI navigates to Alice's profile screen
    Then Adam's app UI shows Alice's displayName "Alice (P-02 adult power)"
    Then Adam's app UI shows Alice's stats (followers, following, beans)
  ...
  Scenario: Adam follows Alice — graph mirrors on both sides + Alice's Web UI ticks the counter
    ...
    Then within 3000ms Adam's app UI replaces follow button with "profile_unfollowButton"
  ...
  Scenario: Alice replies — Adam sees the reply + both sides see read receipts
    ...
    Then within 3000ms Adam's app UI shows the reply in the thread
    Then within 5000ms the database has document "messages/<reply-id>" with field "readBy" containing {adamId}
    Then Adam's app UI shows "read" indicator on his original message
  ...
  Scenario: Adam edits a PM within the edit window — Alice sees the update in real time
    ...
    When Adam on the app long-presses the message and taps "Edit"
    When Adam on the app changes the body to "typo here" and confirms
  ...
  Scenario: Adam deletes a PM after the edit window — message replaced with tombstone
    ...
    When Adam on the app long-presses the message and taps "Delete"
    Then within 3000ms the database has document "messages/<id>" with field "deleted" equal to true
    Then within 3000ms Adam's app UI shows "Deleted message" tombstone
  ...
  Scenario: Adam attempts to PM a minor user — 404 wall + audit row + no conversation created
    ...
    When Adam on the app attempts to start a conversation with Marcus via POST /api/conversations
