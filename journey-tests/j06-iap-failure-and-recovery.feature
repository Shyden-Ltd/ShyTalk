...
Feature: j06 — Alice IAP failure paths
  ...
  Background:
    ...
    Given Alice [P-02] is signed in on the app physical with shyCoins=5000
  ...
  Scenario: Receipt replay attack — same receipt submitted twice → second call 409, no double credit
    ...
    When Alice on the app POSTs /api/economy/purchase with productId="coins-1000" and receipt "receipt-R1" again
  ...
  Scenario: Network drop AFTER server credits but BEFORE client confirmation → retry is idempotent
    ...
    When Alice on the app retries the same purchase (same receipt) once network restores
    ...
    Then Alice's app UI shows "Purchase already completed" toast

  @blocker @android-physical @ios-physical
  Scenario: Invalid receipt (signature fail) → 400 + no credit + audit row
    When Alice on the app POSTs /api/economy/purchase with productId="coins-1000" and receipt "INVALID_BASE64_BLOB"
  ...
  Scenario: Missing productId → 400 with clear message
    ...
    When Alice on the app POSTs /api/economy/purchase with purchaseToken="receipt-R1"
    Then the response status is 400
  ...
  Scenario: Mismatched productId / receipt pair → 400, no credit
    ...
    When Alice on the app POSTs /api/economy/purchase
    Then the response status is 400
  ...
  Scenario: Admin refund — coins back out, audit row, transaction marker
    ...
    Then within 5000ms Alice's app UI shows the new "5,000" balance via Firestore listener

  @android-physical @ios-physical
  Scenario: Gacha pull with insufficient balance — clear 402 + no deduction
    Given Alice has shyCoins=50
    When Alice on the app POSTs /api/economy/gacha with pullCount=1 (costs 100)
    Then the response status is 402
    Then the database has document "users/50000010" with field "shyCoins" equal to 50
    Then Alice's app UI shows "Not enough coins" toast
