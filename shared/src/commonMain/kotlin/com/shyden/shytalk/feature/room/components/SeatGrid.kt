...
fun SeatGrid(
    ...
    val visibleSeats =
        remember(seats, effectiveSeatCount) {
    ...
    val (displaySeats, requestSeatIndex) =
        remember(visibleSeats, showRequestSeat) {
            val occupied =
                visibleSeats.entries
    ...
    Row(
        ...
        seats.forEach { (indexStr, seat) ->
            key(indexStr) {
                ...
                val seatRole =
                    remember(seatUserId, ownerId, hostIds) {
                        when {
                ...
                val isDisconnected =
                    (seatUserId != null && seatUserId in disconnectedUserIds) ||
                        (isOwnerAway && seatUserId == ownerId)

                val isOwnerOnOwnSeat =
