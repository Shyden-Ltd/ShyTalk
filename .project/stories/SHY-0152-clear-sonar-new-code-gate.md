...
- 2026-07-01 — **Gate reached OK.** The pre-push sonar scan confirmed the 2 bug fixes cleared `new_reliability_rating`→A, but the 2 `S5693` upload findings persisted (`new_security_rating`=C): `files: 1` hardening does NOT clear a review-class "make sure this is safe" rule. Operator (asked with full context — findings shown, safety reasoning explained) chose **mark reviewed-safe**. Recorded a justification comment + `accept`→`RESOLVED/WONTFIX` on both (banners.js:21 `AZ8dGJGaHYxAQGlr2vGq`, storage.js:25 `AZ8dGJPuHYxAQGlr2vGr`) via the SonarCloud API — the rule's intended review workflow, auditable, NOT a code suppression ([[feedback-never-suppress-fix-or-upgrade]] respected: this is review-resolution, not `// NOSONAR`). Gate re-queried → **OK** (reliability A + security A). **Circular dependency broken:** this branch inherited main's 11 pin-drift failures (SHY-0142's fix was on a separate branch) → its CI would fail; and SHY-0142 couldn't push (its tree lacks these reliability fixes → gate red). Neither could land alone, so cherry-picked SHY-0142's commit onto this branch ([[feedback-stacked-pr-squash-merge-pattern]]): combined tree is fully green (pin 98/98 + sonar 66/66 + gate OK). Lands as ONE unblock-main PR carrying both stories; the redundant local `fix/SHY-0142` branch is dropped (it was never pushed, so no separate PR to close).
- 2026-07-01 (AFK autonomous session) — **Third gate metric surfaced + fixed: duplication.** After the reliability/security fixes, the CI SonarCloud gate flipped to ERROR on `new_duplicated_lines_density` = **10.9%** (limit 3%) — diagnosed via the PR-scoped `api/qualitygates/project_status?pullRequest=1527` (single failing condition; not guessed). Root cause: the I1 fix above *mirrored* `banners.js`'s multer error-callback into `storage.js`, which by construction created a duplicated ~10-line block (the exact thing the metric flags). **Fixed by the refactor the metric points at** (not a suppression, [[feedback-never-suppress-fix-or-upgrade]]): extracted the wrapper into a new shared helper `express-api/src/utils/upload.js` — `singleFileUpload(upload, field)` — both routes now call it; each `multer({…limits…})` config stays inline so the S5693 reviewed-safe marks (keyed to `banners.js:21` / `storage.js:25`) don't re-fire at new line numbers. **Verified:** storage+banners route suites **55/55**; **full backend suite 342 suites / 12,520 tests green**; `upload.js` **100%** line/branch/func coverage (all 3 branches exercised by the real route tests — no new double); eslint `--max-warnings=0` + prettier + no-new-stubs ratchet all clean. `code-reviewer` re-dispatched on the dedup diff (Reviewed-up-to bump pending its result); local device+browser gauntlet launched detached (run-journeys `20260701-184151-local`: real Android CPH2653 + real iPhone + all browsers). **Merge authority (operator, this AFK session):** merge is AUTHORIZED once the local gauntlet → dev gauntlet are BOTH completely green; on merge, cut a release + set `released_in`. Still judgment-gated, no auto-merge.
- 2026-07-01 (AFK) — **`code-reviewer` on the dedup: 0 Blocker / 0 Major / 1 Minor / 1 Polish — both fixed + verified.** **M-1** (Minor): the JSDoc described the 400 branch as "any other *multer* error", but `banners.js`'s `fileFilter` throws a plain `Error` (no `.code`) that also lands there — reworded to make the broad catch explicit. **P-1** (Polish): the `LIMIT_UNEXPECTED_FILE` (wrong multipart field name) path wasn't pinned in either route suite — added a guard to `storage.test.js` + `banners.test.js` asserting **400 `{error:'Unexpected field'}`** (also verifies the reviewer's claim). Verified: **57/57** route tests green (was 55); eslint `--max-warnings=0` + prettier clean. 100%-clean bar met ([[feedback-100-percent-clean-reviews]] — fixed the Minor+Polish, not just the Blockers). `Reviewed-up-to` will be set to the commit SHA. Now blocked only on the local device+browser matrix (`20260701-184151-local`) finishing green before commit→push→dev→merge→release.

- **2026-08-20 — returned to Draft.** This story's only PR (#1527) was closed by
  operator decision rather than rebased: it was seven weeks old, most of its
  content had been superseded, and its CI results were stale. The story itself is
  still valid and unstarted — it was rescued out of that branch, where it existed
  nowhere else, and needs a fresh branch off current develop.
