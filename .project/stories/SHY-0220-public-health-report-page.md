...
- [ ] **Simple-top / detail-below (operator's choice):** the five cards are the top summary; an expandable section below each (or a single expandable panel) shows plain-language detail — which aspects were checked and their status — **with no test jargon** (no "suites", "assertions", "stack traces").
- [ ] The page is fed by a generated `public/health-data.json` built from the SHY-0212 `run-summary.json` + per-framework `metadata.json` by a sync script (`scripts/sync-health-data.mjs`), mapping each framework's `publicArea` onto the five user areas — mirroring the existing `sync-shy-to-roadmap-data.mjs` architecture.
...
- **Blocked by:** SHY-0212 (the `run-summary.json` + per-framework `metadata.json` contract + `publicArea` field it rolls up). Benefits from every other child (0213–0219, 0221–0225) emitting their feed, but renders gracefully with partial data. SHY-0224 can post a live signal into the same JSON.
