...
| [SHY-0069](SHY-0069-pin-local-node-and-hook-observability.md)   | P0  | S      | infra    | Pin local Node to CI version + pre-push hook observability + watchman config                      | 🚧 In Progress | —                | —   |
| [SHY-0061](SHY-0061-renderer-reads-shy-items.md)                | P0  | M      | feature  | Roadmap renderer reads SHY-derived `phases[].items[]` with shyId badges                           | 🚧 In Progress | —                | —   |
| [SHY-0040](SHY-0040-sync-script-per-file-overhead.md)           | P1  | S      | refactor | Cut sync-stories-to-issues.sh per-file subprocess overhead (37.3s→≤10s for 66 files)              | 🚧 In Progress | —                | —   |
| [SHY-0068](SHY-0068-cache-sonar-engine-jar-ci.md)                | P1  | S      | infra    | Cache SonarCloud scanner-engine JAR in CI (eliminate WAF-flake download)                          | 🚧 In Progress | —                | —   |
...
| [SHY-0021](SHY-0021-add-cron-account-deletion-endpoint-test.md) | P0  | S      | infra    | cron-account-deletion endpoint integration test (auth coverage)                                  | 🚧 In Progress | G021             | —   |
...
| SHY-0070     | System-routes observability: structured `sweep_run` logging (auth-result WARN w/ IP, INFO counts) + `{swept,errors}` plumbing from accountDeletion() through createSweepHandler                                  | Deferred from SHY-0021 (architect, 2026-06-10)           |
| SHY-0071     | Sync wall-clock: batch per-file `gh issue list` lookups (one upfront list → bash map; ~25s of 36s) + `--scan`-mode batch validation (~9s) — baseline measurements in SHY-0040 Notes                                  | After SHY-0040 merges (parse layer single-pass)          |
