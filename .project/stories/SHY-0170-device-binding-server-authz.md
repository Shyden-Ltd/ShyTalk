...
- **iOS CLIENT enforcement of the sign-in device-lock + ban checks.** Review found iOS hardcodes `bypassDeviceChecks=true` (`IosPlatformModule.kt:131`, no `BuildVariant` gate), so the sign-in device/ban checks never run on any iOS build. This is a pre-existing gap already scoped under **EPIC-0005 (ban-enforcement-hardening)** — whose thesis is exactly "the single client-triggered sign-in check is bypassable → enforce server-side per-request" (SHY-0149) + reinstall-proof device bans (SHY-0151). SHY-0170 delivers the server device-lock endpoint + Android enforcement + the client-repo migration that makes enabling iOS device-lock *safe* (it removed the old direct-Firestore read that crashed iOS); actually enabling the iOS client checks belongs to EPIC-0005, not here. NOT a new ticket.
...
- 2026-07-09 — **Implemented, TDD, two increments (server `917f46949d5`, client `183573dd0c7`), pushed.** Server (RED→GREEN, real Firestore/Auth emulator): `POST /api/devices/lock-check` (atomic `runTransaction` read→decide→conditional-bind; identity from `req.auth.uniqueId`, never body; `{status,boundToOther}`) — 9 tests incl. concurrent-race one-winner + legacy `{userId}`; `device-info` no longer unconditionally rebinds a foreign-owned device (2 tests); `firestore.rules deviceBindings` → `allow read, write: if false` (8 real-emulator rules tests: user/admin/anon read + create/update/delete all denied). Client: `DeviceRepository` → `resolveDeviceLock(deviceId): Resource<DeviceLockStatus>` (enum ALLOWED/LOCKED); `AuthViewModel.resolveDeviceLockOrBlock()` unifies both branches (LOCKED→signout+isDeviceLocked; Error→lenient); Android impl drops FirebaseFirestore (baseline 34→33); iOS impl drops the firestore param + DI updated. Tests: **4 new device-lock cases in commonTest (JVM AND iOS)** + rewritten `DeviceRepositoryImplTest` (API-mapping via WorkerApiClient double) + migrated mockk stubs + androidTest fake/journey. **Investigation reconfirmed the security value:** the old flow read `deviceBindings` on-device and let the CLIENT decide the lock — bypassable; now the API decides. Verified: `:shared:jvmTest` 31 green · `:app` device+auth unit green · `:shared:compileKotlinIosArm64` clean · ktlint+detekt clean · 48 ratchet tests green. **Admin console confirmed already server-side** (`/api/admin/devices`) so the rules full-deny is safe.
- 2026-07-09 — **code-reviewer round-1 + fixes.** Reviewer verified-clean the security core (transaction atomicity, status/boundToOther consistency, uniqueId coercion, new-user-never-binds, rules deny-all, migration completeness, deliberate lenient-error). Findings fixed: **Important — no rate-limit** → `writeLimiter` on `/api/devices` + `/api/device-info`; **Important — no deviceId validation** → shared `src/utils/deviceId.js` (`^[A-Za-z0-9._-]{1,200}$`, rejects `/`-path-redirection / whitespace / non-string / over-length) wired into both routes, with a 21-case unit test + route-level tests; **coverage** — Android impl test now pins the exact `/api/devices/lock-check` path + body; commonTest +NotFound-ALLOWED, +NotFound-Error, + a negative "checkBanStatus NOT reached after LOCKED" (fake now call-tracks); + a both-fields `uniqueId??userId` precedence test; **+ 2 REAL-emulator `/api/device-info` reconcile tests** (a mock can't prove `merge:true` preserves the foreign owner — now proven end-to-end). **Critical — iOS `bypassDeviceChecks=true` (device+ban checks skip on all iOS builds):** confirmed real, but it is a PRE-EXISTING gap already scoped under [[EPIC-0005]] (operator caught the near-duplicate — [[feedback-check-existing-ticket-before-creating]]); NOT fixed here, DoD + Out-of-Scope updated to route iOS enforcement to EPIC-0005. Re-verified: server 68 tests green (validator 21 + lock-check 15 + device-info 32) · `:shared:jvmTest` AuthViewModelIdentityTest 33 green · Android DeviceRepositoryImplTest green · ktlint clean · ratchet exit 0. NEXT: re-review the delta → PR develop → full gauntlet (device-lock sign-in journey batched to real-device) → judgment-merge.
- 2026-07-11 22:55 WIB — **The routed iOS gap is being closed under [[SHY-0151]] (EPIC-0005's open child), increment 1:** `bypassDeviceChecks` now variant-resolved (local-only) + fail-closed via BuildVariant/doInitKoin — so this story's `resolveDeviceLockOrBlock()` actually runs on iOS dev/release builds. See SHY-0151 Notes for the full fix record.
