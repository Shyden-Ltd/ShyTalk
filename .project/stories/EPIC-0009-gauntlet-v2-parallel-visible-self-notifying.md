...
- **SHY-0238** (DONE, merged to develop) — Gauntlet-v2 orchestrator: overlap the state-independent framework suites with the already-parallel device matrix (devices start immediately after the initial reseed; the Auth-mutating Jest stays in a non-overlapping slot, its wipe already healed by 50-matrix's own reseed) + live console streaming (still write the file) + an aggregated tally. The core, and the measurable speed + visibility win.
- **SHY-0239** (DONE, merged to develop) — Pause/ping + notification mechanism: PIN-ready start gate (pause + ping + await confirm before start), event-driven completion/failure/first-failure pings to console + phone (assistant-mediated; fail-fast; no token-burning polling), and a `--status` reader. The per-moment mid-run PIN pause was dropped as infeasible (the detached runner is unattended by design) — the START gate + pre-disabled secure lock is the sanctioned handling.
- **SHY-0240** — Pre-flight smoke (device-free, AFK-verifiable): a fast bash+curl data-plane round-trip (`/api/health` → persona `signInWithPassword` → owner-gated `PATCH /api/users` → read-back) wired as a `phase "smoke"` BEFORE matrix-dispatch, that `die`s → FAIL sentinel → aborts the run early if the plumbing is dead. For this API-only backend the read-back leg IS the propagation-readable proof.
- **SHY-0241** — Harden cross-platform real-time journey coverage (device-dependent): add live admin/web → Android/iPhone propagation assertions to j01 + j04 (j10/j11 already cover it; consider upgrading their `@ios-sim` to `@ios-device`). **Split out of the original 0240** because it can only be verified by driving real Android + iOS device UIs through the matrix — a hard gate that needs the operator + devices present (the release session), NOT AFK-verifiable.
...
**2026-07-24:** Filed at operator direction after v1's serial/invisible/babysat run surfaced the pain during the develop→main release. First draft proposed persona-sliced parallel lanes; the grounding investigation (see Vision) proved that unsound and surfaced that the matrix is already 3-way parallel and the real bottleneck is the framework-before-matrix phase. Operator chose **Pragmatic v2** (overlap + visibility + pause/ping + self-notify + smoke + cross-platform coverage; no data-isolation rewrite). Sequence: SHY-0236 (done) → SHY-0238 (overlap + streaming) → SHY-0239 (pause/ping + self-notify) → SHY-0240 (smoke) → SHY-0241 (cross-platform journeys) → first v2 run doubles as the release gate + iOS verification → develop→main.

**2026-07-25:** SHY-0238 + SHY-0239 merged to develop. Building SHY-0240, the original "smoke + cross-platform coverage" child was **split**: the pre-flight smoke (0240) is pure device-free tooling, fully verifiable against the local stack and buildable/mergeable AFK; the cross-platform journey hardening (0241) can only be verified by driving real Android + iOS device UIs (a hard matrix gate) and is deferred to the operator-present release session. SHY-0239's mid-run PIN pause was also dropped (infeasible for an unattended detached runner) — the START gate covers it. So the epic's remaining path is: SHY-0240 (smoke — building) → the release run (which also exercises SHY-0241's device journeys + SHY-0239's phone-push end to end) → develop→main.
