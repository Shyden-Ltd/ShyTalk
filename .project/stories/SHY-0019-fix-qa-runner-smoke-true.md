...
1. Add `express-api/tests/scripts/manual-qa-runner-smoke-flag.test.js`:
   ...
   - Test B: runner invoked with same flag + a forced parse error **induced for real** (feed the runner a genuinely malformed journey/config input on disk, not a mocked error, per `CLAUDE.md` § No Stubs / Mocks / Fakes — Real Only) → exits non-zero.
...
**Not `*.md`-only** (changes `manual-qa-runner.js` + a workflow) → the FULL gauntlet applies. The twist: this story modifies the **journey runner itself**, so the most meaningful impact test is that the full journey corpus still executes correctly THROUGH the modified runner.

**Frameworks exercised (RED→GREEN before code):**
- ✅ **Express/Node Jest** — `manual-qa-runner-smoke-flag.test.js`: ECONNREFUSED-on-known-port → exit 0 + skip message; parse error / auth-failure → non-zero; no-flag → unchanged; YAML grep for `|| true` → 0 matches. The story's primary RED→GREEN.
- ✅ **eslint** (`--max-warnings=0`) + **actionlint** — the runner JS + the edited `qa-runner-driver-checks.yml`.
- ✅ **Manual-QA journey matrix** — run the FULL journey corpus on **real Android + real iPhone + all browsers** THROUGH the modified runner, proving the exit-code change didn't break journey execution or dispatch (this IS the impact test, since the runner is the harness).
- ✅ **[[feedback-workflow-verify-by-running]]** — dispatch `qa-runner-driver-checks.yml`; observe it PASS with the local stack down (SKIPPED) AND FAIL on an intentionally-broken runner state. Non-negotiable per the rule.
- ⬜ **Kotlin / detekt / ktlint / iOS compile / Playwright web** — N/A (no app/web/Kotlin change); apps run the corpus as the net (above).
- ✅ **SonarCloud** — quality gate.

**LOCAL gauntlet:** Jest + lint green → the full journey corpus runs cleanly through the modified runner on real Android + real iPhone + all browsers. Any failure → fix TDD → restart.
**DEV gauntlet:** the workflow-dispatch verification (PASS-when-stack-down / FAIL-when-broken) on the unmerged branch is the DEV gate; full journeys on real devices, web = Chrome. **Judgment-merge** only when production-ready with zero doubt — this fix exists precisely to STOP silent green, so its own verification must be airtight.
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): Jest + lint green + the full journey corpus runs cleanly through the modified runner on **real Android + real iPhone + all browsers** + the workflow-dispatch verification (PASS-when-stack-down / FAIL-when-broken, per [[feedback-workflow-verify-by-running]]) → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green → **judgment-merge** (zero doubt; NO auto-merge).
...
- 2026-06-12 ~23:59 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): changes the journey RUNNER itself → the impact test is the full corpus running cleanly THROUGH the modified runner on real devices + all browsers, plus the mandatory workflow-dispatch verification ([[feedback-workflow-verify-by-running]]: PASS-stack-down / FAIL-broken). DoD auto-merge → judgment-merge. Pickup-fitness: no dupes/stale found.
- 2026-06-13 ~01:45 BST — **No-Stubs scrub** ([[feedback-no-stubs-mocks-fakes-real-only]]): the Test Plan's "forced parse error (mock)" is replaced by a REAL induced malformed input — the runner genuinely fails to parse a real bad file on disk, never a mocked error. The rest of the ticket already exercises the runner end-to-end on real devices/browsers, so nothing else to scrub.
