...
As the ShyTalk operator, I want **Verify Room mutation P3 deploy status + reconcile** delivered per the roadmap row(s) for G009 G027, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-A2` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G009 G027. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G009 (TBD on pickup)**

- **Given** the spec for G009's gap as documented in the roadmap row
...
**Scenario: Refined behaviour for G027 (TBD on pickup)**

- **Given** the spec for G027's gap as documented in the roadmap row
- **When** the implementation lands per the Fix column guidance
- **Then** the AC bullets pinned at pickup pass
- **And** the validator + reviewer agents return ZERO findings
...
- Roadmap row(s) for G009 G027 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-A2` (roadmap_ids: G009 G027). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
