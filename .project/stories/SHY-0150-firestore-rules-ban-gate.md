...
- 2026-07-11 — **code-reviewer R2: ZERO FINDINGS — MERGE-READY → In Review.** All six R1 closures independently verified by the reviewer tracing current source (per-finding CLOSED verdicts: dedup guard mutation-sensitive; dual-target fan-out traced through both standing paths; the 3 admin surfaces gated with reads preserved — blockedTopics public read `if true` intact, identityGraphs read half ungated; ASN bridge, null-uniqueId early-return, oldest-key eviction each exercised). New-issue sweep on the fix commit: none; no-stubs compliance confirmed; ratchet recount 32 raw − 1 comment = 31 exact. `Reviewed-up-to: c31e59dc19a`. Flipping In Review; push + PR → develop next (autonomous merge per develop authority). Verify on the merge push: the SHY-0177 organic sync-trigger proof (a sync run must auto-fire) + dispatch Deploy-To-Dev (`ref: develop`) after merge.
- 2026-07-11 — **code-reviewer R1: 2 Critical + 1 Important + 3 Minor — ALL FIXED (no production-behavior bugs found).** (C1) dedup guard had no removal-detecting test → R1-1 asserts the guard's own `reason:'dedup'` return, distinct from the idempotent no-change path; **mutant (guard disabled) proven red then restored**. (C2) `syncClaimsForDeviceBan` dual-target fan-out (linked account ≠ bound owner) never exercised together → R1-2 bans one device with a linked user AND a different bound owner, asserts BOTH claims minted; **mutant (first-target-only) proven red then restored**. (I3) blockedTopics write / suggestionDisputes update / identityGraphs write were admin-only WITHOUT the gate, contradicting the banned-outranks-admin invariant → RED-first engine tests (3 denies + clean-admin controls + reads-unchanged pins), then gated all three (identityGraphs read/write split so admin reads stay ungated; Admin-SDK console flows unaffected); ratchet floor 28→31. (M4) ASN-attribution branch of `computeUserBanStanding` → R1-4 (stored binding `AS64500` matches digits-only ban value via normalizeAsn, recompute mints). (M5) null-uniqueId lazy path → R1-5 (doc-less token + IP-matched ban → 403, no crash, no mint). (M6) dedup-map bound → R1-6 (501 distinct marks; oldest key evicted and re-syncs, recent key still deduped). Suites after fixes: rules-dir 253/253 (engine suite 94), lifecycle 25/25, statics 7/7; eslint/prettier clean; rules dry-run compiles. Full express suite single-run stamp: **13315/13315 EXIT 0** (first run's 61 cron fails were MinIO/Docker down post-reboot — fixed by booting the stack, unrelated to this diff).
- 2026-07-11 — **TDD RED→GREEN complete (real engines, both layers).** RED first, failures verified for the right reason: (1) `tests/firestore-rules/banned-claim-rules.test.js` — 90 tests against the REAL Rules engine; a 30-row OPS table (every gated collection+verb) generates paired banned-denied/clean-allowed tests so a deny is provably the claim's doing; 32 RED (30 DENY + 2 banned-admin) with "Expected request to fail, but it succeeded", 58 controls green. (2) `tests/routes/banned-claim-lifecycle.test.js` — 20 tests against the REAL Auth+Firestore emulators (no doubles; real `authMiddleware` in the chain, real ID tokens via `mintRealUser` + new `extraClaims` option): 11 RED (mint/clear/revoke asserts, `Received: undefined`), 9 green-by-design guards (anti-blind-clear ×3, adversarial non-admin/anon, recompute-keeps, no-double-revoke, exempt-path-no-sync, merge pin). GREEN: `isBanned()` token-only helper + `!isBanned()` on 28 write verbs (READS untouched; `settings` + `subscriptions` combined rules split so their read halves stay ungated; suggestions-update restructured so `banned` outranks the admin branch), `computeUserBanStanding` (utils/bans.js — user-attributable recompute: device standing + linked network bans + binding-ASN matches; IP-free by design), `syncBannedClaim` chokepoint (utils/banned-claim.js — mint via `mintClaimsMerging` + `revokeRefreshTokens` on set, NEVER revoke on clear, recompute-only clear authority, 60s lazy dedup, never-throws), hooks in all 5 `admin-bans.js` mutations (unbans capture `linkedUniqueId` pre-delete; device paths also sync the BOUND owner) + `autoApplyBans`/`liftAutoAppliedBans` + BOTH middlewares (verdict-driven mint covers the unlinked-network structural limit; clear only via recompute). All 20 lifecycle + 90 rules tests green; sibling rules suites 249/249. (3) Static pin suite `banned-claim-rules-static.unit.test.js` (7 pins): token-only/no-`get()` (Performance AC), gate-count ratchet ≥28, split-rule read halves ungated, observability log lines, single-revocation-site, middleware wiring parity. **Mutation-verified**: a dropped-gate mutant (notifications update) turned the engine DENY test red AND exposed the ratchet's original floor as off-by-one PLUS comment-counting — both weaknesses fixed (comment-stripping count, floor 28), mutant re-run red on both layers, gate restored, 249/249 green. eslint `--max-warnings=0` + prettier clean; `firebase deploy --only firestore:rules --dry-run` compiles clean.
