...
# SHY-0025: Locale parity test key-set + PR #1010 string verification

## User Story

As the ShyTalk operator, I want **Locale parity test key-set + PR #1010 string verification** delivered per the roadmap row(s) for G042 G052, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-I3` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G042 G052. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G042 (TBD on pickup)**

- **Given** the spec for G042's gap as documented in the roadmap row
...
**Scenario: Refined behaviour for G052 (TBD on pickup)**

- **Given** the spec for G052's gap as documented in the roadmap row
- **When** the implementation lands per the Fix column guidance
- **Then** the AC bullets pinned at pickup pass
- **And** the validator + reviewer agents return ZERO findings
...
- Roadmap row(s) for G042 G052 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-I3` (roadmap_ids: G042 G052). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
