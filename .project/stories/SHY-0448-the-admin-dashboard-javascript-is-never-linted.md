...
- 2026-09-01 — **Reformat: 37 files, 5,510 insertions, 3,858 deletions**, in its own commit. A first attempt formatted the locale tables as well and came to 40 files / 11k insertions; excluding them halved it and removed the risk entirely. Verified semantics-preserving rather than assumed: the 228 Jest suites that load `public/` are identical before and after at 8889 passing, the lint ratchet is unchanged at 56 findings, and the full Playwright chromium suite is green at **1429 passing**. (The Jest baseline also caught stale local `node_modules` — the lockfile had firebase-admin 14.3.0 and 14.2.0 was installed; `npm ci` fixed it, which is that SHY-0371 test doing its job.)
...
- 2026-09-01 — **The formatter and the source-scanning specs collide, and the dangerous half is the one that stays GREEN.** 20 web specs read `public/` JavaScript as text and assert on it — that a key is reached through `sgT()`/`tAdmin()`, and that the English string is gone. All of them hardcoded double quotes, which `public/` happened to use because nothing had ever formatted it. A positive assertion that stops matching goes red and gets fixed; a **negative** one — `not.toMatch(/showToast\("Vote failed:/)` — silently stops being able to fail and passes forever while the thing it prevents walks past. The quotes are now matched as `['"]`, and it is mutation-proven: re-introducing a single-quoted `'just now'` into `suggestions-board.js` turns `suggestions-relative-time-i18n` red, where before it stayed green.
- 2026-09-01 — **The locale tables are excluded, and that decision came from the tests.** A first pass formatted them and broke locale-completeness specs across `event-translations.js`, `admin/translations.js` and `portal-translations.js`. They are DATA: several written one line per language so a missing translation is visible at a glance across twenty, and all six matched by shape in 36 specs. `express-api/.prettierignore` had already said this about `legal-translations.js` — the entry simply never took effect from the root.
- 2026-09-01 — Converged LOCALLY before pushing, not through CI: the full Playwright chromium suite was run with the same env the pre-push hook uses (`API_BASE_URL`, `WEB_BASE_URL`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `TEST_API_KEY`) until green — **1429 passed, 2 known flakes that passed on retry, 36 skipped**.

Reviewed-up-to: d97552f9abd
