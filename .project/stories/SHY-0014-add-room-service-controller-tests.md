...
**Not `*.md`-only** (adds platform tests + may fix the Android/iOS controllers) → the FULL gauntlet applies. This story is **inherently cross-platform** — its whole purpose is Android↔iOS voice-room parity — so the device gauntlet on BOTH real devices IS the point, not an add-on.
...
- ✅ **SonarCloud** — coverage gate (≥85% on both controllers, per AC).

**LOCAL gauntlet:** both platform unit suites green → the voice-room leave/background journeys on real Android + real iOS (parity check) → impact-selected each loop, full corpus at the pre-push gate. Any failure → fix TDD → restart the whole local gauntlet.
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): both platform unit suites green + voice-room parity journey green on **real Android + real iOS device** (NOT a simulator — supersedes the old "iOS simulator" smoke) → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green → **judgment-merge** (zero doubt; NO auto-merge).
