...
**Not `*.md`-only** (adds Kotlin commonTest + may fix `shared/commonMain` core code) → the FULL gauntlet applies. All three components are `commonMain` substrate → fixes ship to **both** apps. `ModerationFilter` is the **OSA #17 safety boundary**, so its gauntlet is non-negotiable.

**Frameworks exercised (RED→GREEN before any production fix):**
- ✅ **Kotlin/JVM unit** — `RoomLifecycleManagerTest` + `AnimationQueueTest` + `ModerationFilterTest` (`./gradlew :shared:jvmTest`), incl. the adversarial unicode/RTL/zero-width bypass matrix; the story's primary RED→GREEN.
- ✅ **detekt + ktlint** — new test + any production-fix Kotlin passes static analysis.
- ✅ **iOS shared compile-check** — `./gradlew :shared:compileKotlinIosArm64` (commonMain fixes must not break the iOS build).
- ✅ **Android instrumented BDD + Manual-QA journey matrix** — on a **real Android device AND a real iOS device**: join→seat→leave a voice room (RoomLifecycle race), send a gift (AnimationQueue ordering), and send a message / set a room name containing a banned term (ModerationFilter block surfaces the non-revealing UX message). Any production fix re-proven on the live surfaces.
- ⬜ **Web E2E / integration / eslint / Express Jest** — N/A (no web/API surface); the full journey corpus still runs as the regression net.
- ✅ **SonarCloud** — coverage gate (≥85% line / ≥80% branch, per AC).

**LOCAL gauntlet:** unit suite (incl. the bypass matrix) green → room/gift/moderation journeys on real Android + real iOS → impact-selected each loop, full corpus at the pre-push gate. Any failure → fix TDD → restart the whole local gauntlet.
**DEV gauntlet:** redeploy the unmerged branch via Deploy-To-Dev `ref`; real Android + real iOS; web = Chrome only. Restart from LOCAL on failure. **Judgment-merge** only when production-ready with zero doubt — a moderation regression is a compliance incident, so the bar here is absolute.
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): unit suite (incl. the moderation bypass matrix) green + room/gift/moderation journeys green on **real Android + real iOS** (LOCAL) → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green → **judgment-merge** (zero doubt; NO auto-merge). The old "infra → no dev-smoke" shortcut is SUPERSEDED — a ModerationFilter/race fix IS user-visible and runs the full device gauntlet.
...
- 2026-06-12 ~23:40 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): named the framework set (Kotlin JVM unit primary incl. moderation bypass matrix + detekt/ktlint + iOS compile + room/gift/moderation journeys on real Android + real iOS). SUPERSEDED the old "infra → no dev-smoke" DoD shortcut — a ModerationFilter/race fix is user-visible safety code and runs the full device gauntlet. DoD auto-merge → judgment-merge. Pickup-fitness: no dupes/stale found.
- 2026-06-13 ~01:55 BST — **No-Stubs flag (review-surfaced)** ([[feedback-no-stubs-mocks-fakes-real-only]]): AC/Out-of-Scope name `FakeVoiceService` + injected ModerationFilter fixtures + "mocking RoomLifecycleManager" — new in-process doubles the rule bans. Same foundational-fake class as [[SHY-0010]] (🔴 operator-decision item on the SHY-0091 handoff). New work should induce real conditions against the real local stack where possible; the AC/BDD prose is superseded by the No-Stubs banner atop `## Acceptance Criteria` pending operator direction — NOT re-architected here (opportunistic, no big-bang).
