...
### UX
- [ ] The "Reset PIN" row copy ("Verify your identity to set a new PIN") matches behaviour for both first-set (no verify) and change (verify) — reword the row/desc if first-set should read differently.
- [ ] A new user is routed toward setting a PIN (wire `needsPinSetup`, or consciously defer with a documented rationale).
