...
- [ ] **Swift/iOS SAST:** iOS Swift code is statically analyzed for security issues — preferably by extending `codeql-config.yml` + `codeql.yml` to add the `swift` language (with the iOS build providing compilation), or, if the CI image can't support CodeQL-Swift within budget, a documented $0 fallback Swift security linter. Registered `sec-sast-swift` (`host`/`stack` per tool needs, `publicArea: Safety`).
