...
**Not `*.md`-only** (adds Kotlin commonTest across 10 VMs + may fix `shared/commonMain` code) → the FULL gauntlet applies. All 10 VMs live in `commonMain` → fixes ship to **both** apps. **If the pickup split fires** (the 3 A/B/C clusters), EACH replacement SHY runs the full gauntlet for ITS cluster's screens — the protocol is inherited, not skipped per split.

**Frameworks exercised (RED→GREEN before any production fix):**
- ✅ **Kotlin/JVM unit** — the 10 `*ViewModelTest.kt` files (`./gradlew :shared:jvmTest`); the story's primary RED→GREEN.
- ✅ **detekt + ktlint** — new test + any production-fix Kotlin passes static analysis.
- ✅ **iOS shared compile-check** — `./gradlew :shared:compileKotlinIosArm64`.
- ✅ **Android instrumented BDD + Manual-QA journey matrix** — the screens these VMs drive — **messaging** (conversation list, private chat, group setup, report flow), **profile** (gift wall, required-DOB onboarding), **settings** (app + room), **daily/splash** (daily reward, fun-fact splash) — walked on a **real Android device AND a real iOS device**; surfaced fixes (push-merge, clock-skew, report idempotency, moderation-name reject) re-proven on the live screens.
- ⬜ **Web E2E / integration / eslint / Express Jest** — N/A (no web/API surface); full journey corpus runs as the regression net.
- ✅ **SonarCloud** — coverage gate (≥85% per VM, per AC).

**LOCAL gauntlet:** unit suite green → the affected screens' journeys on real Android + real iOS → impact-selected each loop, full corpus at the pre-push gate. Any failure → fix TDD → restart the whole local gauntlet.
**DEV gauntlet:** redeploy the unmerged branch(es) via Deploy-To-Dev `ref`; real Android + real iOS; web = Chrome only. Restart from LOCAL on failure. **Judgment-merge** per PR only when production-ready with zero doubt.
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol) for each PR (incl. every A/B/C split sub-PR): unit suite green + affected-screen journeys green on **real Android + real iOS** (LOCAL) → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green → **judgment-merge** (zero doubt; NO auto-merge).
- [ ] `released_in: vX.Y.Z` set after the release cut (each split SHY tracks its own).
...
- 2026-06-12 ~23:35 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): named the 10-VM framework set + per-cluster screen journeys on real Android + real iOS; clarified the gauntlet is INHERITED by each A/B/C split sub-PR (not skipped); DoD auto-merge → **judgment-merge**. Pickup-fitness: no dupes/stale found.
- 2026-06-13 ~01:47 BST — **No-Stubs foundational-harness flag** ([[feedback-no-stubs-mocks-fakes-real-only]]): reuses the `FakeRepository` pattern established by [[SHY-0010]]/[[SHY-0011]] across the 10-VM batch — same foundational-fake collision (see [[SHY-0010]]'s flag + the SHY-0091 handoff's 🔴 operator-decision item). NOT re-architected here per "opportunistic, no big-bang"; no Test-Plan rewrite pending operator direction.
- 2026-07-09 — **Reconciliation (SHY-0167) — KEPT DRAFT; AC-deviation gap found in code-reviewer verification.** All 10 ViewModel test files DO exist (ConversationList/GroupSetup/PrivateChat/ReportReview/FunFactSplash/DailyReward/AppSettings/RoomSettings/GiftWall/RequiredDOB), but at `app/src/test/java/...` using JUnit4 + MockK — NOT at `shared/src/commonTest` (kotlin.test) as this story's AC requires. The ViewModels are cross-platform (`shared/commonMain`), so they have Android-JVM coverage but **ZERO iOS test-execution proof** (Tri-Platform gap). Not cleanly delivered per AC → stays Draft. **Operator decision needed:** accept `app/src/test`+MockK as standard (rewrite ACs) OR relocate to `shared/commonTest` for iOS coverage. See `.project/audit/mvp-backlog-reconciliation-2026-07-08.md`.
