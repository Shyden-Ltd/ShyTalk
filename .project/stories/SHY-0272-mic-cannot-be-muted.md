...
- **2026-08-17 ~20:5x WIB — `code-reviewer` round 1 on `1727417b9d3`: 1 Critical + 1 Important. Both verified against the real code before acting; both were real. Applied in `de58319835d`.**
  - **Critical (SHY-0272):** `RoomScreen.onToggleMic` wrapped `toggleSelfMute` in `if (hasPermission) { … }` with no else, so a denied-permission tap was swallowed whole — the operator's original report verbatim, and SHY-0272's own AC ("a refusal is surfaced rather than swallowed") went unmet. The gate was also **over-broad**: direction-blind, so a user without mic permission could not even MUTE. Fixed by refreshing from the live OS query then delegating unconditionally — `toggleSelfMute` already sets `uiState.error` (rendered as a snackbar) and blocks only the unmute direction.
  - **Coverage:** `toggleSelfMute` had 4 tests and none touched the permission path; the "Microphone permission required" string was asserted nowhere in the repo. Added two (229 → 231). **Mutation-proven via gradle output, not XML:** changing the error string kills `rejects unmute when mic permission denied, and says so`; making the guard direction-blind kills `allows mute even when mic permission denied`.
  - **Important (SHY-0275):** `Local.xcconfig` still shipped dead knobs documented as live. Reviewer found three; there were **four** (`LOCAL_FIREBASE_PROJECT_ID` too), and one was actively misleading rather than inert — it documented `?ns=demo-shytalk-default-rtdb` while `AppEnvironment.swift:77` ships `?ns=demo-shytalk`. All four deleted; the iOS-vs-Android `?ns=` knowledge kept with the correct value; `ios-local-xcconfig.test.js` count pin 6 → 2 plus a `test.each` absence guard, mutation-proven.
