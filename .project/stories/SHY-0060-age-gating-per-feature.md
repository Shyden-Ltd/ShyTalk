...
- 2026-07-08 — **Pickup-fitness + operator decision: BUILD THE ENGINE (flag-OFF), decompose the rest.** Re-validated against current code: the verified-age store **exists** (`core/model/User.kt`: `dateOfBirth: Long?`, `ageVerified`, `ageVerifiedAt`, `ageVerificationMethod`; comment already cites "Apple App Store 18+ enforcement on PMs + gacha") — so the SHY-0007 data dependency is satisfied; but there is **no per-feature `SafetyGate`/`AgeThresholds` framework yet** (net-new). Confirmed still-blocked-for-merge: (1) final threshold VALUES are an operator/product/legal decision; (2) legal + T&S sign-offs are non-skippable pre-merge gates; (3) XL scope can't be one PR. **Operator (2026-07-08, AFK AskUserQuestion) chose "Build the engine now, behind an OFF flag."** → **THIS SHY narrows to the ENGINE** and ships behind a default-OFF feature flag (inactive in prod ⇒ safe to land in develop without the sign-offs; the flag-flip is the later gate that needs final values + legal/T&S). **Engine PR scope:** `AgeThresholds` config (feature→threshold map + region-override map, values = the story's happy-path proposals **labelled PROVISIONAL**) + a CI validator (reject <13 / >21 / missing feature) + `SafetyGate.canAccess(feature,user): GateResult` sealed-result engine (age-from-DOB, region override, verified-age/Reverify, tamper compare) + server-side enforcement on the age-gated endpoints + the operator feature flag (default OFF) + full TDD (Jest ≥60 + Kotlin ≥40). **Decomposed to FOLLOW-UP SHYs (to file):** Android blocked-UX + Settings "Feature access by age" screen; iOS mirror; 11-locale i18n; legacy-account migration job; region-override legal finalisation; T&S copy review; audit-log T&S dashboard. `public` stays false. Status → In Progress.
- 2026-07-08 — **Engine increment 1/4 DONE + committed** (`0f4ccb96999` on `story/SHY-0060-age-gating-per-feature`, not pushed — partial engine). `core/safety/AgeThresholds.kt`: `Feature` enum (10 gated features), provisional `base` map, GDPR-Art.8 `regionOverrides` (DE/NL SIGNUP→16), `thresholdFor(feature, countryCode?)`, and `validate()` (COPPA-floor 13 / sanity-max 21 / missing-feature → the CI gate, enforced via :shared:jvmTest). TDD: RED (unresolved-ref) → GREEN, **11 commonTest cases** verified via JUnit XML (11/0/0), **iOS compile verified** (`:shared:compileKotlinIosArm64` exit 0), **ktlint clean**. Reused existing `DateUtils.calculateAge(dateOfBirthMillis): Int`. **NEXT increment 2/4 = `SafetyGate.canAccess(feature, user): GateResult`** (sealed Allowed / BlockedUnderAge / BlockedRegion + verified-age handling + null-DOB conservative-block + region-detection-failure conservative-max), then 3/4 Express server enforcement + audit, 4/4 default-OFF operator feature flag → full local gauntlet + reviewer + PR → develop.
- 2026-07-08 — **Engine increment 2/4 DONE + committed** (`26d414b57c7`). `core/safety/SafetyGate.kt`: `VerificationKind` (NONE/REVERIFY), sealed `GateResult` (Allowed / BlockedUnderAge(threshold, actualAge, requiredVerification) / BlockedRegion(threshold, reason)), and the pure `canAccess(feature, verifiedAgeYears: Int?, countryCode: String?)` engine + `conservativeThreshold(feature)`. Deterministic (no wall-clock/I/O) so it's identical Android/iOS/server-mirror. Encodes: age≥threshold→Allowed; unverified age→Reverify for stricter-than-COPPA-floor features but floor features stay open for legacy accounts (AC63); region-elevation vs plain-under-age distinction (AC118 vs AC136); undetected-region→conservative-max (AC61). TDD RED (41 unresolved) → GREEN, **18 commonTest cases** (full behavioural matrix) via JUnit XML (18/0/0); iOS compile verified; ktlint clean. **Safety package total = 29 Kotlin tests green.** Branch `story/SHY-0060-age-gating-per-feature` has 2 commits (`0f4ccb96999` + `26d414b57c7`), **NOT pushed** (engine incomplete). **NEXT increment 3/4 = Express server-side enforcement:** a JS mirror of AgeThresholds+gate-logic (parity-pinned to the Kotlin source-of-truth), wired into the age-gated endpoints (DM-send / gift-send / gacha-spin / voice-mic), + safety-audit log + client-age-claim tamper→403+ALERT + 100-req/min/user rate-limit, ≥60 Jest cases. Needs a backend-surface investigation first (endpoint files, audit-log helper, rate-limit middleware, feature-flag pattern). Then 4/4 default-OFF flag. All still INERT (no call sites wired) so nothing is enforced yet — safe in-progress state.
- 2026-07-08 — **Engine increment 3a DONE + committed — the JS server ENGINE (mirror + port).** `1050d252495` = `express-api/src/safety/age-thresholds.js` (JS mirror of the Kotlin config + `validate()`) **PARITY-PINNED** to `AgeThresholds.kt` — a Jest test parses the Kotlin source + asserts BASE/REGION_OVERRIDES/COPPA_FLOOR/SANITY_MAX all match (CI fails on drift); 15 Jest. `440083b2664` = `express-api/src/safety/safety-gate.js` (`canAccess` port mirroring `SafetyGate.kt`, incl. null/undefined-input edges + eqeqeq-clean `?? null` normalisation); 20 Jest. Runner = `node --experimental-vm-modules node_modules/.bin/jest tests/safety/`; **35 Jest green (15+20)**, prettier + eslint clean. **✅ ENTIRE GATE ENGINE COMPLETE: 64 tests (29 Kotlin + 35 JS), Kotlin+iOS+JS-server, parity-pinned, all verified.** Branch has **4 commits**, still **NOT pushed** (engine incomplete — no enforcement wired). **NEXT = increment 3b/4 (INTEGRATION layer):** default-OFF operator feature flag (investigate `src/routes/config.js` pattern) gating all enforcement → wire `canAccess` into the age-gated endpoints (`conversations.js`/`pm-lock-check.js` DM, `economy.js` gift+gacha, `livekit.js` voice) extracting verified-age (`ageVerified`+`dateOfBirth`→`calculateAge`) + region (`user.nationality`) → integrate `src/utils/age-verification-audit.js` (safety-audit log of blocked attempts) → client-age-claim **tamper→403+ALERT** (AC60: compare claim vs server `ageVerified` record) → 100-req/min/user rate-limit (reuse `src/middleware/rateLimit.js`, AC86) → REAL-service integration Jest (no-mocks rule; **local stack is UP** — Docker livekit/mailpit/minio + emulator UI :4000 responding). Then full local gauntlet + code-reviewer 100% clean → PR → develop.
- 2026-07-08 — **Engine increment 3b DONE — the server INTEGRATION layer (default-OFF flag + enforcement wired + audit + tamper + rate-limit).** All INERT while the flag ships OFF; REAL-emulator no-mocks tests throughout (per-file `NODE_ENV=local`, isolated `config/safety-test-*` flag docs via `__setSafetyConfigDocForTests`, real Auth-emulator tokens via `mintRealUser`). Sub-increments + commits:
  - **Flag + primitive:** `age-gating-flag.js` — `isAgeGatingEnabled(db)` reads `config/safety`.`ageGatingEnabled` (strict `=== true`) through a 60s cache, catch→fail-safe-OFF (the shipped default = NO enforcement). `enforce.js` `checkFeatureAccess(db, feature, userDataOrLoader, nowMs?)` — flag OFF→null; else lazy-load user (loader form = zero read while OFF) → rate-limit → verdict → 403 `{errorId:'AGE_GATE_BLOCKED', ageGate:{feature,verdict,threshold,requiredVerification}}` or null. Unit + integration Jest.
  - **Endpoint enforcement (7 call sites):** gacha `GACHA_SPEND`(18) + gift/gift-direct/gift-batch `GIFTING_SEND`(18) [`economy.js`, gate the sender/spender]; DM-send `DIRECT_MESSAGE_WITH_FOLLOWED_USER`(13) vs `DIRECT_MESSAGE_WITH_STRANGER`(18) by mutual-follow [`conversations.js`, 1:1 only, groups exempt]; voice seat-claim + accept-invite `VOICE_ROOM_ACTIVE_SPEAKING`(16) [`room-mutations.js`, lazy loader]. Real-route integration tests each (gacha 4, gift 8, DM 8, voice 7).
  - **Safety-audit (AC53):** `safety-audit.js` `logBlockedFeatureAttempt` — fire-and-forget (AC79) row to `safetyAudit` `{userIdHash (SHA-256, AC85 — never plaintext), feature, threshold, userAge, region, timestamp}`, wired into `checkFeatureAccess` blocks; 5 real-emulator tests.
  - **Tamper (AC60 / BDD143):** `tamper-check.js` `checkAgeClaimTamper` — client `claimedAge` compared to server DOB-age (±1yr tolerance for TZ/birthday drift), never feeds the gate; on a material lie → 403 `AGE_CLAIM_TAMPER` + awaited critical `AGE_CLAIM_TAMPER` alertManager alert (userId, claimedAge, serverAge). Wired ahead of the gate on the 4 economy spend endpoints (age-claim is economically meaningful on spend, per BDD143 gift-send; DM/voice carry no age claim). 10 unit + 7 integration + 4 real-route (incl. exact BDD143).
  - **Rate-limit (AC86):** `gate-rate-limit.js` `recordGateCheck` — pure user-keyed sliding window (100/user/60s; userId-keyed sidesteps the shared-`::1` loopback that forces the express-rate-limit middleware to skip non-prod → testable in all envs) + lazy sweep. Wired into `checkFeatureAccess`: breach → 429 `AGE_GATE_RATE_LIMITED` + one-per-window critical `AGE_GATE_ENUMERATION` alert. 8 unit + 4 integration.
  - **Two scoping decisions (documented, not silent gaps):** (1) tamper wired on the economy spend endpoints only (BDD143's surface; DM/voice requests carry no age claim); (2) AC86's "temporary account suspension" auto-action is **deferred to T&S policy / the flag-flip gate** (policy-laden + false-positive-prone per Risk line; the enumeration alert is the T&S review trigger, consistent with tamper routing to review rather than auto-acting).
  - **Totals:** safety dir + route age-gate suites all green; existing route suites regression-green (economy, conversations-cohort, room-mutations 164). ktlint/eslint/prettier clean; zero suppressions. Branch `story/SHY-0060-age-gating-per-feature`, **16 commits, not yet pushed** (pending full-suite green + code-reviewer 100% clean). **NEXT:** full express-api Jest suite → code-reviewer → status In Review → push → PR to develop (device gauntlet deferred to the MVP real-device batch per the sprint decision, since the flag ships OFF = no user-observable behaviour to walk; the flag-flip is the later gate that runs the full gating gauntlet + needs final values + legal/T&S).
- 2026-07-08 — **code-reviewer round 1 (xhigh, full branch): 1 Critical + 5 Important + coverage gaps. ALL addressed:**
  - **[Critical — FIXED] DM gate unreachable in production (String/Number id mismatch).** participantIds are stored as STRINGS (iOS `IosPrivateMessageRepositoryImpl` writes `listOf(uid1,uid2)`; firestore.rules `string(callerUniqueId()) in participantIds`; the GET route + gateCrossCohort already `.map(String)`), but `req.auth.uniqueId` is a NUMBER (auth.js `parseInt`). My DM gate compared them uncoerced → the recipient filter never matched → the gate was structurally unreachable for real conversations, AND the pre-existing POST participant-check (line 298) + recipient filter (line 331) had the SAME latent bug (would 403 String convs / self-notify). Fixed all three with the coercion pattern already proven in this file's GET route (`participantIds.map(String)` + `String(senderId)`); strict superset (a legitimately-passing conv still passes). Test fixture was seeding Numbers (masking it) → now seeds realistic STRING participantIds + a NEW legacy Number-participantIds test proves the coercion is symmetric. All 5 conversations suites green (72 tests). *The line-298/331 fix is a pre-existing route bug repaired in-scope because the age-gate cannot function without it (per [[feedback-fix-pre-existing-and-new-same]]).*
  - **[Important #2 — DOCUMENTED as follow-up] region signal (`nationality`) is user-editable + unverified.** Real GDPR-Art.8 bypass risk, but ZERO blast radius today: none of the 4 wired endpoints (GACHA/GIFTING_SEND/DM/VOICE) carry a region override — only SIGNUP does, and SIGNUP isn't server-wired. Must be backed by a verified/authoritative region source BEFORE SIGNUP is wired or any override is added to a wired feature. → the decomposed **region-override legal-finalisation** follow-up SHY.
  - **[Important #3 — CLARIFIED + JS coverage expanded] parity scope.** The DATA (thresholds) IS mechanically pinned (Jest parses `AgeThresholds.kt` source → CI fails on drift). The LOGIC (`canAccess`) is not source-pinned (a mechanical cross-language logic pin is impractical in KMP); it is covered by MATCHED behavioural matrices — accidental one-sided drift breaks that side's own matrix. JS `safety-gate.test.js` now drives ALL 10 features through `canAccess` + ES/US no-override regions + SIGNUP-override isolation (28 cases). (Kotlin `SafetyGateTest.kt` has 18; extending its matrix to full symmetry is a follow-up.)
  - **[Important #4 — DOCUMENTED deliberate distinction] rate-limit 429 is not written to safetyAudit.** safetyAudit records age-VERDICT blocks (its `{feature,threshold,userAge,region}` shape). A rate-limit breach instead fires a dedicated once-per-window `AGE_GATE_ENUMERATION` critical alert (userId+feature+count) — a stronger, de-duplicated T&S signal than a per-attempt row, and mixing rate-limit rows into the age-block audit would corrupt "how many age-blocks" counts. Deliberate split, not a gap.
  - **[Important #5 — PINNED + explained] non-finite claimedAge treated as no-claim.** `Infinity`/`-Infinity`/`NaN` cannot arrive via the wired path (JSON.parse never yields them) and the gate ignores claimedAge regardless, so nothing is bypassed; storing Infinity in a Firestore alert is also impossible. Behaviour pinned with explicit unit tests.
  - **[Important #6 — JUSTIFIED] `age-gating-flag.test.js` touches the real `config/safety`.** Necessary + intended: it's the binding-proof test (verifies the reader defaults to the real doc, not the test doc). It writes only `{enabled:false}` and deletes in before/afterEach; no suite sets it ON. (The cron global-`config`-wipe race is the separate cron-isolation follow-up, not this.)
  - **[Coverage] added:** 4 previously-untested features through `canAccess`; ES/US regions; SANITY_MAX(21)+COPPA_FLOOR(13) inclusive-boundary validator cases; future/negative DOB → fail-safe under-age; Infinity tamper; accept-invite symmetry (16/adult/unverified); Number-participantIds DM variant. **Branch now 17+ commits.** Reviewed-up-to set at the review-fix commit; re-review before merge per [[feedback-continuous-review-loop-workflow]].
- 2026-07-08 — **code-reviewer round 2 (xhigh, reviewed `34a552f55b5`): 0 Critical, 0 production-code findings.** Extensive "Verified Clean" confirmed the round-1 Critical fix is a strict superset (cross-checked vs `firestore.rules:325`, iOS `IosPrivateMessageRepositoryImpl.kt:112`, Android `PrivateMessageRepositoryImpl.kt:118` — all agree String is canonical), self-notify bug fixed, group convs unaffected, mutual-follow fails safe, region-source zero-blast-radius reconfirmed by grepping every `checkFeatureAccess(` site, 429-no-audit correct in code, non-finite claim unreachable, flag-OFF inert at all 7 sites, no `==`/eslint-disable. **3 findings, all test-coverage or pre-existing — resolved:**
  - **[#1 FIXED] `safety-gate.test.js` "every remaining feature" swept 9/10 — GACHA_SPEND was never driven through `canAccess` anywhere in the corpus** (my own comment overclaimed). Added the GACHA_SPEND(18) allowed/blocked/REVERIFY case. Now all 10 features have a direct `canAccess` verdict test.
  - **[#2 FIXED] no test proved a 429 (rate-limited) writes NO `safetyAudit` row** — the documented invariant was correct in code (`enforce.js` returns before the audit write) but unguarded. Added a delta assertion around the single rate-limited call in `gate-rate-limit.test.js` (audit-count unchanged) — a regression guard so a future reorder that audits the 429 path fails loudly.
  - **[#2 table gaps FIXED] Number-conv ALLOW path + follow-fields-ABSENT fail-safe** — added a verified-adult Number-participantIds test (proves the participant-check coercion admits a legitimate sender on the allow path, not just 403s) and a verified-sender-with-no-followingIds/followerIds test (proves the `Array.isArray` guard falls to STRANGER on its own, distinct from the REVERIFY path).
  - **[#3 OUT OF SCOPE → follow-up SHY] `translate.js:127-133` `verifyParticipant` has the IDENTICAL String/Number bug** (`participantIds.includes(uniqueId)`, uniqueId a Number, participantIds Strings). NOT a security bypass — `participantVerified=false` only skips the message-cache read/write, so translations still return via the uncached path but silently never cache DM-message translations → wasted provider calls / `FREE_DAILY_LIMIT` quota. Pre-existing, untouched by this branch's diff → its own story + PR this session (per [[feedback-fix-pre-existing-and-new-same]] + 1-PR-1-SHY), not bundled here.
  - Round-2 fixes are TEST-ONLY (no production code changed since round 2's clean production verdict). 44 touched tests green. **Reviewed-up-to: 34a552f55b5** (round-2 baseline); a tight round-3 pass confirms the test additions before the In-Review flip.
- 2026-07-08 — **code-reviewer round 3 (tight test-delta confirm, reviewed `5277848cceb`): 1 Critical + 2 Important — all in the NEW tests (production still clean). Fixed:**
  - **[Critical FIXED] the Number-conv ALLOW-path test was a tautology.** An adult verdict is `Allowed` whether the gate runs correctly OR is skipped by broken coercion, and a Number `senderId` vs Number `participantIds` passes the participant check without any coercion — so `not AGE_GATE_BLOCKED` proved nothing. Removed it. The genuine Number-conv coercion guard is the BLOCKED test (`dm-numeric-1`): a 15yo can only return `AGE_GATE_BLOCKED` if `dmRecipientIds.length === 1`, which for a Number array REQUIRES the line-317 `.map(String)` (without it → length 2 → gate skipped → no block → test fails). Strengthened it with `verdict: 'BlockedUnderAge'` and a comment stating why the allow path is deliberately omitted. (A real 200-send test isn't cheaply reachable here — the main `conversations.test.js` gets 200s via a MOCKED db; this real-emulator file asserts age outcomes via `errorId` by design, since a real 200 needs cross-cohort + recipient seeding out of scope for an age-gate test. This corrects the round-2 "Number-conv ALLOW path" claim.)
