...
- [ ] BDD coverage gap: fewer `**Scenario:**` blocks under `## BDD Scenarios` than AC checkboxes (`- [ ]` lines) under `## Acceptance Criteria` → exit 13, stderr contains `BDD coverage gap: N AC bullets, M scenarios (M < N)`. **Counting algorithm** is sectional (see Test Plan Green step 1) — checkboxes outside `## Acceptance Criteria` and Scenario blocks outside `## BDD Scenarios` are excluded.
