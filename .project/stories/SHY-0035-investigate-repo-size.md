...
- [ ] `.gitignore` updated to explicitly ignore: `/data/`, `/playwright/pr/`, `/playwright/deploy/`, `/playwright/latest/`, `/express/pr/`, `/history/`, `/kotlin/pr/`, `/android-e2e/`, plus reinforced extension blocks for `*.apk`, `*.aab`, `*.ipa`, `*.zip` (with operator-curated allowlist exceptions for any legitimate small zips currently tracked — verify via `git ls-files '*.zip' | xargs -I{} du -h {}`).
