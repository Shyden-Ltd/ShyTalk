...
- 2026-07-09 — **PICKUP BLOCKED — the AC error-path fired: premise is stale AND the re-audit surfaced a likely security gap. Escalated to operator.** Ran the mandated live re-audit (`MainActivity` + `AppLockRepository` + nav graph) before writing a word. Findings, all grep-verified against current code (source only, excluding generated `shared/build/**` headers):
  - **Premise WRONG — there is no `MainActivity` app-lock intercept.** `MainActivity.kt` touches `appLockRepository` only for (a) `updateLastActiveTimestamp()` on background (`onStop`, line 152 — the timeout clock) and (b) `hasCredential` as a start-route hint (line 468). No `isLockRequired()` gate, no `LockScreen()` render. The AC's (b)/(d)/(e) sub-bullets ("owned by MainActivity", "why NOT in SharedNavGraph", "iOS equivalent") are built on a false premise.
  - **The actual PIN-app-lock GATE is `AuthViewModel.init` (commonMain, cross-platform)** — computes `needsLockScreen = true` when `hasCredential && isAppLockEnabled && isLockRequired()` (lines 129/131) or when the Firebase session expired (145). `AppLockRepository` (commonMain, `jvmTest`-covered) supplies `hasCredential`/`isAppLockEnabled`/`isLockRequired()`/`lockTimeoutMinutes`/`storedUniqueId`.
  - **…but that gate's output is UNWIRED — the PIN-lock UI appears to be dead/never-rendered code:** `needsLockScreen` is WRITTEN (AuthViewModel 131/145) but READ nowhere in Kotlin/Swift source (only its own declaration, line 65). `Screen.Lock` (`Screen.kt:75`, `"lock"`) is DEFINED + unit-tested but **never registered** in `SharedNavGraph` (which registers SignIn/Main/Settings/… but no Lock route). `LockScreen()` (`LockScreen.kt:37`) has **zero call-sites** repo-wide (only `LockScreenViewModel` is DI-bound at `ViewModelModule.kt:47` + `jvmTest`-covered). Net: a user who enables PIN App-Lock would have `AuthViewModel` correctly flag `needsLockScreen=true` after the timeout — but nothing consumes it, so **the lock screen never appears and the app opens straight to Main**. Potential **security defect** (App-Lock does not lock), not merely stale docs.
  - **`isDeviceLocked` is a SEPARATE, LIVE feature — device-BAN, not PIN-lock.** Consumed at `SignInScreen.kt:223` (an `AlertDialog` titled `account_restricted`/`device_locked_description`), set in `AuthViewModel` 319/347 (logs "Device locked for uniqueId="/"Device bound — blocking new account creation"), cleared at 745, covered by `AuthViewModelTest`. Do not conflate with app-lock.
  - **BLOCKED on operator decision (documented, not guessed):** SHY-0056 is a *docs* story and cannot honestly document a half-wired feature. The real question is upstream: **is PIN App-Lock in MVP scope?** If YES → this is a P0/P1 **security bug** needing its own story (wire `needsLockScreen` → register `Screen.Lock` in the nav graph → render `LockScreen()`; TDD + real-device proof that the lock actually gates re-entry after timeout) — and SHY-0056 documents it *after* it works. If the feature was intentionally descoped → the vestigial `needsLockScreen`/`Screen.Lock`/`LockScreen`/`LockScreenViewModel`/`AppLockRepository` is dead code for a **cleanup** story, and SHY-0056 is Cancelled. Kept **Draft**; not force-documented. (Investigation-first per [[feedback-never-guess-always-investigate]]; artifact-exists≠behaviour-works per [[feedback-test-must-fail-if-logic-skipped]].)
