...
- 2026-06-12 ~15:00 BST — **VERIFIED + CLOSED (Done, released_in: v0.97.14).** Both fixes confirmed GREEN by live operator-gated prod-deploy smokes (single approval gate worked on both runs — one approval each):
  - **Android boot smoke SUCCESS** on **v0.97.14 run 27417981599** — verifies the #1403 API-33 foreground fix end-to-end (`dumpsys activity activities` resumed-activity read). All deploys (backend/web/Android) + API/Web/**Android** smokes green.
  - **iOS boot smoke SUCCESS** on **v0.97.13 run 27411098883** — verifies the iOS-smoke `timeout-minutes 20→45` fix (iOS deploy success + boot/launch/crash-check all ran and passed; previously cancelled mid-build at 20 min).
  - **Identical-binary rationale:** v0.97.14 ↔ v0.97.13 differ ONLY in the CI verifier script (#1403) + story docs — no app/iOS code — so v0.97.13's green iOS smoke is valid evidence for the v0.97.14 binary. v0.97.14's own iOS deploy+smoke (redundant re-confirmation of the identical binary) was completing at closeout (~16:07); not required for verification.
  - Three verifier bugs were found + fixed across SHY-0084's life (dash `set -o pipefail`; deploy-ref-pinned smoke checkout #1394; API-33 `dumpsys window windows` foreground read #1403) — each surfaced only under live prod conditions, each fixed with a value-level regression test. Closed per operator "finish the closeout".
- 2026-06-12 ~12:55 BST — **LIVE VERIFICATION (run 27411098883, v0.97.13) surfaced a THIRD verifier bug — foreground check false-failed on API 33.** Operator approved the deploy ~12:45; backend/web/Android-Play-Store all deployed `success`, but **Smoke Test (Android Boot) FAILED**. Diagnosis from the job log: the APK installed (`Success`), `am start -W` reported `Status: ok` + `Activity: com.shyden.shytalk/.MainActivity` + `TotalTime: 5275` (app drew, no crash) — i.e. **the app genuinely launched** — yet the foreground check printed `::error::ShyTalk is not the foreground activity. Got:` with an **empty** value. Root cause: line 78 read the foreground app via `dumpsys window windows | grep mCurrentFocus`, but **API 33 no longer emits `mCurrentFocus` under the `windows` subcommand**, so the grep returned empty → false-fail of a foreground app. (The empty `Got:` — not the launcher, not another app — is the tell: a parse miss, not a real backgrounding.) **Fix:** read the resumed activity from the activity manager first (`dumpsys activity activities | grep -E 'mResumedActivity|topResumedActivity'`, reliable across API levels) with the window focus (`dumpsys window`, NOT `window windows`) as a fallback. **Tests:** the unit test's adb stub was made subcommand-aware (`dumpsys activity` vs `dumpsys window` now return distinct output — the old stub returned the same string for any `dumpsys`, which is why a green test never caught this); +2 regression cases (API-33 empty-window→resumed-read passes; activity-manager-empty→window-fallback passes); **14/14 pass**. The smoke jobs check out the workflow ref (main, per the #1394 fix), so once this merges, a fresh deploy's Android smoke runs the fixed verifier even with an unchanged app binary. Operator chose **push fix + re-deploy** (cut v0.97.14 for a green-on-record Android smoke). Note: the v0.97.13 deploy still pushed v0.97.13 to prod (backend/web/Android live, iOS uploading) — only the post-deploy verification tripped; the run shows red but prod is on v0.97.13.
