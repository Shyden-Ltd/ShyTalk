...
- 2026-07-09 — **⛔ PICKUP BLOCKED on an architect/operator decision — the `errors` AC premise doesn't hold against current code.** Fitness-check confirms `swept`-side is fit + valuable (createSweepHandler:system.js:87 still `res.json({status:'ok'})` success + error-only log; accountDeletion:533 returns void; no test asserts counts). **BUT** the `errors`-side has a design flaw the story flagged for the architect: `accountDeletion` phase-1 is continue-on-error (per-account try/catch on `hardDeleteAccount`), and **ALL 12 `hardDeleteAccount` steps swallow their own errors internally** (each `cleanup*`/`delete*` has its own try/catch that logs + returns void; verified by awk-scan — 12/12 SWALLOW, 0 PROPAGATE). Consequences:
  1. A loop-level `errors` counter (the literal AC) is **~always 0** even when every step fails → misleading observability, the opposite of the story's audit intent. `hardDeleteAccount` can only throw at the un-guarded step-12 audit-log write, which is not per-account inducible.
  2. The **`errors:1` BDD scenario cannot be satisfied with REAL data** (no-mocks rule): there is no real, deterministic, per-account condition that makes `hardDeleteAccount` throw, because the steps swallow. Making `errors` meaningful requires instrumenting the 12 steps to report/propagate failure — **the refactor this story explicitly scopes OUT** ("any refactor urge → separate story").
  - **Decision needed from operator/architect (the gate the story itself defers to):** either (a) redefine `errors` to count swallowed per-step failures + approve the step-instrumentation as in-scope (or a stacked story), OR (b) descope `errors` for this PR and ship `swept`-only (delivers the headline swept=0 audit — real-inducible + testable), OR (c) accept loop-level `errors` as near-always-0 with a documented caveat. Each is a real trade-off; I will not silently pick one (silently descoping = the [[feedback-fix-pre-existing-and-new-same]]/SHY-0165 descope mistake; silently refactoring = scope-guard violation).
  - Per [[feedback-blocker-switch-not-halt]]: staying Draft, switching to unblocked work, surfacing this for the operator. `swept`-only would be a clean, valuable, real-testable reduced-scope PR the moment (b) is approved.
