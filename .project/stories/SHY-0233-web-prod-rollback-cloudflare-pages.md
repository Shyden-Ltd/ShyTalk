...
**2026-07-24 (retarget → develop + release-flow, operator directive):**
- **Base moved `main` → `develop`.** Diagnosis of the two CI reds on the main-based branch: (a) `integration-tests` (4 fails: deviceBindings read + cohort room-create ×3) = develop's already-merged Firestore-rules fixes (SHY-0150 ban-gate #1578) **not yet released to main** — pre-existing main-lag, NOT a regression. Proof: this branch's entire change surface is 5 CI files, none touching `firestore.rules` or any integration spec; develop's `integration-tests` pass; the spec files are byte-identical main↔develop, only the rules differ (main was **42 commits behind** develop). (b) `mobile-safari` = the known WebKit trace-DOM-snapshot flake ([[reference-webkit-playwright-trace-dom-snapshot-cost]], SHY-0200) — headless-browser, unaffected by a CI change.
- **Merged `origin/develop` into the branch** so it carries current rules (→ `integration-tests` pass) and ships to main via the develop→main **release** rather than direct-to-main. The folded setup-java composite heal is now **inert** — develop already carries the identical `03ad4de0` SHA; the sole merge conflict was comment-only (`# v5` vs develop's `# v5.6.0`), resolved to develop's.
- **Pre-existing develop `lint` failure healed (folded in):** the merge surfaced a red guard — **SHY-0177 #1570** made the `sync-stories` concurrency group GLOBAL (one shared story board) but never allowlisted it in `check-workflow-concurrency-scoping.sh`, so develop's `lint` has failed on `Unscoped concurrency group 'sync-stories'` since #1570 (confirmed: develop CI run 29842430945 `lint`=failure on exactly that). Added the allowlist entry with SHY-0177's rationale — heals develop on merge ([[feedback-fix-pre-existing-and-new-same]], [[feedback-clean-merge-can-still-break-the-tree]]).
- **Post-develop-merge verification (all green):** concurrency-scoping (exit 0 after heal), action-shas, no-paid-runners, `ci-action-pin-consistency` (15) + `rollback-workflow-structure` (16) = **31/31**, story-frontmatter `--scan` (exit 0), actionlint via CI invocation `SHELLCHECK_OPTS='-e SC2086'` (0 findings — a bare local `actionlint` falsely flagged 2 SC2086 that CI intentionally excludes per SHY-0191; [[feedback-verify-the-harness-not-just-the-result]]).
- Reviewed-up-to: tip after the develop merge + `sync-stories` allowlist heal + SHY-INDEX row (self-certified; deltas are merge-conflict resolution + a one-line CI-config allowlist + docs, over the R1-reviewed base).
