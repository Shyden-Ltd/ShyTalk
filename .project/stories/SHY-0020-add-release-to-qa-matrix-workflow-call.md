...
# SHY-0020: release.yml → manual-qa-matrix.yml workflow_call

## User Story

As the ShyTalk operator, I want **release.yml → manual-qa-matrix.yml workflow_call** delivered per the roadmap row(s) for G022 G049, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-G3` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G022 G049. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G022 (TBD on pickup)**

- **Given** the spec for G022's gap as documented in the roadmap row
...
**Scenario: Refined behaviour for G049 (TBD on pickup)**

- **Given** the spec for G049's gap as documented in the roadmap row
- **When** the implementation lands per the Fix column guidance
- **Then** the AC bullets pinned at pickup pass
- **And** the validator + reviewer agents return ZERO findings
...
- Roadmap row(s) for G022 G049 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-G3` (roadmap_ids: G022 G049). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
