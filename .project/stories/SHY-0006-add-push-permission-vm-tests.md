...
**Note**: G006 (the `push_permission.feature` BDD file referenced in roadmap line 31) is a SEPARATE missing G-ID, not bundled here — it'll be handled by SHY-0033's gap-fill PR. This SHY covers the VM + UI + integration tests only.
...
- **The `push_permission.feature` BDD file** — covered by SHY-0033 (G006 gap fill).
...
- [ ] Per-type Done gate satisfied (`bug` → auto-merge + dev smoke).
- [ ] PR merged.
