...
**Not `*.md`-only** (rewrites Android navigation + deletes 979 LOC + adds adapters/tests) → the FULL gauntlet applies. This is a high-blast-radius Android-app refactor — every route is in scope — so the device gauntlet is the primary safety net, not a formality.

**Frameworks exercised (RED→GREEN before the migration code):**
- ✅ **Android instrumented BDD + Manual-QA journey matrix** — the 3 new migration tests + ALL 11 existing journey files + every route in `Screen.kt`, walked on a **real Android device**; the headline check is the previously-impossible **Gacha → AgeVerificationSubmit** path reaching its destination. The FULL journey corpus is the regression net (a nav refactor can break anything).
- ✅ **Kotlin/JVM + Android unit** — `AndroidPlatformAdaptersTest` (every one of the 13+35 hooks) + `SharedNavGraphLaunchTest` (`./gradlew :app:testDevDebugUnitTest`).
- ✅ **iOS shared compile-check** — `./gradlew :shared:compileKotlinIosArm64` (the iOS code path must be ZERO-diff — proven by `git diff main -- shared/src/iosMain` being empty) + the iOS app runs the full regression corpus on the **real iPhone** to confirm no shared-nav regression.
- ✅ **detekt + ktlint** — new adapter + test Kotlin passes static analysis.
- ⬜ **Web E2E / integration / eslint / Express Jest** — N/A (no web/API surface); web journeys still run as the regression net.
- ✅ **SonarCloud** — coverage gate (≥80% on the adapter, per AC) + the cold-start ±5% perf budget.

**LOCAL gauntlet:** the 3 migration tests + 11 journey files + every route green on a **real Android device**; iOS regression corpus green on the **real iPhone**; cold-start within ±5% baseline. Impact-selected (nav journeys) each loop, full corpus at the pre-push gate. Any failure → fix TDD → restart the whole local gauntlet.
**DEV gauntlet:** redeploy the unmerged branch via Deploy-To-Dev `ref`; re-walk every route on real Android (esp. Gacha→AgeVerificationSubmit) + real iPhone regression; web = Chrome only. Restart from LOCAL on failure. **Judgment-merge** only when production-ready with zero doubt — the dev gauntlet's route-walk REPLACES the old "post-merge dev smoke" (verification now happens BEFORE merge).
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): LOCAL gauntlet green (3 migration tests + 11 journeys + every route on real Android, iOS regression on real iPhone, cold-start ±5%) → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green (Gacha→AgeVerificationSubmit confirmed on dev BEFORE merge — this REPLACES the old post-merge smoke) → **judgment-merge** (zero doubt; NO auto-merge).
- [ ] `released_in: vX.Y.Z` set after the release cut.
- [ ] `status: Done` set; `pr:` populated; merge timestamp + dev gauntlet outcome in Notes log.
...
- 2026-06-12 ~23:45 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): high-blast-radius Android nav refactor → full journey corpus on real Android + iOS regression on real iPhone is the safety net; the Gacha→AgeVerificationSubmit verification MOVED from post-merge smoke to the pre-merge DEV gauntlet. DoD auto-merge → judgment-merge. Pickup-fitness: no dupes/stale found.
