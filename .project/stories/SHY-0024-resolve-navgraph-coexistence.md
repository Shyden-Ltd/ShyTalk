...
# SHY-0024: NavGraph.kt vs SharedNavGraph.kt — needs operator decision

## User Story

As the ShyTalk operator, I want **NavGraph.kt vs SharedNavGraph.kt — needs operator decision** delivered per the roadmap row(s) for G028, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-I1` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G028. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G028 (TBD on pickup)**

- **Given** the spec for G028's gap as documented in the roadmap row
...
- Roadmap row(s) for G028 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- [ ] Per-type Done gate satisfied (`refactor`)
- [ ] PR merged via auto-merge
- [ ] `status: Done` set; `pr:` populated; merge timestamp in Notes log

## Notes (running log)

- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-I1` (roadmap_ids: G028). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
