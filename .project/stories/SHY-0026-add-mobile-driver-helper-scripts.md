...
# SHY-0026: mobile-android flags check + iOS WDA build script

## User Story

As the ShyTalk operator, I want **mobile-android flags check + iOS WDA build script** delivered per the roadmap row(s) for G043 G044, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-I4` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G043 G044. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G043 (TBD on pickup)**

- **Given** the spec for G043's gap as documented in the roadmap row
...
**Scenario: Refined behaviour for G044 (TBD on pickup)**

- **Given** the spec for G044's gap as documented in the roadmap row
- **When** the implementation lands per the Fix column guidance
- **Then** the AC bullets pinned at pickup pass
- **And** the validator + reviewer agents return ZERO findings
...
- Roadmap row(s) for G043 G044 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- [ ] Per-type Done gate satisfied (`infra`)
- [ ] PR merged via auto-merge
- [ ] `status: Done` set; `pr:` populated; merge timestamp in Notes log

## Notes (running log)

- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-I4` (roadmap_ids: G043 G044). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
