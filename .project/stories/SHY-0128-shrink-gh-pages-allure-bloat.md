...
### Observability
- [ ] The cap step logs the gh-pages commit count on EVERY report run (re-bloat in commit terms is visible in any job log) and logs `capped: N commits -> 1` when it fires.
- [ ] Tip-read failures surface as `::error::` annotations; race-skips log the moved tip pair.
...
- **Mutant:** reverted one `keep_files` flip verbatim → exactly the 2 expected tests went RED; restored.
