...
- 2026-07-17 ~04:1x WIB — Post-push correction: the "Enter key triggers user search" `toPass` fix still flaked ONCE on chromium in the pre-push run (1 flaky / 1378 passed). Root cause: the intermediate `toHaveText('Search')` matched the pre-search IDLE button state, letting the populate check race an in-flight search. Fixed in `b3b7fbf41cd` by asserting ONLY the end-state (displayName populated), re-pressing Enter until it settles. Verified 20/20 across chromium+webkit (repeat-each=10). Note: R1 had assessed this test "sound" — it missed the pre-idle race; self-verified this small correction rather than re-spawning a round.
- Reviewed-up-to: b3b7fbf41cdac32c14b11408eb1d84a0edb2ece6
