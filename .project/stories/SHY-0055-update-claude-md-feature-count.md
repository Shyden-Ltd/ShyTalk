...
- 2026-07-09 — **Minor AC-exception noted (code-reviewer):** the primary deliverable (accurate count) is met and released — count re-verified live at 48 `.feature` files / 239 `Scenario`(+`Outline`) lines (the "~235" prose is a fair approximation). The AC's optional `<!-- last verified: … -->` adjacent comment convention was NOT added by the original work; it is trivial doc-hygiene, not worth reopening a released doc-count fix — recorded as a micro-follow-up in the audit report rather than blocking Done.
