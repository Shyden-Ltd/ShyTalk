...
type: bug
roadmap_ids: []
pr: https://github.com/Shyden-Ltd/ShyTalk/pull/1431
...
- 2026-06-15 — **Coordinated DEV gauntlet GREEN + squash-merged to main; status → In Review.** The 0096+0097 coordinated dev pass on the real OnePlus CPH2653 against the real dev backend proved the warning-acknowledge flow end-to-end on dev: the on-device acknowledge tap clears `hasActiveWarning` via the real Express endpoint (coupled with SHY-0096's sign-out-through-warning S3 path). CI green (Detect Changes / Analyze JavaScript / PR Gate); `code-reviewer` (1 Critical + 4 Important, all fixed) + `security-reviewer` clean. **Squash-merged as PR #1431 → main `6efb8147d`** (coordinated pair with SHY-0096 #1418 → `16ae23de1`). Per the merged-not-released convention, status moves **In Progress → In Review**; it flips to Done at the release cut (`released_in:`). The `@known-failure-SHY-0097` j11 acknowledge-scenario tag removal remains with SHY-0101 (acknowledge-scenario robustness), per the notes above.
- 2026-06-15 — **Released in v0.97.15 → Done.** The v0.97.15 release tag ships SHY-0097 (server-authorized warning-acknowledge). Status In Review → Done; `released_in: v0.97.15`. The `@known-failure-SHY-0097` j11 tag removal remains tracked under **SHY-0101** (acknowledge-scenario robustness) — the server fix is shipped; the on-device journey-scenario green-up is the apparatus follow-on.
