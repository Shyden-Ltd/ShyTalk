...
**Not `*.md`-only** (refactors Pages-deploying workflows + adds a lint test) → the FULL gauntlet applies. This is pure CI-infra (no app/web behaviour change), so the impact frameworks are the workflow-lint + actionlint + the live two-deploy serialisation verification; the apps/web run as the regression net.

**Frameworks exercised (RED→GREEN):**
- ✅ **Express/Node Jest (workflow lint)** — `gh-pages-concurrency.test.js`: every Pages-deploying workflow's deploy job declares `group: gh-pages-deploy` + `cancel-in-progress: false`; typo/missing-group variants fail. The story's primary RED→GREEN.
- ✅ **actionlint + eslint** — the refactored workflow YAML + the lint test JS.
- ✅ **[[feedback-workflow-verify-by-running]]** — dispatch TWO Pages deploys in the same window on the unmerged branch; OBSERVE serialisation (one runs, the other "Queued") in the Actions UI. A concurrency fix can ONLY be proven by running it — non-negotiable.
- ⬜ **Kotlin / detekt / ktlint / iOS compile / Playwright web** — N/A (no app/web/Kotlin change); apps + web run the regression corpus as the net.
- ✅ **SonarCloud** — quality gate.

**LOCAL gauntlet:** lint test + actionlint green; the full app/web regression corpus stays green (no behaviour change expected). Any failure → fix TDD → restart.
**DEV gauntlet:** the two-simultaneous-dispatch serialisation observation on the unmerged branch IS the DEV gate (a Pages-race fix cannot be proven any other way); apps regression on real devices, web = Chrome. **Judgment-merge** only when production-ready with zero doubt — branch-protection required-check names must be preserved through the split-job refactor (verify BY NAME).
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): lint test + actionlint green + the two-simultaneous-dispatch serialisation OBSERVED on the unmerged branch ([[feedback-workflow-verify-by-running]]) + branch-protection required-check names preserved BY NAME → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green → **judgment-merge** (zero doubt; NO auto-merge).
...
- 2026-06-12 ~23:59 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): CI-infra Pages-race fix → impact frameworks are the workflow lint + actionlint + the live two-dispatch serialisation observation ([[feedback-workflow-verify-by-running]]); a concurrency fix is only provable by running it. Required-check names preserved BY NAME through the split-job refactor. DoD auto-merge → judgment-merge. Pickup-fitness: no dupes/stale found.
