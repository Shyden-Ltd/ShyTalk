...
Reviewed-up-to: 34314cd4cf3

- 2026-07-16 ~10:5x WIB — **Combined final review (with SHY-0192) CLEAN — this branch's marker bumped to its tip `34314cd4cf3`.** The code-reviewer round ran on the full STACK diff vs develop (story/SHY-0192 tip, which contains every commit of this branch incl. the R3 fixes `aed3d8f1289` + the develop sync `34314cd4cf3`): zero functional/security findings in the SHY-0187 surface; the 4 findings landed in SHY-0192's files and are fixed there (`c341f83376d` — full record in SHY-0192 Notes). Merging under the devices-away stacking protocol; device LOCK-proof rides the develop-wide gauntlet on device return. Post-merge redesign heads-up: SHY-0196 (device-OS-credential App-Lock) replaces the PIN surfaces; this story's nav-gating skeleton survives it.
