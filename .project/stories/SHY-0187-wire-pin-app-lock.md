...
- 2026-07-16 ~10:0x WIB — **Combined final review (with SHY-0192) CLEAN after fixes.** code-reviewer on the full stack diff vs develop: zero functional/security findings in the SHY-0187 surface ("the App-Lock wiring, deep-link fail-closed gating ... correctly implemented and thoroughly tested"); the 4 findings (1 administrative, 2 test-coverage, 1 minor guard) all landed in SHY-0192's scope and are fixed in `c341f83376d`. `Reviewed-up-to` bumped accordingly. NOTE (operator direction, same morning): App-Lock will move to the DEVICE's own credential (OS prompt; serverless) in a follow-up story — this story's nav-gating skeleton (launch resolver, resume gate, deep-link gates) survives that redesign; the PIN-specific surfaces it wired will be replaced there.
