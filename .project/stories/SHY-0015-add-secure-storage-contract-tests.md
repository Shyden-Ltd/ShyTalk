...
As the ShyTalk operator, I want **SecureStorage + CryptoKeyPair contract tests** delivered per the roadmap row(s) for G019, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-E3` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G019. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G019 (TBD on pickup)**

- **Given** the spec for G019's gap as documented in the roadmap row
...
- Roadmap row(s) for G019 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-E3` (roadmap_ids: G019). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
