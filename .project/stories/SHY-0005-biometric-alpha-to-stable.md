...
# SHY-0005: Biometric alpha → stable or rationale comment

## User Story

As the ShyTalk operator, I want **Biometric alpha → stable or rationale comment** delivered per the roadmap row(s) for G002, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-A4` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G002. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G002 (TBD on pickup)**

- **Given** the spec for G002's gap as documented in the roadmap row
...
- Roadmap row(s) for G002 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-A4` (roadmap_ids: G002). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
