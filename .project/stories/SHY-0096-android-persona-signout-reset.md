...
roadmap_ids: []
epic: EPIC-0003
pr: https://github.com/Shyden-Ltd/ShyTalk/pull/1418
...
- 2026-06-15 — **Coordinated DEV gauntlet GREEN + squash-merged to main; status → In Review.** The 0096+0097 coordinated dev pass ran on the real OnePlus CPH2653 against the real dev backend (`installDevDebug` with `DEV_QA_PERSONAS_PASSWORD` baked; firebase-admin-dev / projectId `shytalk-dev`): the real in-app sign-out is **device-proven on dev** — S1 back-to-back two-persona switch + S2/S4 — alongside SHY-0097's warning-acknowledge flag-flip (the coupled S3 sign-out-through-warning path). CI green (Detect Changes / Analyze JavaScript / PR Gate); `code-reviewer` + `security-reviewer` clean. **Squash-merged as PR #1418 → main `16ae23de1`** (coordinated pair with SHY-0097 #1431 → `6efb8147d`). Per the merged-not-released convention, status moves **In Progress → In Review**; it flips to Done when the release tag is cut (`released_in:`). [[feedback-tdd-journey-first]]
- 2026-06-15 — **Released in v0.97.15 → Done.** The v0.97.15 release (`release.yml` patch bump from v0.97.14 → signed `chore: release v0.97.15` on main `e08c8d9d7fd` → `release-tag.yml` tag + GitHub Release) ships SHY-0096. Status In Review → Done; `released_in: v0.97.15`. No code changed after the pre-merge gauntlet, so no re-gauntlet at the release cut (the gauntlet was the gate).
