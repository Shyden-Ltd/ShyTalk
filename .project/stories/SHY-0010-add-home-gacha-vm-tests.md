...
**Not `*.md`-only** (adds Kotlin commonTest + may fix `shared/commonMain` production code) → the FULL gauntlet applies. `HomeViewModel` + `GachaViewModel` live in `commonMain`, so a surfaced fix ships to **both** the Android and iOS apps.

**Frameworks exercised (RED→GREEN before any production fix):**
- ✅ **Kotlin/JVM unit** — `HomeViewModelTest` + `GachaViewModelTest` (`./gradlew :shared:jvmTest`); the story's primary RED→GREEN.
- ✅ **detekt + ktlint** — new test + any production-fix Kotlin passes static analysis (warnings = failures).
- ✅ **iOS shared compile-check** — `./gradlew :shared:compileKotlinIosArm64` (commonMain VM changes must not break the iOS build).
- ✅ **Android instrumented BDD + Manual-QA journey matrix** — the **Home** screen (room list, daily-reward, push banner, gacha entry-card, event banner) and the **Gacha** lucky-spin (incl. the under-18 age-gate redirect + spin animation) walked on a **real Android device AND a real iOS device**; any production fix surfaced by the unit tests is proven not to regress the live screen.
- ⬜ **Web E2E / integration / eslint / Express Jest** — N/A (no web/API surface); the full journey corpus still runs as the regression net.
- ✅ **SonarCloud** — coverage gate (≥90% on both VMs, per AC).

**LOCAL gauntlet:** unit suite green → Home + Gacha journeys on real Android + real iOS (both <18 and >18 personas for the age-gate path) → impact-selected journeys each loop, full corpus at the pre-push gate. Any failure → fix TDD → restart the whole local gauntlet.
**DEV gauntlet:** redeploy the unmerged branch via Deploy-To-Dev `ref`; real Android + real iOS; web = Chrome only. Restart from LOCAL on failure. **Judgment-merge** only when production-ready with zero doubt.
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): unit suite green + Home & Gacha journeys green on **real Android + real iOS** (LOCAL) → `code-reviewer` 100% clean → push → CI green by name (Detect Changes, Analyze JavaScript, PR Gate) → DEV gauntlet green → **judgment-merge** (zero doubt, production-ready; NO auto-merge).
...
- 2026-06-12 ~23:35 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): named the frameworks this story exercises (Kotlin JVM unit primary + detekt/ktlint + iOS compile + Home/Gacha journeys on real Android + real iOS) + the device gauntlet in the Test Plan; DoD updated auto-merge → **judgment-merge**. Pickup-fitness: no dupes/stale found.
- 2026-06-13 ~01:47 BST — **No-Stubs foundational-harness flag** ([[feedback-no-stubs-mocks-fakes-real-only]]): these ViewModel tests are built on the `FakeRepository` pattern (`commonTest/.../fake/`: FakeRoom/DailyReward/PushPermission/Gacha/User) — in-process test doubles the No-Stubs rule targets. This is a FOUNDATIONAL harness (same class as the Android BDD fakes + [[SHY-0071]]'s mock-gh): converting VM *unit* tests to real-emulator-backed data effectively turns them into integration tests, an architectural shift. Per the rule's "opportunistic, no big-bang" scope it is NOT re-architected here. **🔴 Operator decision needed** (on the SHY-0091 handoff): do the KMP VM-test repositories migrate toward real backends, and at what layer (keep VM-unit isolation + add a real-backed integration layer, or migrate wholesale)? No Test-Plan rewrite pending that direction.
- 2026-07-09 — **Reconciliation (SHY-0167) — KEPT DRAFT; AC-deviation gap found in code-reviewer verification.** `HomeViewModelTest.kt` + `GachaViewModelTest.kt` DO exist, but at `app/src/test/java/...` using JUnit4 + MockK (JVM/Android-only, no Kotlin/Native target) — NOT at `shared/src/commonTest` (kotlin.test) as this story's AC + Notes require. The ViewModels are in `shared/commonMain` (cross-platform), so as shipped they have Android-JVM coverage but **ZERO iOS test-execution proof** (Tri-Platform gap; `:shared:iosSimulatorArm64Test` runs none of them). Not cleanly delivered per AC → stays Draft. **Operator decision needed:** accept `app/src/test`+MockK as the standard (rewrite the ACs) OR relocate/reimplement in `shared/commonTest` for iOS coverage. See `.project/audit/mvp-backlog-reconciliation-2026-07-08.md`.
