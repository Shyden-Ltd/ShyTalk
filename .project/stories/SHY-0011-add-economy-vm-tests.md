...
**Not `*.md`-only** (adds Kotlin commonTest + may fix `shared/commonMain` economy code) → the FULL gauntlet applies. `WalletViewModel` / `GiftingViewModel` / `TransactionHistoryViewModel` live in `commonMain`, so any money-math fix ships to **both** apps — the highest-stakes place to be wrong.

**Frameworks exercised (RED→GREEN before any production fix):**
- ✅ **Kotlin/JVM unit** — `WalletViewModelTest` + `GiftingViewModelTest` + `TransactionHistoryViewModelTest` (`./gradlew :shared:jvmTest`); the story's primary RED→GREEN, incl. idempotency / request-ID / integer-overflow assertions.
...
- ✅ **Android instrumented BDD + Manual-QA journey matrix** — the **Wallet** screen, **gift-send** in a room/chat (incl. the gift animation + double-tap dedupe), and **transaction history** pagination walked on a **real Android device AND a real iOS device**; the balance is re-checked after each gift to prove no double-charge on the live surface.
- ⬜ **Web E2E / integration / eslint / Express Jest** — N/A (no web/API surface); the full journey corpus still runs as the regression net. (Express-side gift/wallet dedupe is explicitly Out of Scope here.)
- ✅ **SonarCloud** — coverage gate (≥90% per VM, per AC).

**LOCAL gauntlet:** unit suite green → wallet/gift/history journeys on real Android + real iOS (send a gift, double-tap it, confirm exactly one charge against the live balance) → impact-selected each loop, full corpus at the pre-push gate. Any failure → fix TDD → restart the whole local gauntlet.
...
- [ ] **Pre-Merge Testing Protocol satisfied** (`CLAUDE.md` § Pre-Merge Testing Protocol): unit suite green + wallet/gift/history journeys green on **real Android + real iOS** (exactly-one-charge proven against the live balance) → `code-reviewer` 100% clean → push → CI green by name → DEV gauntlet green → **judgment-merge** (zero doubt; NO auto-merge).
...
- 2026-06-12 ~23:35 BST — **Embedded the Pre-Merge Testing Protocol** ([[SHY-0091]] pass): named the frameworks (Kotlin JVM unit primary + detekt/ktlint + iOS compile + wallet/gift/history journeys on real Android + real iOS, balance re-checked for double-charge) + the device gauntlet; DoD auto-merge → **judgment-merge**. Pickup-fitness: economy VMs correctly lack `mvp:true` (Revenue excluded from MVP); no dupes/stale found.
- 2026-06-13 ~01:47 BST — **No-Stubs foundational-harness flag** ([[feedback-no-stubs-mocks-fakes-real-only]]): inherits the `FakeRepository` pattern from [[SHY-0010]] (FakeWallet/Gift/Transaction) — same foundational-fake collision (see [[SHY-0010]]'s flag + the SHY-0091 handoff's 🔴 operator-decision item). NOT re-architected here per "opportunistic, no big-bang"; no Test-Plan rewrite pending operator direction on the VM-test backend strategy.
