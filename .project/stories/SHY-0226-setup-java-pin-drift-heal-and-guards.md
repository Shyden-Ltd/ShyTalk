...
- [ ] Reverting to an unpinned `firebase-tools` install — or bumping the emulator pin without moving the cache key in lockstep — fails the new pin test; the repo-wide invariant covers EVERY install site under `.github/**` (deploy-dev, deploy-prod, the emulator composite), naming any unpinned line.
...
- **RED first** (all four proven failing at the `origin/main` cut before any fix, via the canonical runner):
...
- **GREEN**: those four after the fixes; the existing `pr-checks-*.test.js` pin family green (regression net over the case-block edit).
...
- 2026-07-20 — **code-review R2 on the firebase-tools leg** (commit `5d7e7743528`): 1 finding at 82, verified before applying — `deploy-dev.yml:105` + `deploy-prod.yml:258` ran the SAME unpinned `npm install -g firebase-tools` (grep-confirmed the only remaining sites). Applied: both pinned at **15.24.0** (the version the green 07-19 dev deploy actually ran — deploys were never proven on 15.15.0-era, so each site pins its own last-proven version; convergence rides the deliberate-upgrade follow-up) + the pin test extended with a repo-wide walker asserting NO unpinned install anywhere under `.github/**` (RED naming both deploy sites → GREEN). Reviewer's regex false-pass analysis + 15.15.0-vs-15.22.4 reasoning both accepted (no change needed); "three→four" Test Plan drift fixed.
- Reviewed-up-to: `f3a4e7b7d9f` — **code-review R2 verdict: zero findings ≥80, clear to push** (reviewer independently confirmed the split-pin rationale: `firebase deploy --only firestore:rules` uploads rules to Google's backend and never runs firebase-tools' bundled emulator rules-engine, so pinning deploys at 15.24.0 cannot reintroduce the emulator-side #1650 denial). Full `tests/scripts` sweep green (exit 0). Cosmetic RED-count drifts (three→four) swept across Test Plan + DoD. CI-config-only class → main.

- **2026-08-17 — develop merge (125 commits behind) + obsolescence review. The story's headline fix is GONE; its other half is still needed.**
  - **DROPPED — the setup-java "heal".** develop has since converged BOTH call sites on `b6effb05e454` (v5.7.0): `setup-jdk-gradle/action.yml` and `test-backend.yml`. There is no drift left, and this branch's pin (`03ad4de`, v5.6.0) would have **regressed** develop. Took develop's side. Nearly missed because `git diff origin/develop...HEAD` shows the **merge base**, not develop's tip — from 125 commits back the old pin still appeared to be there. Enumerating the pins on `origin/develop` directly is what settled it.
  - **KEPT — firebase-tools pinning.** All three sites on develop are still bare `npm install -g firebase-tools` (`start-firebase-emulators/action.yml`, `deploy-dev.yml`, `deploy-prod.yml`), so that drift class is entirely unaddressed. Plus the dependabot composite-actions `directories:` list + same-repo groups, and all three guard suites.
  - **MECHANISM CHANGED — adopted SHY-0284's flag instead of this story's.** SHY-0284 has since added a dedicated `SCRIPTS` flag and argues explicitly against routing CI plumbing through `BACKEND`: the SHY-0127 block reads `BACKEND` as "the shared core changed, retest every client", which would put a workflow-only edit through a Gradle build and device E2E — outside CLAUDE.md's CI-config-only boundary. Sound and newer, so `.github/*` now sets `SCRIPTS=true`. SHY-0226's actual goal is still met: `test-backend` runs on either flag, and `.github/*` previously set **nothing at all**.
  - **Guard found a real gap, as designed:** the composite-actions exhaustiveness test failed because develop added `publish-gh-pages` after this branch wrote its list. Declared.
  - **Pre-existing hole closed:** SHY-0284's `SCRIPTS` flag had **no behavioural coverage** — neither `pr-checks-github-dir-backend-gate.test.js` nor the canonical `pr-checks-app-changed-split.test.js` harness initialised or echoed it, so any case-arm routing to `SCRIPTS` read as `undefined` and passed. Both harnesses now observe it.
  - Verified: 4 guard suites, 55/55; actionlint clean on the touched files; dependabot.yml/pr-checks.yml/action.yml all parse as YAML.

- **2026-08-17 — SCOPE CUT. The pr-checks routing is REVERTED and deferred; this story now lands only its uncontested half.**
  - Routing `.github/*` to any flag is **mutually exclusive with `workflow_only` by construction**: `WORKFLOW_ONLY=true` is *defined* as "APP, BACKEND, WEB, INTEGRATION, SCRIPTS and OTHER are all false" (`pr-checks.yml:160`). Setting a flag necessarily kills it, and `workflow_only` gates `sonarcloud` among others (`:404`). SHY-0284's `pr-checks-root-scripts-run-their-tests.test.js` deliberately pins `workflowOnlyFor(['.github/workflows/lint.yml']) === true` with the comment "the flag must keep working for what it was built for".
  - So SHY-0226's goal and the `workflow_only` short-circuit cannot both hold. Choosing between them is a **CI-minutes trade-off on a $0-budget project** — every workflow tweak would start paying for sonarcloud and test-backend — which is an operator decision, not a merge-time judgement call. `pr-checks.yml` reverted to develop's version and this branch's `pr-checks-github-dir-backend-gate.test.js` removed with it, since it only tested the reverted routing. **Filed as SHY-0325.**
  - **KEPT, all uncontested and all green (80/80 across 5 suites):** firebase-tools pinning (three still-bare sites on develop); the dependabot composite-actions `directories:` list + groups + `publish-gh-pages`; `start-firebase-emulators-pin.test.js`; the `develop-ci-gate.test.js` parser fix; and the `SCRIPTS` observability added to the canonical `pr-checks-app-changed-split.test.js` harness.
  - **Two findings that are NOT this branch's and are filed separately:**
