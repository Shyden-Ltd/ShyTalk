...
# SHY-0030: ios_parity_navigation.feature freshness

## User Story

As the ShyTalk operator, I want **ios_parity_navigation.feature freshness** delivered per the roadmap row(s) for G039, so that the corresponding gap in the zero-gap remediation roadmap closes.

## Why

This SHY mirrors PR-bundle `PR-I8` from the architect's recommended PR sequencing (lines 122–173 of `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md`). The deeper rationale — including the Gap / Fix / Scope columns for each G-ID — lives in the roadmap row(s) for G039. Refinement on pickup will copy the relevant content into this section.
...
**Scenario: Refined behaviour for G039 (TBD on pickup)**

- **Given** the spec for G039's gap as documented in the roadmap row
...
- Roadmap row(s) for G039 in `.project/test-plans/exhaustive/2026-06-05-zero-gap-roadmap.md` (gitignored — local only).
...
- 2026-06-07 — Skeleton generated by `scripts/convert-roadmap-to-stories.sh` from PR-bundle `PR-I8` (roadmap_ids: G039). Status: Draft; AC dimensions are `N/A — TBD refinement on pickup` per SHY-0003 spec. Pickup must refine before TDD.
