...
- 2026-07-09 — **Reconciliation (SHY-0167) — KEPT DRAFT; AC-deviation gap found in code-reviewer verification.** The tracked ViewModel tests (SHY-0010/0011/0012) DO exist, but at `app/src/test/java/...` (JUnit4 + MockK, Android-only) not `shared/src/commonTest` — so the tracked cross-platform ViewModels have **ZERO iOS test-execution proof** (Tri-Platform gap). The tracker's coverage goal is not met on iOS → stays Draft, gated on the same operator decision as SHY-0010/0012. See `.project/audit/mvp-backlog-reconciliation-2026-07-08.md`.
