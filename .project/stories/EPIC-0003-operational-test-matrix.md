...
**Unit↔integration boundary convention (finalised by SHY-0112, enforced by `scripts/check-no-new-stubs.js`):** a UNIT-test location — where doubles are permitted and the ratchet does not scan — is `**/tests/unit/**` or a `*.unit.test.{js,ts}` file (JS/TS), or a **non-instrumented (host) Kotlin test source set** (`commonTest` / `jvmTest` / `androidHostTest` / `src/test`, all run on the host JVM). Everything else is integration/real-only: **instrumented** Kotlin (`androidTest` / `androidInstrumentedTest` / `androidUiTest` — need a real device) and all iOS `*Tests` (no iOS unit-location convention; the 3 Swift doubles mock real collaborators and are counted for migration). Location is a mechanical proxy for "what it exercises"; `code-reviewer` + the per-SHY Test Plan catch a mislabelled integration test. **Widened, policy-aware baseline floor (SHY-0112 regen):** jest.mock 196 · Fake*Repository 25 (9 host-set fakes reclassified as permitted) · page.route 8 · jest.fn 228 · make*Fake* 5 · mockResolved* 204 · Kotlin mockk 0 (all 49 are in host/unit source sets — permitted; the category is forward-looking, blocking a new mockk in `androidTest`) · iOS doubles 3. "Baseline empty in every category" = no doubles remain **outside** unit tests = drain complete.
