...
- **Open question to settle in the first hour, before any code:** whether the Firebase Admin SDK can address a Firebase Installation ID directly, and what replaces `sendEachForMulticast({ tokens })` in `express-api/src/utils/fcm.js`. The SDK source documents the FID as usable "to target this app instance for direct-send messaging", but the Admin-side API for it has **not** yet been verified. If direct-send by FID turns out to be unavailable or materially weaker than multicast, STOP and escalate — that finding changes the shape of this story and is an operator decision, not an implementation detail.
- iOS parity: the Apple SDK's own deprecation timeline for `MessagingDelegate.messaging(_:didReceiveRegistrationToken:)` must be confirmed, since Android and iOS have to land together under the tri-platform policy.
...
- **Risk: Admin SDK cannot target FIDs the way multicast targets tokens.** **Mitigation:** settle it first, before writing code; escalate rather than improvise a fan-out.
