...
// androidShowsMessageInConversationThread: the describe block that lived here asserted "the tag is present
...
// androidShowsInThread: the describe block that lived here asserted "the tag is present
// → true", which is the defect itself rather than the behaviour. Replaced by
// the block of the same name in android-auth-state.test.js, which asserts the
// subject and pins the negative cases the presence check could not express.
...
// androidShowsPmThreadDirection: the describe block that lived here asserted "the tag is present
// → true", which is the defect itself rather than the behaviour. Replaced by
// the block of the same name in android-auth-state.test.js, which asserts the
// subject and pins the negative cases the presence check could not express.
...
// androidShowsCountBadge: the describe block that lived here asserted "the tag is present
// → true", which is the defect itself rather than the behaviour. Replaced by
// the block of the same name in android-auth-state.test.js, which asserts the
// subject and pins the negative cases the presence check could not express.

// androidShowsEditedBodyWithTag: the describe block that lived here asserted "the tag is present
// → true", which is the defect itself rather than the behaviour. Replaced by
// the block of the same name in android-auth-state.test.js, which asserts the
// subject and pins the negative cases the presence check could not express.

// androidShowsInAppGiftNotification: the describe block that lived here asserted "the tag is present
// → true", which is the defect itself rather than the behaviour. Replaced by
// the block of the same name in android-auth-state.test.js, which asserts the
// subject and pins the negative cases the presence check could not express.

// androidShowsInResults: the describe block that lived here asserted "the tag is present
// → true", which is the defect itself rather than the behaviour. Replaced by
// the block of the same name in android-auth-state.test.js, which asserts the
// subject and pins the negative cases the presence check could not express.
...
describe('android-adb-driver — _advancePastLaunchGates / _tapByVisibleText / _dismissDailyRewardIfPresent', () => {
  ...
  test('_advancePastLaunchGates: a FIFTH checkbox would also be ticked', async () => {
    ...
    const driver = await createAndroidDriver();
    // A plain recording function, not jest.fn: this needs to remember which
    // tags were tapped, which is a list — reaching for a mocking library to
    // hold a list is what the no-new-stubs ratchet exists to stop.
