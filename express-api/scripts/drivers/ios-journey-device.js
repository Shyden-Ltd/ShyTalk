  ...
  async dumpXml() {
    ...
    for (let i = 0; i < 4; i++) {
      try {
        ...
        if (xml && xml.includes('XCUIElementType')) return xml;
        if (xml) last = String(xml).slice(0, 200);
      } catch (e) {
        last = (e.message || '').slice(0, 200);
        // A dropped session is recoverable; a dropped phone is not. Clearing the
        // id makes the next attempt reconnect rather than replay a dead handle.
        this._sessionId = null;
      }
      if (i < 3) await sleep(600);
    }
    throw new Error(`XCUITest source failed after 4 attempts; last response: ${last || '(empty)'}`);
  }
...
module.exports = { IosDevice, selectCoreDeviceUuid };
