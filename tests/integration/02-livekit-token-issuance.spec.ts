...
function decodeJwtPayload(jwt: string): Record<string, unknown> {
  ...
  if (parts.length !== 3) {
    throw new Error(`Expected 3-part JWT, got ${parts.length} parts: ${jwt.slice(0, 80)}…`);
  }
...
test.describe('Integration — LiveKit token issuance', () => {
  test('POST /api/livekit/token returns a signed JWT for an authenticated user', async ({
    ...
    expect(payload.sub, "JWT.sub must be the user's uniqueId").toBe(String(sender.uniqueId));
    expect((payload as { video: { room: string } }).video.room).toBe(roomName);
    expect((payload as { video: { roomJoin: boolean } }).video.roomJoin).toBe(true);
    expect((payload as { video: { canPublish: boolean } }).video.canPublish).toBe(true);
    expect((payload as { video: { canSubscribe: boolean } }).video.canSubscribe).toBe(true);
  ...
  test('POST /api/livekit/token returns 401 when Authorization header missing', async ({ api }) => {
  ...
  test('POST /api/livekit/token returns 400 when roomName missing', async ({ api, sender }) => {
    const res = await api.post(`${API_BASE}/api/livekit/token`, {
      headers: {
        Authorization: `Bearer ${sender.idToken}`,
        'Content-Type': 'application/json',
