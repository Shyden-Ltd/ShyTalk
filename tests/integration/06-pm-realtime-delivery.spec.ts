...
test.describe('Integration — PM real-time delivery', () => {
  test('sender→recipient PM writes Firestore message + RTDB event', async ({ api, pair }) => {
    ...
    expect(seedRes.ok(), `seed conversation: ${await seedRes.text()}`).toBe(true);
    ...
    const postRes = await api.post(`${API_BASE}/api/conversations/${convId}/messages`, {
      headers: {
    ...
    const listRes = await api.get(`${API_BASE}/api/conversations/${convId}/messages`, {
    ...
    expect(ours, `our message must be in the list: ${JSON.stringify(messages)}`).toBeDefined();
    expect(ours!.senderId).toBe(pair.sender.uniqueId);
  });

  test('non-participant cannot post to a conversation (403)', async ({ api, pair }) => {
    ...
    const res = await api.post(`${API_BASE}/api/conversations/${convId}/messages`, {
      headers: {
        Authorization: `Bearer ${pair.sender.idToken}`,
        'Content-Type': 'application/json',
      },
