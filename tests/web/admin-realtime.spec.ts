...
async function filterPendingReports(page: Page): Promise<void> {
  const btn = page.locator(
    '#report-filter-bar button[data-report-filter="pending"]',
  );
  await btn.click();
...
test.describe("Admin Realtime Features", () => {
  ...
  test("spin monitor coins display updates after API balance change", async ({
    ...
    await testData.api.post(
      `/api/users/${testData.user.uniqueId}/adjust-balance`,
      {
        currency: "COINS",
        amount: 100,
      },
    ...
    await expect(async () => {
      const updatedCoinsText = await page
        .locator("#monitor-coins")
        .textContent();
  ...
  test("logs live mode indicator activates and deactivates", async ({
    page,
  }) => {
  ...
  test("alert bell badge count matches API alert count on load", async ({
    page,
    testData,
  }) => {
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          el && (el.style.display === "none" || Number(el.textContent) > 0)
        );
      },
      { timeout: 15_000 },
    );

    // Both values in ONE evaluation, because the assertion below COUPLES them.
    //
    // They used to be read in two separate round trips, and `startGlobalRefresh()`
    // keeps polling the alert count in between — so the count could come from one
    // moment and the visibility from another, and the invariant was compared
    // across a state change that never actually violated it. That is what failed
    // a push on 2026-08-24 and passed on retry.
    //
    // A snapshot cannot tear: whatever the badge was at that instant, count and
    // visibility describe the SAME instant.
    const { count, hidden } = await badge.evaluate((el) => ({
      count: Number(el.textContent),
      hidden: (el as HTMLElement).style.display === "none",
    }));

    // The invariant, stated once rather than as two branches: the badge is
    // visible exactly when it has something to show.
    expect({ hasAlerts: count > 0, visible: !hidden }).toEqual({
      hasAlerts: count > 0,
      visible: count > 0,
    });
  });
  ...
  test("navigating away from Reports stops onSnapshot listeners", async ({
    page,
  }) => {
  ...
  test("stopping spin monitor cleans up listeners", async ({
    page,
    testData,
  }) => {
    const consoleErrors: string[] = [];
  ...
  test("sign out clears all listeners and intervals without errors", async ({
    page,
  }) => {
    const consoleErrors: string[] = [];
    page.on("console", (msg) => {
