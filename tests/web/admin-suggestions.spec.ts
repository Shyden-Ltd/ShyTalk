...
function resetSeededSuggestions(): void {
  DYNAMIC_SEEDED.length = 0;
}
...
const MOCK_IDENTITY_GRAPH = {
  nodes: [
    ...
    },
    {
      ...
      suspended: false,
      linkedAccounts: ['u1'],
      metadata: { deviceId: 'dev-1', manufacturer: 'Google', model: 'Pixel 6' },
    },
    {
      id: 'node-3',
      type: 'network',
      ...
      suspended: false,
      linkedAccounts: ['u1'],
      metadata: { ip: '203.0.113.1', isp: 'Test ISP' },
    },
  ],
  edges: [
...
async function setupApiMocks(page: Page): Promise<void> {
  ...
  await page.route('**/api/admin/suggestions/*', async (route) => {
    ...
    if (url.includes('/disputes') || url.includes('/history') || url.includes('/link')) {
      await route.fallback();
      return;
    }
    ...
    try {
      ...
      if (real.ok()) {
        ...
        if (
          ...
          (body.identifiers && body.identifiers.length > 0)
        ) {
          await route.fulfill({
            status: 200,
            contentType: 'application/json',
...
async function waitForPendingQueueLoaded(page: Page): Promise<void> {
  await page.waitForFunction(
    () => {
      ...
      return (
        queue.querySelector('.suggestion-card') !== null ||
        queue.textContent!.includes('No pending') ||
        queue.textContent!.includes('empty')
      );
    },
    { timeout: 15_000 },
  );
...
async function waitForDisputeQueueLoaded(page: Page): Promise<void> {
  await page.waitForFunction(
    () => {
      ...
      if (!queue) return false;
      return (
        queue.querySelector('.dispute-card') !== null ||
        queue.textContent!.includes('No disputes') ||
        queue.textContent!.includes('empty')
      );
    },
    { timeout: 15_000 },
...
async function waitForIdentityGraphLoaded(page: Page): Promise<void> {
  await page.waitForFunction(
    () => {
      ...
      return (
        graph.querySelector('.graph-node') !== null ||
        graph.querySelector('canvas') !== null ||
...
async function seedMultipleSuggestions(
  testData: TestData,
  count: number,
  overrides: Record<string, any> = {},
): Promise<string[]> {
  ...
  for (let i = 0; i < count; i++) {
    const result = await seedSuggestion(testData, {
      title: `Bulk E2E Suggestion ${i + 1} — ${Date.now()}`,
      ...overrides,
    });
    ids.push(result.id);
  }
...
async function cleanupSuggestions(testData: TestData, ids: string[]): Promise<void> {
  for (const id of ids) {
    try {
      await testData.api.delete(`/api/admin/suggestions/${id}`);
    } catch {
      /* ignore */
    }
  }
...
test.describe('Admin Suggestions Moderation (11.16)', () => {
  ...
  test.afterAll(async ({ testData }) => {
    await cleanupSuggestions(testData, seededIds);
  });

  test('pending queue loads with suggestion cards showing title, description, submitter, timestamp', async ({
    page,
    testData,
  }) => {
    const result = await seedSuggestion(testData);
  ...
  test('approve button moves suggestion to accepted and removes from queue', async ({
    page,
    testData,
  }) => {
    const result = await seedSuggestion(testData);
    ...
    const card = page.locator(
      `#suggestions-pending-queue .suggestion-card[data-id="${result.id}"]`,
    );
    await expect(card).toBeVisible();
  ...
  test('reject without reason shows warning that reason will be displayed publicly', async ({
    page,
    testData,
  }) => {
    const result = await seedSuggestion(testData);
  ...
  test('reject reason field is optional but UI makes encouragement clear', async ({
    page,
    testData,
  }) => {
    const result = await seedSuggestion(testData);
    ...
    const rejectDialog = page.locator('#suggestion-reject-dialog');
    await expect(rejectDialog).toBeVisible();
    const placeholder = await rejectDialog
      .locator('#reject-reason-input')
      .getAttribute('placeholder');
    expect(placeholder).toMatch(/optional|recommended|encouraged/i);
    await rejectDialog.locator('.btn-cancel-reject').click();
  });

  test('merge duplicate: search for original suggestion, select, confirm', async ({
    page,
    testData,
  }) => {
    const original = await seedSuggestion(testData, {
      title: 'Original Suggestion E2E',
      status: 'accepted',
    });
    ...
    await refreshSuggestionsList(page);
    const card = page.locator(
      `#suggestions-pending-queue .suggestion-card[data-id="${duplicate.id}"]`,
    );
    await expect(card).toBeVisible();
  ...
  test('merge duplicate transfers upvote count to original', async ({ page, testData }) => {
    ...
    await testData.api.post(`/api/admin/suggestions/${duplicate.id}/merge`, {
  ...
  test('merge duplicate creates notification for submitter', async ({ page, testData }) => {
    const original = await seedSuggestion(testData, {
      title: 'MergeNotify Orig E2E',
      status: 'accepted',
    });
    const duplicate = await seedSuggestion(testData, {
      title: 'MergeNotify Dup E2E',
      submitterUid: testData.secondUser.uid,
      submitterUniqueId: testData.secondUser.uniqueId,
    });
    seededIds.push(original.id, duplicate.id);
    await testData.api.post(`/api/admin/suggestions/${duplicate.id}/merge`, {
      targetId: original.id,
    });
    const notifications = await testData.api.get(
      `/api/admin/notifications?userId=${testData.secondUser.uid}&type=suggestion_merged`,
    );
    expect(
      (notifications.notifications || []).find((n: any) => n.suggestionId === duplicate.id),
    ).toBeTruthy();
  });

  test('dispute queue lists disputed merges', async ({ page, testData }) => {
    const original = await seedSuggestion(testData, {
      title: 'DisputeQ Orig E2E',
      status: 'accepted',
    });
    const duplicate = await seedSuggestion(testData, {
      title: 'DisputeQ Dup E2E',
      status: 'merged',
    });
    seededIds.push(original.id, duplicate.id);
    await testData.api.post(`/api/admin/suggestions/${duplicate.id}/dispute`, {
      reason: 'Different features',
    });
    await refreshSuggestionsList(page);
  ...
  test('dispute uphold is final and notification sent to submitter', async ({ page, testData }) => {
    const original = await seedSuggestion(testData, {
      title: 'DisputeUphold Orig',
      status: 'accepted',
    });
    const duplicate = await seedSuggestion(testData, {
      title: 'DisputeUphold Dup',
      status: 'merged',
      submitterUid: testData.secondUser.uid,
      submitterUniqueId: testData.secondUser.uniqueId,
    });
    ...
    const notifs = await testData.api.get(
      `/api/admin/notifications?userId=${testData.secondUser.uid}&type=dispute_resolved`,
    );
  ...
  test('dispute reject restores suggestion to pending queue', async ({ page, testData }) => {
    const original = await seedSuggestion(testData, {
      title: 'DisputeReject Orig',
      status: 'accepted',
    });
    const duplicate = await seedSuggestion(testData, {
      title: 'DisputeReject Dup',
      status: 'merged',
    });
    seededIds.push(original.id, duplicate.id);
    await testData.api.post(`/api/admin/suggestions/${duplicate.id}/dispute`, {
    ...
    expect((await testData.api.get(`/api/admin/suggestions/${duplicate.id}`)).status).toBe(
      'pending',
    );
  });

  test('link to roadmap: dropdown of roadmap features, selection saves', async ({
    page,
    testData,
  }) => {
  ...
  test('complete button with confirmation dialog', async ({ page, testData }) => {
    const result = await seedSuggestion(testData, {
      status: 'planned',
      linkedRoadmapFeature: 'voice-rooms',
      linkedRoadmapId: 'voice-rooms',
    });
    seededIds.push(result.id);
    await refreshSuggestionsList(page);
    ...
    expect((await testData.api.get(`/api/admin/suggestions/${result.id}`)).status).toBe(
      'completed',
    );
  });

  test('overturn available on all non-pending states with correct state transitions', async ({
    page,
    testData,
  }) => {
    ...
    await overturnDialog
      .locator('#overturn-reason-input')
      .fill('Reconsidered after team discussion');
  ...
  test('submitter identity links to view full identity graph', async ({ page, testData }) => {
    ...
    await expect(page.locator('.user-subtab[data-subtab="profile"]')).toBeVisible({
      timeout: 15_000,
    });
  ...
  test('duplicate highlighting shows similar existing suggestions alongside pending review', async ({
    page,
    testData,
  }) => {
    const existing = await seedSuggestion(testData, {
    ...
    await refreshSuggestionsList(page);
    const card = page.locator(
      `#suggestions-pending-queue .suggestion-card[data-id="${pending.id}"]`,
    );
    await expect(card).toBeVisible();
...
test.describe('Admin Unified Ban Management (11.17)', () => {
  ...
  test('graph shows all linked accounts, devices, networks with metadata', async ({
    page,
    testData,
  }) => {
    await navigateToIdentityGraph(page, String(testData.user.uniqueId));
  ...
  test('suspend: duration picker offers 1d/3d/7d/30d/permanent options', async ({
    page,
    testData,
  }) => {
    await navigateToIdentityGraph(page, String(testData.user.uniqueId));
  ...
  test('suspend: scope picker offers full and suggestions-only', async ({ page, testData }) => {
    ...
    expect(values).toContain('suggestions-only');
    await dialog.locator('.btn-cancel-suspend').click();
  });

  test('suspend: cascade preview shows affected devices, networks, accounts', async ({
    page,
    testData,
  }) => {
    await navigateToIdentityGraph(page, String(testData.user.uniqueId));
  ...
  test('suspend cascades to all linked identifiers at same level', async ({ page, testData }) => {
    ...
    expect(
      (graph.nodes || []).every((n: any) => n.suspended === true || n.type === 'account'),
    ).toBe(true);
    await testData.api.post(`/api/user/${testData.user.uniqueId}/unsuspend`, {});
  ...
  test('multi-account alert displayed when device linked to multiple accounts', async ({
    page,
    testData,
  }) => {
    await navigateToIdentityGraph(page, String(testData.user.uniqueId));
  ...
  test('unsuspend graph clears all identifiers', async ({ page, testData }) => {
    await testData.api.post(`/api/admin/identity-graph/${testData.user.uniqueId}/suspend-all`, {
      duration: 1,
      scope: 'full',
      reason: 'E2E unsuspend test',
    });
    await navigateToIdentityGraph(page, String(testData.user.uniqueId));
...
test.describe('Admin Audit Log (11.18)', () => {
  ...
  test('audit log tab loads with entries', async ({ page }) => {
    expect(
      (await page.locator('#audit-log-tbody tr').count()) > 0 ||
        (await page.locator('#audit-log-empty').isVisible()),
    ).toBe(true);
  });
...
test.describe('Admin Moderation Edge Cases (11.30)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    await setupApiMocks(page);
    page.on('dialog', async (dialog) => {
      if (dialog.type() === 'prompt') await dialog.accept('E2E');
      else await dialog.accept();
    });
    await adminLogin(page);
  });
  ...
  test('approve then immediately reject: second action fails', async ({ page, testData }) => {
    const r = await seedSuggestion(testData);
    seededIds.push(r.id);
    ...
    try {
      ...
      expect(e.message).toMatch(/4(09|00|22)/);
    }
  });

  test('reject with very long reason (2000+ chars): truncated or max enforced', async ({
    page,
    testData,
  }) => {
    const r = await seedSuggestion(testData);
    seededIds.push(r.id);
    try {
      await testData.api.post(`/api/admin/suggestions/${r.id}/reject`, {
        reason: 'X'.repeat(2500),
      });
      expect(
        (await testData.api.get(`/api/admin/suggestions/${r.id}`)).rejectReason.length,
      ).toBeLessThanOrEqual(2000);
    } catch (e: any) {
      expect(e.message).toMatch(/4(00|13|22)/);
    }
  });

  test('merge suggestion with 50+ votes: all transferred to original', async ({
    page,
    testData,
  }) => {
    ...
    expect(
      (await testData.api.get(`/api/admin/suggestions/${orig.id}`)).voteCount || 0,
  ...
  test('merge suggestion with itself returns error', async ({ page, testData }) => {
    ...
    seededIds.push(r.id);
    try {
      await testData.api.post(`/api/admin/suggestions/${r.id}/merge`, { targetId: r.id });
      expect(true).toBe(false);
    } catch (e: any) {
      expect(e.message).toMatch(/4(00|22)/);
    }
  });
  ...
  test('dispute on already-resolved dispute returns error', async ({ page, testData }) => {
    ...
    try {
      await testData.api.post(`/api/admin/suggestions/${dup.id}/dispute`, { reason: 'Again' });
      expect(true).toBe(false);
    } catch (e: any) {
      expect(e.message).toMatch(/4(00|09|22)/);
    }
  });
  ...
  test('admin overturn: audit log shows full history', async ({ page, testData }) => {
    ...
    await testData.api.post(`/api/admin/suggestions/${r.id}/overturn`, {
  ...
  test('two admins acting simultaneously: first wins, second gets conflict', async ({
    page,
    testData,
  }) => {
    const r = await seedSuggestion(testData);
  ...
  test('sort pending queue by submission date works', async ({ page, testData }) => {
    ...
    expect(
      await page
        .locator('#suggestions-pending-queue .suggestion-card')
        .first()
        .locator('.sg-title')
        .textContent(),
    ).toContain('SortTest Second');
  });

  test('bulk approve 10 suggestions: all transitioned, all audit logged', async ({
    page,
    testData,
  }) => {
    const ids = await seedMultipleSuggestions(testData, 10);
    seededIds.push(...ids);
    await navigateToSuggestions(page);
    await waitForPendingQueueLoaded(page);
    ...
    for (const id of ids)
      expect((await testData.api.get(`/api/admin/suggestions/${id}`)).status).toBe('accepted');
    await navigateToAuditLog(page);
...
test.describe('Admin Identity Graph Visualization (11.65)', () => {
  ...
  test('multi-account nodes: warning icon on device linking multiple accounts', async ({
    page,
    testData,
  }) => {
    const graph = await testData.api.get(`/api/admin/identity-graph/${testData.user.uniqueId}`);
  ...
  test('empty graph: "No identity data yet" message', async ({ page, testData }) => {
    ...
    // admin panel.
    const emptyGraph = await testData.api.get(
      `/api/admin/identity-graph/nonexistent-uid-${Date.now()}`,
    );
...
test.describe('Admin Panel Responsive Design (11.86)', () => {
  ...
  test('identity graph scrollable on mobile', async ({ browser, testData }) => {
    ...
    expect(
      await g.evaluate((el) => getComputedStyle(el).overflow || getComputedStyle(el).overflowX),
    ).toMatch(/scroll|auto/);
    await ctx.close();
  });
  ...
  test('audit log table horizontally scrollable on mobile', async ({ browser }) => {
    ...
    expect(
      await t.evaluate((el) => {
        ...
        return (
          s.overflowX === 'scroll' || s.overflowX === 'auto' || el.scrollWidth > el.clientWidth
        );
      }),
    ).toBe(true);
    await ctx.close();
  ...
  test('moderation action buttons accessible on mobile', async ({ browser }) => {
    ...
    const btns = page.locator(
      '#suggestions-panel .sg-btn-approve, #suggestions-panel .sg-btn-reject, #suggestions-panel .sg-btn-merge',
    );
...
test.describe('Admin Notifications (11.92)', () => {
  ...
  test('new pending suggestion shows badge on Suggestions tab', async ({ page, testData }) => {
    ...
    await page.reload();
    await adminLogin(page);
    await expect(page.locator('#tab-suggestions .tab-badge, #tab-suggestions .badge')).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('dispute filed shows indicator on dispute queue', async ({ page, testData }) => {
    ...
    await navigateToSuggestions(page);
    await expect(
      page.locator('#suggestions-dispute-tab').locator('.dispute-indicator, .badge'),
...
test.describe('Admin Bulk Operations (11.93)', () => {
  ...
  test('bulk approve: confirmation dialog, all transitioned', async ({ page, testData }) => {
    test.setTimeout(40_000);
    const ids = await seedMultipleSuggestions(testData, 3);
    seededIds.push(...ids);
    await refreshSuggestionsList(page);
    await page.locator('#suggestions-select-all').check();
    await page.locator('#suggestions-bulk-approve-btn').click();
  ...
  test('bulk reject: confirmation dialog, optional shared reason', async ({ page, testData }) => {
    ...
    await expect(page.locator('.toast.visible')).toBeVisible({ timeout: 30_000 });
    for (const id of ids) {
      const s = await testData.api.get(`/api/admin/suggestions/${id}`);
      expect(s.status).toBe('rejected');
      expect(s.rejectReason).toContain('not aligned');
    }
  });
...
test.describe('Admin Suggestion History Timeline (11.94)', () => {
  ...
  test('timeline: created -> approved -> planned -> completed', async ({ page, testData }) => {
    ...
    seededIds.push(r.id);
    await testData.api.post(`/api/admin/suggestions/${r.id}/approve`);
    await testData.api.post(`/api/admin/suggestions/${r.id}/status`, {
      status: 'planned',
      linkedRoadmapFeature: 'voice-rooms',
    });
    await testData.api.post(`/api/admin/suggestions/${r.id}/status`, { status: 'completed' });
    await navigateToSuggestions(page);
    await page.locator('#suggestions-completed-tab').click();
  ...
  test('timeline: created -> edited -> re-reviewed -> approved (edit diff shown)', async ({
    page,
    testData,
  }) => {
    const r = await seedSuggestion(testData, { title: 'TL Edit Orig' });
    seededIds.push(r.id);
    await testData.api.patch(`/api/admin/suggestions/${r.id}`, {
      title: 'TL Edit Updated',
      description: 'Updated',
    });
    await testData.api.post(`/api/admin/suggestions/${r.id}/approve`);
    await navigateToSuggestions(page);
  ...
  test('timeline: overturns with admin name and reason', async ({ page, testData }) => {
    ...
    seededIds.push(r.id);
    await testData.api.post(`/api/admin/suggestions/${r.id}/reject`, {
      reason: 'Initially rejected',
    });
    await testData.api.post(`/api/admin/suggestions/${r.id}/overturn`, {
      targetStatus: 'accepted',
      reason: 'Team reconsidered',
    });
    await navigateToSuggestions(page);
    await page.locator('#suggestions-accepted-tab').click();
    const card = page.locator(`.suggestion-card[data-id="${r.id}"]`);
    ...
    await expect(
      tl.locator('.timeline-entry:has-text("overturn")').locator('.timeline-admin-name'),
    ).toBeVisible();
  });
...
test.describe('Admin Contact Opt-In Flow (11.95)', () => {
  ...
  test('shows "Open to contact" indicator when submitter opted in', async ({ page, testData }) => {
    const r = await seedSuggestion(testData, { contactOptIn: true });
    seededIds.push(r.id);
    await refreshSuggestionsList(page);
  ...
  test('shows "No contact" when submitter did not opt in', async ({ page, testData }) => {
    const r = await seedSuggestion(testData, { contactOptIn: false });
    seededIds.push(r.id);
    await refreshSuggestionsList(page);
    const card = page.locator(`#suggestions-pending-queue .suggestion-card[data-id="${r.id}"]`);
    await expect(card).toBeVisible();
  ...
  test('admin clicks "Contact submitter": shows uniqueId to look up in Users tab', async ({
    page,
    testData,
  }) => {
    const r = await seedSuggestion(testData, { contactOptIn: true });
