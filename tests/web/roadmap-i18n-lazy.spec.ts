...
async function setupPage(
  ...
  await page.context().route('https://github.com/**', (r) =>
    r.fulfill({
      status: 200,
  ...
  await page.route('**/api/translate', async (route: Route) => {
    ...
    for (const t of body.texts) translations[t] = `[${body.target.toUpperCase()}] ${t}`;
    return route.fulfill({
      status: 200,
      contentType: 'application/json',
...
test.describe('non-English lazy translation', () => {
  ...
  test('item names translate in place; legacy features keep embedded i18n path', async ({
    ...
    await expect(page.locator('.feature-item', { hasText: '[DE] Tracked story one' })).toHaveCount(
      1,
    );
    // Legacy feature untouched by the service (no embedded de payload → raw name)
    await expect(page.locator('.feature-item', { hasText: 'Legacy feature' })).toHaveCount(1);
  });
  ...
  test('>50 strings chunk into multiple requests fired as one round', async ({ page }) => {
    const { translateCalls } = await setupPage(page, {
  ...
  test('service failure: page keeps English, exactly one console.error, no layout break', async ({
    ...
    await expect(page.locator('.feature-item', { hasText: 'Tracked story one' })).toHaveCount(1);
    const translateErrors = consoleErrors.filter((e) => e.includes('[translate]'));
  ...
  test('rate-limited response is fail-silent too', async ({ page }) => {
    ...
    await expect(page.locator('.feature-item', { hasText: 'Tracked story one' })).toHaveCount(1);
    expect(consoleErrors.filter((e) => e.includes('[translate]'))).toHaveLength(1);
  });
});
...
test.describe('RTL locale', () => {
  ...
  test('Arabic translations apply and the dialog renders in the RTL layout', async ({ page }) => {
    await setupPage(page);
    await expect(page.locator('.feature-item', { hasText: '[AR] Tracked story one' })).toHaveCount(
      1,
    );
    await page.locator('a.shy-badge', { hasText: 'SHY-9001' }).click();
    await expect(page.locator('.shy-story-dialog')).toBeVisible();
