  ...
  (function walk(dir: string) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        discovered.push('/' + path.relative(publicRoot, full).split(path.sep).join('/'));
      }
    }
  })(publicRoot);
...
test.describe('Preview watermark — git identity lines (SHY-0205)', () => {
  ...
  test('branch and sha arrive via serve-web meta injection', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(page.locator('#preview-watermark')).toContainText(liveSha.slice(0, 7));
  });

  test('server-echo line shows the api marker with a health dot', async ({ page }) => {
    await page.goto('/');
    const watermark = page.locator('#preview-watermark');
  ...
  test('an unreachable backend turns the dot red — REAL induced failure', async ({ page }) => {
    const healthLogs: string[] = [];
    page.on('console', (msg) => {
      if (msg.text().includes('[ShyTalk] server health')) healthLogs.push(msg.text());
    });
  ...
  test('every badge line clamps to a single row — no wrap explosion', async ({ page }) => {
    await page.goto('/');
    await page.evaluate(() => {
...
test.describe('Preview watermark — QA context lines (SHY-0205)', () => {
  test('journey marker line appears while set and disappears when cleared', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('#preview-watermark')).not.toContainText('▶');
    await page.evaluate(() => {
      (window as any).__journey_marker = 'j01 s03';
    });
    await expect(page.locator('#preview-watermark')).toContainText('▶ j01 s03', {
      timeout: 5_000,
    });
    await page.evaluate(() => {
      (window as any).__journey_marker = '';
  ...
  test('cohort pairs with the UID when the hook is set', async ({ page }) => {
    ...
    await page.goto('/');
    await expect(page.locator('#preview-watermark')).toContainText('UID: 10000005 · adult');
  });

  test('locale and route pair on one line', async ({ page }) => {
    await page.goto('/roadmap.html');
    await expect(page.locator('#preview-watermark')).toContainText('· /roadmap.html');
  });

  test('zh locale switch is reflected in the locale line (real i18n path)', async ({ page }) => {
