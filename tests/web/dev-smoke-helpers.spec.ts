...
test.describe("deriveOwnerFirebaseUid — uid claim extraction", () => {
  test("prefers user_id when both user_id and sub are present", () => {
    expect(
      deriveOwnerFirebaseUid({ user_id: "uid-primary", sub: "uid-standard" }),
    ).toBe("uid-primary");
  });

  test("falls back to sub when user_id is absent", () => {
    expect(deriveOwnerFirebaseUid({ sub: "uid-standard" })).toBe(
      "uid-standard",
    );
  });
  ...
  test("skips a non-string user_id and uses a string sub", () => {
    ...
    expect(deriveOwnerFirebaseUid({ user_id: 12345, sub: "uid-standard" })).toBe(
      "uid-standard",
    );
  });
  ...
  test("returns undefined when both claims are non-string values", () => {
    expect(
      deriveOwnerFirebaseUid({ user_id: 12345, sub: { nested: true } }),
    ).toBeUndefined();
  });
