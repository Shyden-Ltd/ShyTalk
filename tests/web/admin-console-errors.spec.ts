...
test.describe('Admin Console Error Checks', () => {
  ...
  test('full tab navigation produces zero console errors', async ({ page }) => {
    const tabs = [
      'Users', 'Reports', 'Appeals', 'Logs', 'Devices',
      'Gifts', 'Banners', 'Fun Facts', 'Economy',
      'Spin Monitor', 'Backups', 'Maintenance',
    ];
  ...
  test('user search and subtab navigation produces zero console errors', async ({ page, testData }) => {
    const errors = await collectConsoleErrors(page, async () => {
  ...
  test('opening and closing dialogs produces zero console errors', async ({ page }) => {
    const errors = await collectConsoleErrors(page, async () => {
  ...
  test('triggering and cancelling operations produces zero console errors', async ({ page, testData }) => {
    const errors = await collectConsoleErrors(page, async () => {
      ...
      await page.locator('#monitor-uid-input').fill(String(testData.user.uniqueId));
      ...
      await page.waitForFunction(() => {
        ...
        return list && (list.querySelector('.report-card') !== null ||
    ...
    const realErrors = filterBenignErrors(errors);
    if (realErrors.length > 0) {
      console.log('Console errors during trigger/cancel operations:', realErrors);
    }
    expect(realErrors.length).toBe(0);
  });
});
