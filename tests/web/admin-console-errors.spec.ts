 ...
 */
async function collectConsoleErrors(page: Page, callback: () => Promise<void>): Promise<string[]> {
  const errors: string[] = [];
...
function filterBenignErrors(errors: string[]): string[] {
  return errors.filter((e) => {
    ...
    if (lower.includes('stream') && (lower.includes('error') || lower.includes('close')))
      return false;
...
test.describe('Admin Console Error Checks', () => {
  ...
  test('maintenance nuclear dialog produces zero console errors', async ({ page }) => {
    const errors = await collectConsoleErrors(page, async () => {
  ...
  test('gifts add and remove row produces zero console errors', async ({ page }) => {
    const errors = await collectConsoleErrors(page, async () => {
  ...
  test('logs settings expand and collapse produces zero console errors', async ({ page }) => {
    const errors = await collectConsoleErrors(page, async () => {
