...
class WorkerApiClientDeviceIdTest {
    ...
    @Test
    fun `X-Device-Id header is present on postPublic requests`() = runTest {
        val interceptor = RecordingInterceptor()
        val client = createClient("test-device-123", interceptor)
    ...
    @Test
    fun `X-Device-Id value is consistent across multiple public calls`() = runTest {
        val interceptor = RecordingInterceptor()
    ...
    @Test
    fun `X-Device-Id header value matches provided deviceId`() = runTest {
        val interceptor = RecordingInterceptor()
    ...
    @Test
    fun `X-Device-Id header is present with empty device id`() = runTest {
        val interceptor = RecordingInterceptor()
    ...
    @Test
    fun `X-Device-Id header is present on postPublic with body`() = runTest {
        val interceptor = RecordingInterceptor()
    ...
    @Test
    fun `different clients with different device IDs send correct headers`() = runTest {
