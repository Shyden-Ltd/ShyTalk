...
class SeatRequestRepositoryImplTest {
    ...
    @Test
    fun `createRequest returns Success`() =
        runTest {
            coEvery { api.post("/api/rooms/room-1/seat-requests", any()) } returns
                JSONObject().apply {
                    put("requestId", "req-1")
                }
    ...
    @Test
    fun `createRequest returns Error on exception`() =
        runTest {
    ...
    @Test
    fun `approveRequest returns Success`() =
        runTest {
    ...
    @Test
    fun `approveRequest returns Error on exception`() =
        runTest {
    ...
    @Test
    fun `denyRequest returns Success`() =
        runTest {
    ...
    @Test
    fun `denyRequest returns Error on exception`() =
        runTest {
            every { mockDocRef.update(any<Map<String, Any>>()) } returns Tasks.forException(RuntimeException("Fail"))
    ...
    @Test
    fun `cancelApprovedRequest returns Success`() =
        runTest {
    ...
    @Test
    fun `cancelApprovedRequest returns Error on exception`() =
        runTest {
            every { mockDocRef.update(any<Map<String, Any>>()) } returns Tasks.forException(RuntimeException("Fail"))
