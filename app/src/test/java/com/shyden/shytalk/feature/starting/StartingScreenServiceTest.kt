...
class StartingScreenServiceTest {
    ...
    private fun parseScreens(data: Map<String, Map<String, Any?>>): Map<String, StartingScreen> {
        return data.map { (id, map) ->
            id to StartingScreen(
                screenId = id,
        ...
        return when (apiResult) {
            is Resource.Success -> {
                ...
                if (blocker != null) {
                    ...
                    val dismissable = enabledScreens
            ...
            is Resource.Error -> {
                if (cachedBlocker != null) StartupState.BLOCKED
    ...
    fun `parses API response with all fields`() {
        val data = mapOf(
            "preLaunchGate" to mapOf<String, Any?>(
                "enabled" to true,
    ...
    fun `parses multiple screens`() {
        val data = mapOf(
            "screen1" to mapOf<String, Any?>(
                "enabled" to true, "dismissable" to false,
                "frequency" to "every_launch", "template" to "warning",
                "title" to "Blocked", "message" to "msg",
            ),
            "screen2" to mapOf<String, Any?>(
                "enabled" to true, "dismissable" to true,
                "frequency" to "once", "template" to "info",
                "title" to "Info", "message" to "msg",
            ),
        )
        val screens = parseScreens(data)
    ...
    fun `API failure with cached blocker returns BLOCKED (fail-safe)`() {
        val cached = StartingScreenCache.CachedScreen(
            screenId = "cached", contentHash = "hash", enabled = true,
            dismissable = false, frequency = "every_launch", template = "warning",
            title = "Cached", message = "msg", imageType = null,
            backgroundImage = null, backgroundImagePath = null,
        )
    ...
    fun `allowlist override makes blocking screen dismissable in API response`() {
        ...
        val screen = StartingScreen(
            "blocker", true, true, "every_launch", "warning", "B", "msg",
        )
