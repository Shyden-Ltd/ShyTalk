...
Every piece of work is captured as ONE detailed user-story `.md` file at `.project/stories/SHY-XXXX-slug.md` and ships as ONE PR. The story IS the spec — operator and reviewer score against the AC + BDD scenarios; architect validates the spec before code starts. Source-of-truth lives in this repo; the Projects v2 board (every story) + GitHub Issues (bugs only) are an automatically-synced mirror (SHY-0074 architecture v2).
...
- `scripts/sync-stories-to-issues.sh` (SHY-0074 architecture v2) mirrors each story `.md` to the Projects v2 board — draft cards for non-bugs, bug-report Issues (+ issue-backed cards) for `type: bug`. One-way sync — `.md` is the source of truth, the board/issue is a derived view.
...
### Board + Issues mirror (SHY-0074 architecture v2)

- **Each GitHub surface has ONE job:** Issues tab = open bug list; Project board = delivery state for EVERY story; `.md` files = the specs (source of truth). Operator MUST edit spec content in the `.md` file (not on GitHub) — board/issue edits are stomped on the next sync.
- **Routing:** non-bug stories become board DRAFT items (`addProjectV2DraftIssue`) carrying the full spec verbatim — NO Issues-tab entry. `type: bug` stories get a real GitHub Issue framed as a bug report (`## Bug` = the story's `## Why` verbatim; `## Tracking` = Source link + board link + status line) plus their issue-backed board card. A `type:` flip deletes the stale backing and recreates the correct one (bug→non-bug also closes the orphaned issue as not-planned).
- **Issue lifecycle follows the story:** on every lifecycle move the sync posts a `Status: <old> → <new>` comment on the bug issue (detected via the body-footer `_Status: X_` marker — status lives in frontmatter, outside the body hash, so the marker is what makes pure status flips visible); `Done` closes the issue as completed (comment names `released_in:` when present); `Cancelled` closes as not-planned. Drafts get body/field refreshes only (no timeline).
- **Body format (both backings):** content + footer `_Source: <absolute blob URL>_` + `_Status: <lifecycle>_` + `_Last synced: <UTC> from commit <sha> body-hash: <hex>_`. Bodies that would exceed GitHub's 65,536-char cap are line-truncated with an explicit `[spec truncated — N chars omitted…]` notice ahead of the intact footer.
- **Labels (single-source):** bug issues carry exactly ONE mirror label — `story`. The old `status:*` / `priority:*` / `effort:*` / `type:*` / `roadmap:*` families are gone: a fact lives in its board column only (Status / Pri / Effort / Type / Roadmap IDs). The sync deletes any resurrected family labels repo-wide on every run (idempotent; foreign labels like `dependencies` untouched).
- **Board Status column:** story lifecycle maps 1:1 onto the board's built-in Status field — `Draft`→`Todo`, `In Progress`→`In Progress`, `In Review`→`In Review`, `Done`→`Done`, `Cancelled`→`Cancelled` — set on create AND update for both backings, ordered last (last-writer over GitHub's "Item added → Todo" automation).
- **Items map:** ONE paginated GraphQL query (100/page) loads every board item keyed by SHY ID at run start — it feeds every create-vs-update decision (replacing per-story `gh issue list` searches). If that query fails the run aborts exit-40 BEFORE any mutation (a wrong map would mean duplicate creates).
...
- **`--rebuild` (one-shot v1→v2 migration):** deletes every board item (`deleteProjectV2Item`) + every `story`-labeled issue (`deleteIssue`) then resyncs fresh. Gated on `REBUILD_CONFIRM=yes` (exit 2 without it); dispatchable via the sync workflow's `rebuild` input (the dispatch click IS the confirm). `deleteIssue` permission gaps surface as loud actionable warnings; teardown continues.
- **PR `Closes #N` injection:** the `inject-pr-closes.yml` workflow appends `Closes #<issue-number>` to a PR body when the branch matches `story/SHY-NNNN-*` and the body doesn't already contain the close ref. Auto-merge then closes the issue. Non-bug stories have no issue — the workflow SKIPs cleanly (exit 0). Fork PRs skip with a log entry (their `pull_request` trigger token is read-only).
