...
- **`currentlyWorkingOn` derivation:** SHYs with `public: true` AND `status: In Progress` OR `status: In Testing` appear in `roadmap-data.json`'s `currentlyWorkingOn` array (SHY-0161 — In Testing is active work under the batch gauntlet). Any other status only appears under `phases[].items`.
- **Lag expectation:** a SHY status flip propagates to the public webpage within ~90s of the push **to `main`** (CI workflow wall-clock budget; typical ~30s). Under git-flow (SHY-0161) `sync-roadmap-data.yml` regenerates the public roadmap ONLY on `main` pushes, so a flip that happens on `develop` first (feature→`develop` merge) surfaces publicly at the `develop → main` promotion, not before — the public roadmap intentionally shows `main` state, never the in-flight `develop` batch.
