...
- **Develop-source + concurrency (SHY-0177):** the sync triggers on push to **develop** (the git-flow integration branch) ONLY and pins its checkout to `ref: develop`, so even a `workflow_dispatch` clicked from main syncs develop truth. Never add main back as a trigger: change detection is an equality body-hash with no version ordering, so a main-triggered run after a develop one would regress the board to main's older content. Footer `_Source:` links point at `blob/develop`; the `board-items.json` sidecar is committed to develop (live-head `expectedHeadOid` + one head-race retry, `[skip ci]`). Concurrency is a GLOBAL group (`sync-stories`, `cancel-in-progress: false`) — one shared board ⇒ at most one sync at a time across all triggers.
...
- **NEVER open the develop→main promotion PR with `develop` as the head branch (HARD, SHY-0296).** Cut a throwaway `promote/YYYY-MM-DD` from develop and open the PR from THAT. GitHub deletes a merged PR's _head_ branch and this repo sets `delete_branch_on_merge: true` — so merging a promotion PR whose head is `develop` **deletes the integration branch**. It happened on 2026-08-13 with PR #1652: `gh pr merge` reported success, `main` correctly received all 78 commits and 41 stories, and the first sign of trouble was `git fetch` failing with `unknown revision origin/develop`. The same trap applies to ANY long-lived branch used as a PR head; the danger is the head-branch deletion, not the name.
  **Recovering a deleted protected branch:** `git push origin <sha>:refs/heads/develop` is REJECTED — creating a branch has no status checks to satisfy ruleset `19719048`'s `required_status_checks` (its `do_not_enforce_on_create` is `false`), so it fails with "N of 3 required status checks are expected". Do NOT disable the ruleset. Push a commit that **already carries those checks** — the branch's own pre-merge head qualifies, having been the head of a merged PR that ran the full develop CI — which satisfies the rule honestly and is also the correct restoration, since it puts the branch back exactly where it was. Mechanically: `git branch <tmp> <sha>` then `git push origin <tmp>:refs/heads/develop` (a plain create + push; `git branch -f` and the raw git-refs API read as force operations).
...
- **Develop branch protection: ruleset `19719048` (`name: develop`)** — delivered by SHY-0242. `required_status_checks` with the SAME 3 contexts as main (`Detect Changes` + `Analyze JavaScript` + `PR Gate`, `integration_id` 15368, non-strict), scoped to `refs/heads/develop` only (NOT `~ALL`). The Release App (App ID 29110) is in `bypass_actors` (`always`) so the board-sync sidecar `createCommitOnBranch` to develop still lands. develop also inherits the global `non_fast_forward` ruleset `16058327` (target `~ALL`). **So every develop PR — stories AND Dependabot — now gates on CI green** (`pr-checks.yml`/`codeql.yml` trigger on develop PRs; device-E2E stays base-main-guarded so feature→develop pays no emulator cost). To change the required checks: edit ruleset 19719048 via `gh api`.
- **Merge policy — EVERYTHING goes through develop first (operator 2026-07-25):** all work (INCLUDING CI-config/workflow changes) is cut from `develop` and lands on `develop`, where it is CI-tested (and device-gauntleted if it touches a runtime surface). `main` receives changes ONLY via the develop→main promotion (the release gauntlet) — develop autonomous, main gated. The old "CI-config-only → main directly" shortcut is **RETIRED** (it only existed because develop had no CI; SHY-0242 gave develop its own gate). Mechanical caveat: `.github/dependabot.yml` must physically reach `main` (Dependabot reads config from the default branch), but it too lands on develop first and rides to main on the next promotion — never a direct main PR.
...
- **Backend = full gauntlet (HARD GLOBAL — operator 2026-06-18, SHY-0127; scope narrowed by SHY-0339, operator 2026-08-18):** ANY backend **runtime** change (`express-api/src/**`, `express-api/scripts/**`, `express-api/package.json` and other non-test files under `express-api/`, plus `firestore.rules`, `database.rules.json`) is treated as "everything needs testing" — ALL apps + ALL webpages — because the backend is the **shared core** every client depends on, so a backend regression can break any of them. The FULL device + all-browser matrix runs; NO per-platform skip applies to a backend change. CI enforces this: `detect-changes` forces `app/android/ios/web/integration` true (and overrides the `[skip-*-e2e]` markers) whenever `backend_changed`. **`express-api/tests/**` is deliberately EXCLUDED (SHY-0339):** it sets `backend_tests_changed`, which runs the backend suite but does NOT force the client matrix. The rule's rationale is that a backend regression can break any client — a test file is never shipped to a client and cannot cause one. Before this, editing a test ran five browser suites and the device matrix. Same reasoning SHY-0284 applied to root `scripts/**`. Pinned by `express-api/tests/scripts/pr-checks-playwright-scope.test.js`.
...
## No Stubs / Mocks / Fakes — Real Only (HARD GLOBAL RULE — operator 2026-06-13, hardened 2026-06-17)
...
**The ONE place test doubles are allowed — unit tests (operator hardened 2026-06-17):** test doubles (mock / fake / stub / spy / fetch-mock / hand-rolled fake) are permitted **ONLY in unit tests** — pure isolated logic with **no real collaborator**. **Every other layer — integration, journey-runner, e2e and device — is real-only.** Classification is **by what the test exercises**, not by what it is named: if it touches Firestore / Auth / the Express API / LiveKit / a repository / the network, it is an integration test and must run against the real local stack. A real-services failure is assumed to be a **real product bug**, not a broken test — prove it with evidence first; never "fix" a red by faking the pass.

**The unit↔integration boundary convention (greppable; enforced by `scripts/check-no-new-stubs.js`):** a unit-test location is `**/tests/unit/**` or a file named `*.unit.test.js` / `*.unit.test.ts` (JS/TS), or a **non-instrumented (host) Kotlin test source set** — `commonTest` / `jvmTest` / `androidHostTest` / `src/test` (these run on the host JVM). Everything else is integration/real-only, including **instrumented** Kotlin source sets (`androidTest` / `androidInstrumentedTest`, which need a real device) and all iOS `*Tests`. A double in a unit-test location is exempt from the ratchet; the same pattern anywhere else is a violation. The location is a mechanical proxy for "what it exercises" — `code-reviewer` + the per-story Test Plan are the backstops that catch a mislabelled integration test.

**What this bans (outside unit tests):**
...
- Tests run against **real services**: the local emulator stack (`local/start.sh` — Firebase Emulators + LiveKit + MinIO + Mailpit are _real_ local backends, $0), real dev backends, real test personas, real devices, real browsers. The local stack IS the real backend for local testing — use it instead of in-process fakes.
