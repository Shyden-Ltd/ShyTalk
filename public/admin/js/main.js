...
const _tabPaths = {
  ...
  support: "/admin/js/tabs/support.js",
...
window._updatePityHardLimit = (val) => {
  cachedPityHardLimit = val;
};
...
const _moduleDeps = {
  ...
  searchUserByUniqueId: (uid) =>
    TAB_MODULES["users"]?.searchUserByUniqueId?.(uid),
  renderEvidence: (urls) => TAB_MODULES["users"]?.renderEvidence?.(urls) ?? "",
  openEvidenceLightbox: (url, type) =>
    TAB_MODULES["users"]?.openEvidenceLightbox?.(url, type),
  auth,
};
  ...
  if (user) {
    ...
    if (tokenResult.claims.admin !== true) {
      ...
      loginError.textContent =
        "Access denied \u2014 admin privileges required. If you have a portal account, go to /portal/ instead.";
    ...
    try {
      ...
      if (
        savedReportFilter &&
        ["pending", "resolved", "archived"].includes(savedReportFilter)
      ) {
        for (const btn of document.querySelectorAll("[data-report-filter]")) {
          btn.classList.toggle(
            "active",
            btn.dataset.reportFilter === savedReportFilter,
          );
        }
      }
...
loginBtn.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (
      err.code === "auth/visibility-check-was-unavailable" ||
      (err.message && err.message.includes("visibility-check"))
    ) {
      try {
        await signInWithEmailAndPassword(auth, email, pass);
      } catch (retryErr) {
