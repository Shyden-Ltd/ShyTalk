...
export function _register(name, fn) {
  const map = {
    ...
    flushNotifications: (f) => {
      _flushNotifications = f;
    },
    loadSecurityPanel: (f) => {
      _loadSecurityPanel = f;
    },
    loadIdentitySubtabGraph: (f) => {
      _loadIdentitySubtabGraph = f;
    },
    onAgeVerifSubtabOpen: (f) => {
      _onAgeVerifSubtabOpen = f;
    },
  };
...
function isVideoUrl(url) {
  return (
    /\.(mp4|mov|webm|avi|mkv|3gp)(\?|$)/i.test(url) ||
...
export function switchUserSubtab(subtab) {
  document
    .querySelectorAll('.user-subtab')
    .forEach((b) => b.classList.toggle('active', b.dataset.subtab === subtab));
  document
    .querySelectorAll('.user-subpanel')
    .forEach((p) => p.classList.toggle('visible', p.dataset.subtab === subtab));
...
export function init(deps) {
  ...
  if (searchUidEl)
    searchUidEl.addEventListener('keydown', (e) => {
...
function getListWidget(field) {
  const map = {
    ...
    followingIds: 'list-followingIds',
    followerIds: 'list-followerIds',
  };
  return $(`#${map[field]}`);
}
...
export async function populateForm(data) {
  ...
  if (allUids.size > 0) {
    try {
      const result = await apiCall('POST', '/api/resolve/uids-to-uniqueIds', {
        uids: [...allUids],
      });
  ...
  if (createdAtEl)
    createdAtEl.textContent = data.createdAt ? new Date(data.createdAt).toLocaleString() : '\u2014';
  const lastSeenEl = $('#field-lastSeenAt');
  if (lastSeenEl)
    lastSeenEl.textContent = data.lastSeenAt
...
export function isEconomyFieldChanged(fieldName) {
  ...
  if (fieldName === 'loginStreak' || fieldName === 'pityCounter')
    return (Number(v) || 0) !== (Number(orig) || 0);
  if (fieldName === 'superShyExpiry') {
...
export async function undoFieldSave(fieldEl, previousValue) {
  ...
  try {
    await apiCall('PATCH', '/api/user/' + currentUid + '?silent=true', {
...
export function wireEmailAndClearButtons() {
  ...
  for (const field of ['blockedUserIds', 'followingIds', 'followerIds']) {
    ...
    const doAdd = () => {
      ...
      if (exists) {
        showToast(window.tAdmin('toast_already_in_list'), 'error');
        return;
      }
...
export function populateSuspensionSection(data) {
  ...
  if (data.isSuspended) {
    const since = data.suspensionStartDate
      ? new Date(data.suspensionStartDate).toLocaleString()
      : 'unknown';
    const until = data.suspensionEndDate
      ? new Date(data.suspensionEndDate).toLocaleString()
      : window.tAdmin('msg_permanent');
    if (suspensionStatus) {
      suspensionStatus.className = 'suspension-status suspended';
      suspensionStatus.textContent = window.tAdminFmt('status_suspended_badge', {
        since,
        until,
        reason: data.suspensionReason || window.tAdmin('msg_no_reason_provided'),
      });
    }
  ...
  if (suspendEndDate) suspendEndDate.value = '';
  const suspendCanAppeal = $('#suspend-can-appeal');
  if (suspendCanAppeal) suspendCanAppeal.checked = false;
}
...
export function populateDeletionSection(data) {
  ...
  if (data.deletionScheduledAt) {
    ...
    if (deletionStatusBadge) {
      deletionStatusBadge.textContent = window.tAdminFmt('status_deletion_scheduled', {
        days: daysRemaining,
        date: executeDate,
      });
...
export function populateGcsSection(data) {
  ...
  if (gcsLastDeduction)
    gcsLastDeduction.textContent = lastDeduction
      ? new Date(lastDeduction).toLocaleString()
      : 'Never';
}
...
export async function loadReportHistory(uid) {
  ...
  try {
    ...
    const allReports = Array.isArray(result)
      ? result
      : result.users
        ? result.users.flatMap((u) => u.reports)
        : [];
    if (allReports.length === 0) {
      reportHistoryList.innerHTML =
        '<div style="color:var(--text2);font-size:12px;font-style:italic;">No report history</div>';
      return;
    }
    reportHistoryList.innerHTML = '';
    for (const r of allReports.slice(0, 10)) {
      ...
      div.innerHTML =
        '<strong>' +
        escapeHtml(r.reason || 'Unknown') +
        '</strong> \u2014 ' +
        escapeHtml(r.resolvedAction || '?') +
        (r.severity ? ' (Severity: ' + r.severity + ')' : '') +
        '<br><span style="color:var(--text2)">' +
        (r.adminNote ? escapeHtml(r.adminNote) : '') +
        '</span><br><span style="color:var(--text2);font-size:11px">' +
        (r.resolvedAt ? escapeHtml(new Date(r.resolvedAt).toLocaleString()) : '') +
        '</span>';
      reportHistoryList.appendChild(div);
    }
  } catch (err) {
    reportHistoryList.innerHTML =
      '<div style="color:var(--danger);font-size:12px;">' + escapeHtml(err.message) + '</div>';
  }
}
...
function renderWarningItem(w, uid) {
  ...
  sev.textContent = window.tAdminFmt('status_severity_gcs', {
    severity: w.severity,
    deduction: w.gcsDeduction || 0,
  });
  ...
  if (!w.revoked) {
    ...
    rb.className = 'btn-revoke-warning';
    rb.textContent = window.tAdmin('btn_revoke');
    rb.addEventListener('click', () => revokeWarning(uid, w.id, w.gcsDeduction || 0, rb));
    right.appendChild(rb);
  } else {
    const rs = document.createElement('span');
    rs.style.cssText = 'color:var(--text2);font-size:11px;font-style:italic;';
    rs.textContent = window.tAdmin('inline_revoked');
  ...
  if (w.adminNote) {
    ...
    note.textContent = window.tAdminFmt('inline_warning_note', { note: w.adminNote });
    item.appendChild(note);
  }
  const meta = document.createElement('div');
  meta.style.cssText = 'color:var(--text2);font-size:11px;margin-top:2px;';
  meta.textContent = window.tAdminFmt('inline_warning_meta', {
    issuedBy: w.issuedByName || w.issuedBy || 'System',
    gcsBefore: w.gcsBefore || '?',
    gcsAfter: w.gcsAfter ?? '?',
  });
...
export async function populateFormFull(data) {
  ...
  if (window._suspensionTimer) {
    clearTimeout(window._suspensionTimer);
    window._suspensionTimer = null;
  }
  if (data.isSuspended) {
    ...
    if (data.suspensionEndDate) {
      ...
      if (msLeft <= 0) {
        ...
        window._suspensionTimer = setTimeout(() => {
          if (banner) banner.style.display = 'none';
        }, msLeft);
      }
    }
...
export async function loadSecurityPanel() {
  ...
  try {
    ...
    if (pinSetAt)
      pinSetAt.textContent = authRes.pinSetAt
        ? new Date(authRes.pinSetAt).toLocaleString()
        : '\u2014';
    ...
    if (pinLockedUntil)
      pinLockedUntil.textContent = authRes.pinLockedUntil
        ? new Date(authRes.pinLockedUntil).toLocaleString()
        : '\u2014';
    ...
    if (pinIsLocked) {
      pinIsLocked.textContent = authRes.isLocked ? 'YES' : 'No';
      pinIsLocked.style.color = authRes.isLocked ? 'var(--danger, #f44)' : 'inherit';
    }
    const resetBtn = document.getElementById('reset-pin-lockout-btn');
    if (resetBtn)
      resetBtn.style.display =
        authRes.pinAttempts > 0 || authRes.isLocked ? 'inline-block' : 'none';
    ...
    if (keysList) {
      if (authRes.biometricKeys.length === 0) {
        ...
        for (const k of authRes.biometricKeys) {
          ...
          btn.addEventListener('click', () =>
            revokeBiometricKey(btn.dataset.uid, btn.dataset.deviceId),
          );
...
export function wireModerationListeners() {
  ...
  if (suspendBtn)
    suspendBtn.addEventListener('click', async () => {
      ...
      if (!reason) {
        showToast(window.tAdmin('toast_reason_required'), 'error');
        return;
      }
      ...
      try {
        const result = await apiCall('POST', `/api/user/${currentUid}/suspend`, {
          reason,
          endDate,
          canAppeal,
        });
        window.PartialFailureToast?.showResultToast(
          showToast,
          result,
          window.tAdmin('toast_user_suspended'),
        );
      ...
      try {
        ...
        if (result?.alreadyUnsuspended) {
          ...
          window.PartialFailureToast?.showResultToast(
            showToast,
            result,
            window.tAdmin('toast_user_unsuspended'),
          );
        }
        const data = await apiCall('GET', `/api/user/${currentUid}`);
        await populateFormFull(data);
  ...
  if (resetGcsBtn)
    resetGcsBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showToast(window.tAdmin('toast_gcs_reset_100'));
        const data = await apiCall('GET', `/api/user/${currentUid}`);
        populateGcsSection(data);
      } catch (err) {
        showToast(err.message, 'error');
  ...
  // Warning load more
  const warningLoadMoreBtn = document.getElementById('warning-load-more-btn');
  if (warningLoadMoreBtn)
    warningLoadMoreBtn.addEventListener('click', () => {
  ...
  if (directWarnBtn)
    directWarnBtn.addEventListener('click', async () => {
      ...
      if (!reason) {
        showToast(window.tAdmin('toast_select_reason'), 'error');
        return;
      }
      const severity = parseInt(
        document.querySelector('input[name="direct-warn-severity"]:checked')?.value || '3',
      );
      const adminNote = $('#direct-warn-note')?.value?.trim() || undefined;
      if (
        !confirm(
          window.tAdminFmt('confirm_issue_warning', { reason, severity, deduction: severity * 5 }),
        )
      )
        return;
      directWarnBtn.disabled = true;
      directWarnBtn.textContent = window.tAdmin('btn_issuing');
      try {
        const result = await apiCall('POST', `/api/user/${currentUid}/warn`, {
          reason,
          severity,
          adminNote,
        });
        ...
        window.PartialFailureToast?.showResultToast(
          showToast,
          result,
          window.tAdmin('toast_warning_issued_successfully'),
        );
        if (result.autoEscalateSuggested)
          showToast(window.tAdmin('toast_auto_escalate_5_warnings'), 'error');
        const data = await apiCall('GET', `/api/user/${currentUid}`);
        populateGcsSection(data);
        ...
        directWarnBtn.textContent = window.tAdmin('btn_issue_warning');
      }
    });
  // Account deletion
  const scheduleDeletionBtn = $('#schedule-deletion-btn');
  if (scheduleDeletionBtn)
    scheduleDeletionBtn.addEventListener('click', async () => {
      if (scheduleDeletionBtn.disabled) return;
      const reason = prompt(window.tAdmin('prompt_deletion_reason'));
      if (reason === null) return;
      if (!confirm(window.tAdmin('confirm_schedule_deletion'))) return;
      scheduleDeletionBtn.disabled = true;
      try {
        await apiCall('POST', `/api/user/${currentUid}/delete`, { reason });
        alert(window.tAdmin('alert_deletion_scheduled'));
        const freshData = await apiCall('GET', `/api/user/${currentUid}`);
        populateDeletionSection(freshData);
      } catch (err) {
        alert(window.tAdminFmt('alert_schedule_deletion_failed', { error: err.message || err }));
      } finally {
        scheduleDeletionBtn.disabled = false;
      }
    });
  const cancelDeletionBtn = $('#cancel-deletion-btn');
  if (cancelDeletionBtn)
    cancelDeletionBtn.addEventListener('click', async () => {
      ...
      try {
        await apiCall('POST', `/api/user/${currentUid}/cancel-delete`);
        alert(window.tAdmin('alert_deletion_cancelled'));
        const freshData = await apiCall('GET', `/api/user/${currentUid}`);
        populateDeletionSection(freshData);
      } catch (err) {
        alert(window.tAdminFmt('alert_cancel_deletion_failed', { error: err.message || err }));
      } finally {
  ...
  if (resetDeviceBtn)
    resetDeviceBtn.addEventListener('click', async () => {
      ...
      if (!currentUid) {
        showToast(window.tAdmin('toast_no_user_loaded'), 'error');
        return;
      }
      if (!confirm(window.tAdmin('confirm_remove_all_device_bindings'))) return;
      resetDeviceBtn.disabled = true;
      resetDeviceBtn.textContent = window.tAdmin('btn_resetting');
      try {
        const result = await apiCall('POST', `/api/cleanup/device-binding/${currentUid}`);
        showToast(
          window.tAdminFmt('toast_device_bindings_removed', { count: result.deleted || 0 }),
          'success',
        );
        populateDeviceBindingCard(currentUid);
      } catch (err) {
        showToast(window.tAdminFmt('toast_action_failed', { error: err.message }), 'error');
      } finally {
        resetDeviceBtn.disabled = false;
        resetDeviceBtn.textContent = window.tAdmin('btn_reset_device_binding');
      }
    });
}
...
export function populateEconomySection(data) {
  ...
  const cd = $('#eco-coins-display');
  if (cd) cd.textContent = _ecoCoins;
  const bd = $('#eco-beans-display');
  if (bd) bd.textContent = _ecoBeans;
  const ca = $('#eco-coins-amount');
  if (ca) ca.value = '';
  const ba = $('#eco-beans-amount');
  if (ba) ba.value = '';
  const ss = $('#eco-super-shy');
  if (ss) ss.value = data.isSuperShy ? 'true' : 'false';
  const isSuperShy = data.isSuperShy === true;
  const isUnlimited = isSuperShy && !data.superShyExpiry;
  const ul = $('#eco-super-shy-unlimited');
  if (ul) {
    ul.checked = isUnlimited;
    ul.disabled = !isSuperShy;
  }
  const ex = $('#eco-super-shy-expiry');
  if (ex) {
    ex.disabled = !isSuperShy || isUnlimited;
  }
  if (isSuperShy && data.superShyExpiry) {
    const d = new Date(data.superShyExpiry);
    const pad = (n) => String(n).padStart(2, '0');
    if (ex)
      ex.value = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
  } else {
    if (ex) ex.value = '';
  }
  const st = $('#eco-streak');
  if (st) st.value = data.loginStreak || 0;
  const pt = $('#eco-pity');
  if (pt) pt.value = data.pityCounter || 0;
}
...
export function renderBackpack() {
  ...
  if (filtered.length === 0) {
    ...
    empty.textContent =
      _backpackItems.length === 0
        ? window.tAdmin('msg_backpack_empty')
        : window.tAdmin('msg_no_matching_gifts');
  ...
  filtered.forEach(function (item) {
    ...
    img.src = sanitizeImageUrl(gift.iconUrl);
    ...
    qtyInput.style.cssText =
      'width:100%;text-align:center;padding:4px;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:4px;';
    qtyInput.addEventListener('blur', function () {
      const newQty = Math.max(0, parseInt(qtyInput.value, 10) || 0);
      card.classList.remove('editing');
      if (newQty !== item.quantity) autoSaveBackpackItem(item.giftId, newQty, item.quantity);
    });
...
export async function autoSaveBackpackItem(giftId, newQty, originalQty) {
  try {
    const gift = _giftCatalog.find(function (g) {
      return g.id === giftId;
    });
    const result = await apiCall('POST', '/api/users/' + currentUid + '/backpack', {
      giftId: giftId,
      quantity: newQty,
      giftName: gift ? gift.name : giftId,
    });
    window.PartialFailureToast?.showResultToast(showToast, result, null);
    const item = _backpackItems.find(function (i) {
      return i.giftId === giftId;
    });
    if (item) {
      if (newQty <= 0) {
        _backpackItems = _backpackItems.filter(function (i) {
          return i.giftId !== giftId;
        });
      } else {
    ...
    if (newQty <= 0) showToast(name + ' removed (was ' + originalQty + ')');
    else showToast(name + ': ' + originalQty + ' \u2192 ' + newQty);
  } catch (err) {
    showToast(window.tAdminFmt('toast_failed_to_save', { error: err.message }), 'error');
  }
}
...
function showClearAllConfirmation() {
  ...
  dialog.style.cssText =
    'background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:24px;max-width:400px;width:90%;text-align:center;';
  ...
  warning.textContent =
    "This will permanently remove all items from this user's backpack. This action cannot be undone.";
  dialog.appendChild(warning);
  const btnRow = document.createElement('div');
  btnRow.style.cssText = 'display:flex;gap:12px;justify-content:center;';
  const cancelBtn = document.createElement('button');
  cancelBtn.style.cssText =
    'padding:8px 20px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:14px;';
  ...
  confirmBtn.style.cssText =
    'padding:8px 20px;background:var(--danger);color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:14px;opacity:0.5;';
  confirmBtn.disabled = true;
  let countdown = 5;
  confirmBtn.textContent = window.tAdminFmt('btn_confirming', { countdown });
  const timer = setInterval(function () {
    ...
    if (countdown <= 0) {
      ...
      confirmBtn.textContent = window.tAdmin('btn_confirm_clear_all');
    } else {
      confirmBtn.textContent = window.tAdminFmt('btn_confirming', { countdown });
    }
  }, 1000);
  confirmBtn.addEventListener('click', async function () {
    ...
    clearInterval(timer);
    confirmBtn.disabled = true;
    confirmBtn.textContent = window.tAdmin('btn_clearing');
    try {
...
async function clearAllBackpack() {
  ...
  for (const item of _backpackItems) {
    try {
      await apiCall('POST', '/api/users/' + currentUid + '/backpack', {
        giftId: item.giftId,
        quantity: 0,
        silent: true,
      });
  ...
  renderBackpack();
  if (errors > 0)
    showToast(window.tAdminFmt('toast_cleared_with_errors', { cleared, errors }), 'error');
  else showToast(window.tAdminFmt('toast_backpack_cleared', { count: cleared }));
}
...
export function wireEconomyListeners() {
  ...
  if (ecoSuperShy)
    ecoSuperShy.addEventListener('change', () => {
      const isSS = ecoSuperShy.value === 'true';
      const ul = $('#eco-super-shy-unlimited');
      if (ul) ul.disabled = !isSS;
      if (!isSS) {
        ...
        const ex = $('#eco-super-shy-expiry');
        if (ex && ul) ex.disabled = ul.checked;
      }
    });
  ...
  if (unlimitedCb)
    unlimitedCb.addEventListener('change', () => {
      ...
      if (unlimited) {
        ...
        expiryEl.value = _prevExpiryValue || '1970-01-01T00:00';
        autoSaveEconomyField('superShyExpiry');
      }
    });
  const coinsApply = $('#eco-coins-apply');
  if (coinsApply)
    coinsApply.addEventListener('click', async () => {
      if (!currentUid) return;
      const op = $('#eco-coins-op')?.value;
      const amount = parseInt($('#eco-coins-amount')?.value) || 0;
      if (amount <= 0) {
        showToast(window.tAdmin('toast_enter_positive_amount'), 'error');
        return;
      }
      try {
        ...
        window.PartialFailureToast?.showResultToast(
          showToast,
          result,
          window.tAdminFmt(op === 'add' ? 'toast_coins_added' : 'toast_coins_deducted', {
            amount,
            balance: _ecoCoins,
          }),
        );
  ...
  if (beansApply)
    beansApply.addEventListener('click', async () => {
      if (!currentUid) return;
      ...
      try {
        ...
        window.PartialFailureToast?.showResultToast(
          showToast,
          result,
          window.tAdminFmt(op === 'add' ? 'toast_beans_added' : 'toast_beans_deducted', {
            amount,
            balance: _ecoBeans,
          }),
        );
  ...
  const bpSearch = document.getElementById('backpack-search');
  if (bpSearch) bpSearch.addEventListener('input', renderBackpack);
  const bpCatFilter = document.getElementById('backpack-category-filter');
  if (bpCatFilter) bpCatFilter.addEventListener('change', renderBackpack);
  const addBtn = $('#backpack-add-btn');
  if (addBtn)
    addBtn.addEventListener('click', async () => {
      const giftId = $('#backpack-gift-select')?.value;
      const qty = parseInt($('#backpack-qty')?.value) || 0;
      if (!giftId || !currentUid || qty <= 0) {
        showToast(window.tAdmin('toast_select_gift_qty'), 'error');
        return;
      }
      try {
        ...
        const result = await apiCall('POST', `/api/users/${currentUid}/backpack`, {
          giftId,
          quantity: newQty,
          giftName: giftInfo ? giftInfo.name : giftId,
        });
        window.PartialFailureToast?.showResultToast(
          showToast,
          result,
          window.tAdminFmt('toast_gift_added', { qty, total: newQty }),
        );
        loadBackpack(currentUid);
        const bpQty = $('#backpack-qty');
        if (bpQty) bpQty.value = '1';
        const bpSel = $('#backpack-gift-select');
  ...
  if (clearBtn)
    clearBtn.addEventListener('click', () => {
      if (_backpackItems.length === 0) {
        showToast(window.tAdmin('toast_backpack_empty_already'), 'error');
        return;
      }
  ...
  if (txLoadBtn)
    txLoadBtn.addEventListener('click', async () => {
      if (!currentUid) return;
          ...
          txList.innerHTML = txs
            .map((tx) => {
              const date = tx.timestamp
                ? escapeHtml(new Date(tx.timestamp).toLocaleString())
                : '\u2014';
              const details = tx.details || tx.giftName || '';
              return (
                '<div style="padding:6px 0;border-bottom:1px solid var(--border);font-size:13px"><div style="display:flex;justify-content:space-between"><span style="color:var(--accent)">' +
                escapeHtml(tx.type) +
                ...
                ' ' +
                escapeHtml(tx.currency || 'COINS') +
                ' \u2192 Balance: ' +
                escapeHtml(String(tx.balanceAfter ?? '?')) +
                '</div>' +
                (details
                  ? '<div style="color:var(--text2)">' + escapeHtml(details) + '</div>'
                  : '') +
                '</div>'
              );
            })
...
export async function loadIdentitySubtabGraph() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div style="color:var(--text2);font-size:12px;">Loading identity graph\u2026</div>';
...
function renderIdentitySubtabGraph(data) {
  ...
  nodes.forEach((n) => {
    ...
    g.addEventListener('click', () => {
      ...
      const titleEl = document.getElementById('node-metadata-title');
      const body = document.getElementById('node-metadata-body');
      if (titleEl) titleEl.textContent = (n.type || 'node') + ': ' + (n.label || n.id);
      if (body) body.textContent = JSON.stringify(n.metadata || n, null, 2);
      if (panel) {
  ...
  const multiDevices = nodes.filter(
    (n) => n.type === 'device' && n.linkedAccounts && n.linkedAccounts.length > 1,
  );
  if (multiDevices.length > 0) {
    const alert = document.createElement('div');
    alert.className = 'multi-account-alert';
    alert.style.cssText =
      'margin-top:12px;padding:10px 14px;background:rgba(231,76,60,0.1);border:1px solid #e74c3c;border-radius:6px;color:#e74c3c;font-size:13px;';
    alert.textContent =
      '\u26A0 This device is linked to multiple accounts (' +
      multiDevices[0].linkedAccounts.length +
      ')';
...
function showIdentitySuspendDialog() {
  const d = document.getElementById('identity-suspend-dialog');
  if (d) {
...
export function wireIdentityListeners() {
  ...
  if (confirmBtn)
    confirmBtn.addEventListener('click', async () => {
      ...
      try {
        await apiCall('POST', `/api/admin/identity-graph/${uid}/suspend-all`, {
          duration,
          scope,
          reason,
        });
  ...
  if (nodeUnsuspend)
    nodeUnsuspend.addEventListener('click', async () => {
      const uid = currentUid;
      const panel = document.getElementById('node-metadata-panel');
...
export async function populateBansSection(uid) {
  ...
  if (bansDeviceList)
    bansDeviceList.innerHTML = '<div style="color:var(--text2);font-size:12px;">Loading...</div>';
  if (bansNetworkList)
    bansNetworkList.innerHTML = '<div style="color:var(--text2);font-size:12px;">Loading...</div>';
  if (bansDevicesBoundList)
    bansDevicesBoundList.innerHTML =
  ...
  try {
    ...
    if (bansDeviceList) {
      if (deviceBans.length === 0) {
        bansDeviceList.innerHTML =
          '<div style="color:var(--text2);font-size:12px;font-style:italic;">No device bans</div>';
      } else {
        bansDeviceList.textContent = '';
        deviceBans.forEach((b) => {
          const item = document.createElement('div');
          item.className = 'ban-item';
          item.style.cssText =
            'display:flex;align-items:center;justify-content:space-between;gap:8px;';
          const info = document.createElement('div');
          info.className = 'ban-item-info';
          info.style.flex = '1';
          const idSpan = document.createElement('span');
          idSpan.textContent = b.deviceId || b.id;
          const detailSpan = document.createElement('span');
          detailSpan.className = 'ban-item-type';
          ...
          removeBtn.style.cssText =
            'padding:4px 10px;border:none;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600;background:var(--danger);color:#fff;white-space:nowrap;';
          removeBtn.addEventListener('click', async () => {
            if (removeBtn.disabled) return;
            if (!confirm(window.tAdmin('confirm_remove_device_ban'))) return;
            removeBtn.disabled = true;
            try {
              await apiCall(
                'DELETE',
                '/api/admin/bans/device/' + encodeURIComponent(b.deviceId || b.id),
              );
              showToast('Device ban removed', 'success');
          ...
          bansDeviceList.appendChild(item);
        });
      }
    }
    // Network bans
    ...
    if (bansNetworkList) {
      if (networkBans.length === 0) {
        bansNetworkList.innerHTML =
          '<div style="color:var(--text2);font-size:12px;font-style:italic;">No network bans</div>';
      } else {
        bansNetworkList.textContent = '';
        networkBans.forEach((b) => {
          const item = document.createElement('div');
          item.className = 'ban-item';
          item.style.cssText =
            'display:flex;align-items:center;justify-content:space-between;gap:8px;';
          ...
          valSpan.textContent = (b.value || b.id) + ' (' + (b.type || 'ip') + ')';
          const detailSpan = document.createElement('span');
          detailSpan.className = 'ban-item-type';
          detailSpan.textContent =
            (b.reason || 'No reason') +
          ...
          removeBtn.addEventListener('click', async () => {
            if (removeBtn.disabled) return;
            if (!confirm(window.tAdmin('confirm_remove_network_ban'))) return;
            removeBtn.disabled = true;
            try {
              await apiCall('DELETE', '/api/admin/bans/network/' + encodeURIComponent(b.id));
              showToast('Network ban removed', 'success');
              populateBansSection(currentUid);
            } catch (err) {
              showToast(err.message, 'error');
            } finally {
          ...
          item.appendChild(removeBtn);
          bansNetworkList.appendChild(item);
        });
      }
    }
    // Bound devices
    ...
    if (bansDevicesBoundList) {
      if (devices.length === 0) {
        bansDevicesBoundList.textContent = '';
        const emptyMsg = document.createElement('div');
        emptyMsg.style.cssText = 'color:var(--text2);font-size:12px;font-style:italic;';
        emptyMsg.textContent = 'No bound devices';
        bansDevicesBoundList.appendChild(emptyMsg);
      } else {
        bansDevicesBoundList.textContent = '';
        devices.forEach((d) => {
          const isBanned = bannedDeviceIds.has(d.id);
          const ban = isBanned ? deviceBans.find((b) => (b.deviceId || b.id) === d.id) : null;
          const deviceLabel = (d.manufacturer ? d.manufacturer + ' ' : '') + (d.model || d.id);
          const card = document.createElement('div');
          card.className = 'device-card' + (isBanned ? ' banned' : '');
          const header = document.createElement('div');
          header.className = 'device-card-header';
          const chevron = document.createElement('span');
          chevron.className = 'chevron';
          chevron.style.cssText = 'font-size:10px;color:var(--text2);';
          chevron.textContent = '\u25B6';
          const labelSpan = document.createElement('span');
          labelSpan.style.flex = '1';
          labelSpan.textContent = deviceLabel;
          const badge = document.createElement('span');
          badge.className = 'device-ban-badge ' + (isBanned ? 'banned' : 'active');
          ...
          [
            ...
            ['Country', d.country],
            ['Region', d.region],
            ['First Seen', d.firstSeen ? new Date(d.firstSeen).toLocaleString() : null],
            ['Last Seen', d.lastSeen ? new Date(d.lastSeen).toLocaleString() : null],
            ['Device ID', d.id],
          ].forEach(([label, value]) => {
          ...
          if (isBanned && ban) {
            ...
            banInfo.style.cssText =
              'margin-top:8px;padding:8px;background:rgba(231,76,60,0.1);border-radius:6px;font-size:12px;';
            banInfo.innerHTML =
              '<div><strong>Reason: </strong>' +
              escapeHtml(ban.reason || 'No reason') +
              '</div><div><strong>Duration: </strong>' +
              escapeHtml(ban.duration || 'Permanent') +
              '</div>' +
              (ban.createdAt
                ? '<div><strong>Banned: </strong>' +
                  escapeHtml(new Date(ban.createdAt).toLocaleString()) +
                  '</div>'
                : '');
            body.appendChild(banInfo);
            const unbanBtn = document.createElement('button');
            unbanBtn.setAttribute('data-ban-action', 'unban');
            unbanBtn.setAttribute('data-device-id', d.id);
            unbanBtn.style.cssText =
              'margin-top:8px;padding:6px 14px;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;background:#2ecc71;color:#fff;';
            unbanBtn.textContent = 'Unban This Device';
            body.appendChild(unbanBtn);
          } else if (!isBanned) {
            ...
            reasonInput.style.cssText =
              'flex:1;min-width:120px;padding:6px 10px;border:1px solid var(--border);border-radius:6px;background:var(--surface1);color:var(--text);font-size:12px;';
            const durationSelect = document.createElement('select');
            durationSelect.className = 'ban-duration-select';
            durationSelect.style.cssText =
              'padding:6px 10px;border:1px solid var(--border);border-radius:6px;background:var(--surface1);color:var(--text);font-size:12px;';
            [
              ['', 'Permanent'],
              ['1h', '1 Hour'],
              ['24h', '24 Hours'],
            ...
            banBtn.setAttribute('data-ban-action', 'ban');
            banBtn.setAttribute('data-device-id', d.id);
            banBtn.style.cssText =
              'padding:6px 14px;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;background:var(--danger);color:#fff;';
            banBtn.textContent = 'Ban This Device';
            actionRow.appendChild(reasonInput);
          ...
          bansDevicesBoundList.appendChild(card);
        });
      }
    }
    ...
    if (bansDeviceList)
      bansDeviceList.innerHTML =
        '<div style="color:var(--danger);font-size:12px;">Failed to load</div>';
    if (bansNetworkList)
      bansNetworkList.innerHTML =
        '<div style="color:var(--danger);font-size:12px;">Failed to load</div>';
    if (bansDevicesBoundList)
...
export async function populateDeviceBindingCard(uid) {
  ...
  if (cardsEl)
    cardsEl.innerHTML = '<div style="color:var(--text2);font-size:12px;">Loading...</div>';
      ...
      cardsEl.innerHTML = devices
        .map((d) => {
          const rows = [
            ...
            ['OS Version', d.osVersion || 'N/A'],
            [
              'App Version',
              (d.appVersion || 'N/A') + (d.buildNumber ? ' (' + d.buildNumber + ')' : ''),
            ],
            ['Screen', d.screenResolution || 'N/A'],
            ['Density', d.screenDensity || 'N/A'],
            ['Network Type', d.networkType || 'N/A'],
            ['Carrier', d.carrier || 'N/A'],
            ['Last IP', d.lastIp || 'N/A'],
            ['ISP', d.isp || 'N/A'],
            ['ASN', d.asn || 'N/A'],
            ['Country', d.country || 'N/A'],
            ['Region', d.region || 'N/A'],
            ['First Seen', d.firstSeen ? new Date(d.firstSeen).toLocaleString() : 'N/A'],
            ['Last Seen', d.lastSeen ? new Date(d.lastSeen).toLocaleString() : 'N/A'],
          ];
          return (
            '<div style="background:var(--surface2);border-radius:8px;padding:12px;margin-bottom:8px;"><div style="font-weight:600;font-size:14px;margin-bottom:8px;color:var(--text);">' +
            escapeHtml((d.manufacturer || '') + ' ' + (d.model || d.id)) +
            '</div><div style="display:grid;grid-template-columns:140px 1fr;gap:4px 12px;font-size:12px;">' +
            rows
              .map(
                ([label, val]) =>
                  '<div style="color:var(--text2);font-weight:500;">' +
                  escapeHtml(label) +
              ...
              .join('') +
            '</div><div style="margin-top:8px;font-size:11px;color:var(--text2);">Device ID: ' +
            escapeHtml(d.id) +
            '</div></div>'
          );
        })
...
export function populateTempId(data) {
  ...
  if (data.tempUniqueId && data.tempUniqueIdExpiry && data.tempUniqueIdExpiry > Date.now()) {
    ...
    const pad = (n) => String(n).padStart(2, '0');
    if (expiryEl)
      expiryEl.value = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
    if (currentEl)
      currentEl.innerHTML =
        'Active temp ID: <strong>' +
        escapeHtml(String(data.tempUniqueId)) +
        '</strong> (expires ' +
    ...
    if (currentEl)
      currentEl.textContent = data.tempUniqueId ? 'Temp ID expired' : 'No temporary ID set';
  }
}
...
export function updateCurrentPreview() {
  ...
  if (pcName) pcName.textContent = u.displayName || '';
  const pcId = document.getElementById('pc-id');
  if (pcId) pcId.textContent = u.uniqueId ? '#' + u.uniqueId : '';
  const pcBadge = document.getElementById('pc-badge');
  if (pcBadge) pcBadge.textContent = u.userType || '';
  const pcDesc = document.getElementById('pc-desc');
  if (pcDesc) pcDesc.textContent = u.description || '';
...
export function updateDraftPreview() {
  ...
  if (pdName) pdName.textContent = getVal('[data-field="displayName"]') || '';
  const pdId = document.getElementById('pd-id');
  if (pdId) pdId.textContent = loadedData.uniqueId ? '#' + loadedData.uniqueId : '';
  const pdBadge = document.getElementById('pd-badge');
  if (pdBadge) pdBadge.textContent = getVal('[data-field="userType"]') || '';
  const pdDesc = document.getElementById('pd-desc');
  if (pdDesc) pdDesc.textContent = getVal('[data-field="description"]') || '';
...
function setPreviewFlag(id, nationality) {
  ...
  el.textContent = nationality
    ? codeToFlag(nationality) + ' ' + (countryNames.of(nationality) || nationality)
    : '';
}
function setPreviewImage(id, url) {
  ...
  el.src = safe;
  el.style.display = safe ? 'inline-block' : 'none';
}
function setPreviewCoverImage(id, url) {
  const el = document.getElementById(id);
  if (!el) return;
  const safe = sanitizeImageUrl(url);
  el.style.backgroundImage = safe ? 'url(' + safe + ')' : '';
  el.style.backgroundColor = safe ? '' : '#333';
}
function setPreviewCounts(id, data) {
  const el = document.getElementById(id);
  if (!el) return;
  const following = Array.isArray(data.followingIds) ? data.followingIds.length : 0;
  const followers = Array.isArray(data.followerIds) ? data.followerIds.length : 0;
  const stalkers = data._stalkerCount !== undefined ? data._stalkerCount : '...';
  el.textContent =
    'Following: ' + following + ' | Followers: ' + followers + ' | Stalkers: ' + stalkers;
}
...
export async function loadStalkers(uid) {
  try {
    ...
    if (data.stalkers.length === 0) {
      ...
      empty.textContent = 'No stalkers';
      container.appendChild(empty);
    } else {
      data.stalkers.forEach(function (id) {
        const tag = document.createElement('span');
        tag.style.cssText =
          'display:inline-block;padding:2px 8px;margin:2px;background:var(--surface);border:1px solid var(--border);border-radius:4px;font-size:12px;color:var(--text);';
        tag.textContent = id;
...
async function loadIdentityGraphTabular(uid) {
  ...
  tbody.innerHTML =
    '<tr><td colspan="4" style="color:var(--text2);font-size:12px;">Loading...</td></tr>';
  ...
  try {
    ...
    if (identifiers.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" style="color:var(--text2);font-size:12px;font-style:italic;">No identity graph data</td></tr>';
    } else {
      tbody.textContent = '';
      identifiers.forEach((ident) => {
        const tr = document.createElement('tr');
        const tdType = document.createElement('td');
        tdType.textContent = ident.type || 'unknown';
        tr.appendChild(tdType);
        const tdValue = document.createElement('td');
        tdValue.textContent = ident.value || '';
        tdValue.style.cssText = 'font-family:monospace;font-size:11px;word-break:break-all;';
        tr.appendChild(tdValue);
        const tdMeta = document.createElement('td');
        tdMeta.textContent = ident.metadata || '';
        tdMeta.style.cssText = 'font-size:11px;color:var(--text2);';
        tr.appendChild(tdMeta);
        const tdStatus = document.createElement('td');
        if (ident.suspended) {
          tdStatus.className = 'ig-suspended';
          tdStatus.textContent =
            'Suspended' +
            (ident.suspendedUntil
              ? ' until ' + new Date(ident.suspendedUntil).toLocaleDateString()
              : ' (permanent)');
        } else {
          tdStatus.className = 'ig-active';
    ...
    if (data.cascadeInfo && cascadePreview) {
      cascadePreview.textContent =
        'This will also affect: ' +
        (data.cascadeInfo.devices || 0) +
        ' devices, ' +
        (data.cascadeInfo.networks || 0) +
        ' networks, ' +
        (data.cascadeInfo.accounts || 0) +
        ' accounts';
      cascadePreview.classList.add('visible');
    }
  } catch (err) {
    tbody.innerHTML =
      '<tr><td colspan="4" style="color:var(--danger);font-size:12px;">Failed to load: ' +
      escapeHtml(err.message) +
      '</td></tr>';
  }
}
...
export function wireBansListeners() {
  const bansDevicesBoundList = $('#bans-devices-bound-list');
  if (bansDevicesBoundList) {
    bansDevicesBoundList.addEventListener('click', (e) => {
      const header = e.target.closest('.device-card-header');
      if (!header) return;
    ...
    bansDevicesBoundList.addEventListener('click', async (e) => {
      ...
      if (action === 'ban') {
        ...
        const durationSelect = btn
          .closest('.device-card-body')
          ?.querySelector('.ban-duration-select');
        btn.disabled = true;
        try {
  ...
  if (banAllBtn)
    banAllBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const devices = devicesData.devices || [];
        if (devices.length === 0) {
          showToast(window.tAdmin('toast_no_devices_to_ban'), 'error');
          return;
        }
        ...
        const settled = await Promise.allSettled(
          devices.map((d) =>
            apiCall('POST', '/api/admin/bans/device', {
              deviceId: d.id,
              reason,
              linkedUniqueId: currentUid,
            }),
        ...
        if (rejected.length > 0) {
          segments.push(
            window.tAdminFmt('segment_ban_call_failed', {
              count: rejected.length,
              total: devices.length,
              error: rejected[0].reason?.message || 'unknown',
            }),
          );
        }
        if (aggregatePmFailed > 0) {
          segments.push(
            window.tAdminFmt('segment_pm_failed', {
              count: aggregatePmFailed,
              total: aggregatePmTotal,
            }),
          );
        }
        if (segments.length > 0) {
          showToast(
            window.tAdminFmt('toast_partial_retry', { summary: segments.join('; ') }),
            'error',
          );
        } else {
          showToast(
            window.tAdminFmt('toast_banned_n_devices', { count: fulfilled.length }),
            'success',
          );
        }
        populateBansSection(currentUid);
  ...
  if (banIpBtn)
    banIpBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!lastDevice || !lastDevice.lastIp) {
          showToast(window.tAdmin('toast_no_ip_found'), 'error');
          return;
        }
  ...
  if (unbanAllBtn)
    unbanAllBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.PartialFailureToast?.showResultToast(
          showToast,
          result,
          window.tAdminFmt('toast_removed_n_bans', { count: result.removed || 0 }),
        );
        populateBansSection(currentUid);
      } catch (err) {
        showToast(window.tAdminFmt('toast_action_failed', { error: err.message }), 'error');
      } finally {
        unbanAllBtn.disabled = false;
      }
    });
  ...
  if (viewLogsBtn)
    viewLogsBtn.addEventListener('click', () => {
      if (!currentUid) return;
      const logsUserFilter = $('#log-filter-userId');
      if (logsUserFilter) logsUserFilter.value = currentUid;
      _switchTab('logs');
    });
  // Identity graph suspend/unsuspend (tabular version)
  const igSuspendBtn = $('#ig-suspend-btn');
  if (igSuspendBtn)
    igSuspendBtn.addEventListener('click', async () => {
      if (igSuspendBtn.disabled) return;
      if (!currentUid) return;
      const duration = $('#ig-duration-picker')?.value;
      const scope = $('#ig-scope-picker')?.value;
      if (!confirm(window.tAdminFmt('confirm_suspend_identity_graph', { duration, scope }))) return;
      igSuspendBtn.disabled = true;
      try {
        await apiCall('PUT', `/api/admin/bans/graph/${currentUid}`, {
          action: 'suspend',
          duration,
          scope,
        });
        showToast(window.tAdmin('toast_identity_graph_suspended'), 'success');
        populateBansSection(currentUid);
      } catch (err) {
        showToast(err.message, 'error');
      } finally {
        igSuspendBtn.disabled = false;
      }
    });
  const igUnsuspendBtn = $('#ig-unsuspend-btn');
  if (igUnsuspendBtn)
    igUnsuspendBtn.addEventListener('click', async () => {
      if (igUnsuspendBtn.disabled) return;
      if (!currentUid) return;
      if (!confirm(window.tAdmin('confirm_unsuspend_identity_graph'))) return;
      igUnsuspendBtn.disabled = true;
      try {
        await apiCall('PUT', `/api/admin/bans/graph/${currentUid}`, { action: 'unsuspend' });
        showToast(window.tAdmin('toast_identity_graph_unsuspended'), 'success');
        populateBansSection(currentUid);
      } catch (err) {
        showToast(err.message, 'error');
      } finally {
        igUnsuspendBtn.disabled = false;
      }
    });
}
...
export function wireTempIdListeners() {
  ...
  if (checkBtn)
    checkBtn.addEventListener('click', async () => {
      const id = parseInt(document.getElementById('temp-id-input')?.value);
      const resultEl = document.getElementById('temp-id-check-result');
      if (!id || id < 10000000) {
        if (resultEl)
          resultEl.innerHTML =
            '<span style="color:var(--danger);">ID must be at least 10000000</span>';
        return;
      }
      try {
        const data = await apiCall('GET', `/api/admin/users/check-id/${id}`);
        if (resultEl)
          resultEl.innerHTML = data.available
            ? '<span style="color:#2ecc71;">&#10003; Available</span>'
            : '<span style="color:var(--danger);">\u2717 This ID is already in use</span>';
      } catch (err) {
        if (resultEl)
          resultEl.innerHTML =
  ...
  if (applyBtn)
    applyBtn.addEventListener('click', async () => {
      if (!currentUid) return;
      const id = parseInt(document.getElementById('temp-id-input')?.value);
      const expiryVal = document.getElementById('temp-id-expiry')?.value;
      if (!id || id < 10000000) {
        showToast('ID must be at least 10000000', 'error');
        return;
      }
      if (!expiryVal) {
        showToast('Set an expiry date', 'error');
        return;
      }
      const expiryDate = new Date(expiryVal).getTime();
      if (expiryDate <= Date.now()) {
        showToast('Expiry must be in the future', 'error');
        return;
      }
      try {
        await apiCall('POST', `/api/admin/users/${currentUid}/temp-id`, {
          tempUniqueId: id,
          expiryDate,
        });
        showToast('Temporary ID applied', 'success');
        const currentEl = document.getElementById('temp-id-current');
        if (currentEl)
          currentEl.innerHTML =
            'Active temp ID: <strong>' +
            ...
            '</strong> (expires ' +
            escapeHtml(new Date(expiryDate).toLocaleString()) +
            ')';
        const resultEl = document.getElementById('temp-id-check-result');
        if (resultEl) resultEl.textContent = '';
      } catch (err) {
        showToast(err.message || 'Failed to apply temp ID', 'error');
      }
    });
  ...
  if (clearBtn)
    clearBtn.addEventListener('click', async () => {
      ...
      if (!currentUid) return;
      if (!confirm(window.tAdmin('confirm_clear_temp_id'))) return;
      clearBtn.disabled = true;
      try {
        await apiCall('DELETE', `/api/admin/users/${currentUid}/temp-id`);
        showToast('Temporary ID cleared', 'success');
        const inputEl = document.getElementById('temp-id-input');
        if (inputEl) inputEl.value = '';
        const expiryEl = document.getElementById('temp-id-expiry');
        if (expiryEl) expiryEl.value = '';
        const currentEl = document.getElementById('temp-id-current');
        if (currentEl) currentEl.textContent = 'No temporary ID set';
        const resultEl = document.getElementById('temp-id-check-result');
        if (resultEl) resultEl.textContent = '';
      } catch (err) {
        showToast(err.message || 'Failed to clear temp ID', 'error');
      } finally {
...
export function wirePreviewListeners() {
  document
    .querySelectorAll(
      '[data-field="displayName"], [data-field="description"], [data-field="userType"], [data-field="nationality"], [data-field="profilePhotoUrl"], [data-field="coverPhotoUrl"]',
    )
    .forEach(function (el) {
      el.addEventListener('input', updateDraftPreview);
      el.addEventListener('change', updateDraftPreview);
    });
}
