...
(function () {
  ...
  var LABELS = {
    en: {
      ...
      disclaimer:
        'Features and priorities may change as development progresses and user feedback is received.',
    ...
    fr: {
      ...
      tryAgain: 'R\u00E9essayer',
      notifyPrompt: 'Connectez-vous \u00E0 l\u2019appli pour \u00EAtre notifi\u00E9.',
    },
    hi: {
    ...
    pl: {
      ...
      notifyPrompt: 'Zaloguj si\u0119 w aplikacji, aby otrzymywa\u0107 powiadomienia.',
    },
    pt: {
    ...
    tr: {
      ...
      notifyPrompt: 'Bildirim almak i\u00E7in uygulamaya giri\u015F yap\u0131n.',
    },
    uk: {
  ...
  function renderDonutChart(done, inProgress, planned) {
    ...
    var scale = segments.length > 1 ? (circumference - totalGap) / circumference : 1;
    ...
    for (var i = 0; i < segments.length; i++) {
      ...
      html += '<circle cx="' + cx + '" cy="' + cy + '" r="' + radius + '" fill="none" ';
      ...
      html += 'style="transition: stroke-dasharray 0.6s ease, stroke-dashoffset 0.6s ease;"/>';
      offset += segLen + gap;
    }
  ...
  function renderPhases(data) {
    ...
    if (!phases || phases.length === 0) {
      container.innerHTML =
        '<div class="error-state"><p>' + escapeHtml(t('loadFail')) + '</p></div>';
      return;
    }
    ...
    for (var ip = 0; ip < phases.length; ip++) {
      ...
      var ipFeatures = (phases[ip].features || []).concat(phaseItemsAsFeatures(phases[ip]));
    ...
    if (inProgressItems.length > 0) {
      html +=
        '<div id="in-progress-section" class="phase-card" data-testid="in-progress-section" style="border-left: 3px solid var(--in-progress, #ff9800);">';
      html += '<button class="phase-header" aria-expanded="true">';
      html += CHEVRON_SVG;
      html +=
        '<span class="phase-title">' + escapeHtml(t('inProgress') || 'In Progress') + '</span>';
      html += '<span class="phase-progress">';
      html +=
        '<span class="phase-progress-text">' +
        inProgressItems.length +
        ' ' +
        escapeHtml(t('inProg') || 'In Progress') +
        '</span>';
      html += '</span>';
      html += '</button>';
      ...
      for (var ipi = 0; ipi < inProgressItems.length; ipi++) {
        ...
        html +=
          ...
          (ipItem.feature.shyId
            ? '<span class="shy-item-text" data-src="' +
              escapeHtml(ipItem.feature.name) +
              '">' +
              escapeHtml(ipFeatName) +
              '</span>'
            : escapeHtml(ipFeatName)) +
          (ipItem.feature.shyId
            ? ' ' + shyBadgeHtml(ipItem.feature.shyId, ipItem.feature.slug)
            : '') +
          '</div>';
        if (ipFeatDesc) html += '<div class="feature-desc">' + escapeHtml(ipFeatDesc) + '</div>';
        html +=
          '<div class="feature-desc" style="font-size:0.75rem;opacity:0.6;margin-top:2px;">' +
          escapeHtml(ipPhaseLabel) +
          '</div>';
        html += '</div>';
        html +=
          '<button class="feature-bell" aria-label="Notify me about ' +
          escapeHtml(ipFeatName) +
          '" data-testid="feature-bell" data-log="bell-' +
          escapeHtml(ipItem.feature.name) +
          '">' +
          BELL_SVG +
          '</button>';
    ...
    for (var p = 0; p < phases.length; p++) {
      ...
      var phaseTitle = (phase.titleI18n && phase.titleI18n[currentLang]) || phase.title;
      var progressPct = features.length > 0 ? Math.round((phaseDone / features.length) * 100) : 0;
      ...
      var fillClass =
        phaseDone === features.length
          ? 'phase-progress-fill--complete'
          : phaseDone > 0 || phaseIp > 0
            ? 'phase-progress-fill--in-progress'
      ...
      html +=
        '<div class="phase-progress-bar"><span class="phase-progress-fill ' +
        fillClass +
        '" style="width:' +
        progressPct +
        '%"></span></div>';
      ...
      for (var fi = 0; fi < features.length; fi++) {
        ...
        if (featDesc) {
          html += feat.shyId
            ? '<div class="feature-desc"><span class="shy-item-text" data-src="' +
  ...
  function translateItems(data) {
    ...
    Promise.all(
      ...
      if (missedTotal > 0) {
        console.info(
          '[translate] ' + missedTotal + ' string(s) not yet translated — shown in English',
        );
      }
