...
(function () {
  ...
  // The single line is load-bearing, not a style choice. The pre-commit URL
  // guard is line-oriented: every added line naming a shytalk host must also
  // name localhost. Wrapped, two correct branches read as hardcoded URLs.
  // prettier-ignore
  ...
  function standingBannerHtml() {
    if (!state.standing) return '';
    var msgKey = state.standing.kind === 'banned' ? 'standing_banned' : 'standing_suspended';
    var html =
      '<div class="sg-standing-banner" role="alert" data-testid="standing-banner" ' +
      'data-standing="' +
      escapeHtml(state.standing.kind) +
      '">' +
      '<span>' +
      escapeHtml(sgT(msgKey)) +
      '</span>';
  ...
  function relativeTime(dateStr) {
    ...
    var lang =
      (window.ShyTalkLanguage && window.ShyTalkLanguage.get()) ||
  ...
  function isVotingDisabled(status) {
    return status === 'planned' || status === 'completed' || status === 'rejected';
  }
  ...
  function fetchSuggestions() {
    ...
    if (state.filterStatus) params += '&status=' + encodeURIComponent(state.filterStatus);
    if (state.filterTag) params += '&tag=' + encodeURIComponent(state.filterTag);
    if (state.filterLang) params += '&lang=' + encodeURIComponent(state.filterLang);
    if (state.filterPhase) params += '&phase=' + encodeURIComponent(state.filterPhase);

    var path = state.searchQuery
      ? '/api/suggestions/search' + params + '&q=' + encodeURIComponent(state.searchQuery)
  ...
  function submitVote(suggestionId, direction) {
    ...
    apiFetch('/api/suggestions/' + suggestionId + '/vote', {
      ...
      .catch(function (err) {
        showToast(sgT('toast_vote_failed') + ': ' + (err.message || sgT('unknown_error')));
      });
  }
  ...
  function searchDuplicates(query) {
    return apiFetch('/api/suggestions/search?q=' + encodeURIComponent(query) + '&limit=3');
  }
  ...
  function showLoginPromptModal(action) {
    ...
    var html =
      ...
      escapeHtml(sgT('signInRequired')) +
      '">' +
      '<div class="sg-modal-header">' +
      '<h3>' +
      sgT('signInRequired') +
      '</h3>' +
      '<button class="sg-modal-close" data-testid="login-modal-close" aria-label="' +
      ...
      '<button class="auth-google-btn" data-testid="auth-google-btn" aria-label="' +
      escapeHtml(sgT('signInGoogle')) +
      '" style="display:inline-flex;align-items:center;gap:10px;padding:10px 24px;background:#fff;color:#3c4043;border:1px solid #dadce0;border-radius:4px;font-size:14px;font-weight:500;cursor:pointer;min-height:44px;">' +
      ...
      '</button>' +
      '<button class="auth-apple-btn" data-testid="auth-apple-btn" aria-label="' +
      escapeHtml(sgT('signInApple')) +
      '" style="display:inline-flex;align-items:center;gap:10px;padding:10px 24px;background:#000;color:#fff;border:none;border-radius:4px;font-size:14px;font-weight:500;cursor:pointer;min-height:44px;">' +
  ...
  function openSubscribeModal(featureId) {
    ...
    var html =
      ...
      '<div class="sg-modal sg-modal--wide" role="dialog" aria-modal="true" aria-label="' +
      escapeHtml(sgT('subscribe')) +
      '">' +
      '<div class="sg-modal-header">' +
      '<h3>' +
      sgT('subscribe') +
      '</h3>' +
      '<button class="sg-modal-close" data-testid="subscribe-modal-close" aria-label="' +
      escapeHtml(sgT('close')) +
      '">&times;</button>' +
      '</div>' +
      ...
      '<button class="sg-btn sg-btn--secondary" data-testid="subscribe-modal-cancel">' +
      sgT('cancel') +
      '</button>' +
    ...
    var closeBtn = overlay.querySelector('.sg-modal-close');
    var cancelBtn = overlay.querySelector('[data-testid="subscribe-modal-cancel"]');
  ...
  function openSuggestModal() {
    ...
    var html =
      ...
      '<div class="sg-modal sg-modal--wide" role="dialog" aria-modal="true" aria-label="' +
      escapeHtml(sgT('suggestFeature')) +
      '">' +
      '<div class="sg-modal-header">' +
      '<h3>' +
      sgT('suggestFeature') +
      '</h3>' +
      '<button class="sg-modal-close" data-testid="suggest-modal-close" aria-label="' +
      escapeHtml(sgT('close')) +
      '">&times;</button>' +
      '</div>' +
      '<div class="sg-modal-body">' +
      ...
      '</div>' +
      '<div class="sg-modal-actions">' +
      '<button class="sg-btn sg-btn--secondary" data-testid="suggest-modal-cancel">' +
      sgT('cancel') +
      '</button>' +
    ...
    var closeBtn = overlay.querySelector('.sg-modal-close');
    var cancelBtn = overlay.querySelector('[data-testid="suggest-modal-cancel"]');
    var submitBtn = overlay.querySelector('[data-testid="suggest-modal-submit"]');
        ...
        duplicateTimer = setTimeout(function () {
          searchDuplicates(query)
            .then(function (data) {
              ...
              var matchBtns = duplicateResults.querySelectorAll('.sg-duplicate-match');
              ...
              var diffBtns = duplicateResults.querySelectorAll('.sg-duplicate-diff');
    ...
    submitBtn.addEventListener('click', function () {
      ...
      checkBlockedTopics(titleInput.value.trim())
        .then(function (data) {
          if (data.blocked) {
            showToast(sgT('toast_topic_not_allowed') + ': ' + (data.reason || ''));
        ...
        .catch(function (err) {
          showToast(sgT('toast_submit_failed') + ': ' + (err.message || sgT('unknown_error')));
          submitBtn.disabled = false;
          submitBtn.textContent = sgT('submit');
        });
    });
  ...
  function renderCommentSection(suggestion) {
    ...
    var html = '<div class="sg-comments" data-testid="comments-section-' + suggestion.id + '">';
    ...
    if (comments.length > 0) {
      ...
      for (var i = 0; i < comments.length; i++) {
        ...
        var commentText = c.authorDeleted ? sgT('commentFromDeletedUser') : c.text;
        html +=
      ...
      html += '<p class="sg-text-muted sg-comment-empty">No comments yet. Be the first!</p>';
    }
  ...
  function renderBoard() {
    ...
    if (canAct()) {
      html +=
        '<button class="sg-btn sg-btn--primary sg-suggest-btn" data-testid="suggest-btn">' +
        escapeHtml(sgT('suggest')) +
        '</button>';
    }
    html += '</div>';
    ...
    html +=
      ...
      '" data-sort="votes" data-testid="sort-most-voted">' +
      escapeHtml(sgT('mostVoted')) +
      '</button>';
    html +=
      '<button class="sg-sort-btn' +
      (state.sort === 'newest' ? ' sg-sort-btn--active' : '') +
      '" data-sort="newest" data-testid="sort-newest">' +
      escapeHtml(sgT('newest')) +
      '</button>';
    html += '</div>';
    ...
    // Status filter
    html += '<select class="sg-filter-select" data-filter="status" data-testid="filter-status">';
    for (var si = 0; si < STATUS_OPTIONS.length; si++) {
      var sel = state.filterStatus === STATUS_OPTIONS[si].value ? ' selected' : '';
      html +=
        '<option value="' +
    ...
    // Tag filter
    html += '<select class="sg-filter-select" data-filter="tag" data-testid="filter-tag">';
    ...
    // Language filter
    html += '<select class="sg-filter-select" data-filter="lang" data-testid="filter-lang">';
    ...
    // Phase filter
    html += '<select class="sg-filter-select" data-filter="phase" data-testid="phase-filter">';
    ...
    if (state.suggestions.length === 0) {
      var emptyMsg =
        state.searchQuery || state.filterStatus || state.filterTag || state.filterLang
    ...
    if (totalPages > 1) {
      html += '<div class="sg-pagination" data-testid="suggestions-pagination">';
      // Previous
      html +=
  ...
  function renderSuggestionCard(s) {
    ...
    var html =
      '<div class="sg-card" data-testid="suggestion-card-' + s.id + '" data-id="' + s.id + '">';
    ...
    if (!votingDisabled) {
      html +=
        ...
        '" data-id="' +
        s.id +
        '" data-dir="up" aria-label="' +
        escapeHtml(sgT('aria_upvote')) +
        '">' +
        '<svg viewBox="0 0 16 16" width="16" height="16" fill="currentColor"><path d="M8 4l-5 6h10z"/></svg>' +
        '</button>';
    }
    html +=
      '<span class="sg-vote-score" data-testid="vote-score-' + s.id + '">' + score + '</span>';
    if (!votingDisabled) {
      html +=
        ...
        '" data-id="' +
        s.id +
        '" data-dir="down" aria-label="' +
        escapeHtml(sgT('aria_downvote')) +
        '">' +
    ...
    // Description
    html += '<div class="sg-card-desc" data-testid="suggestion-desc-' + s.id + '">';
      ...
      expandBtns[e].addEventListener('click', function () {
        ...
        for (var s = 0; s < state.suggestions.length; s++) {
          if (state.suggestions[s].id === id) {
            ...
            if (descEl) {
              descEl.innerHTML = escapeHtml(state.suggestions[s].description || '');
            }
            break;
          }
        }
    ...
    for (var c = 0; c < commentBtns.length; c++) {
      commentBtns[c].addEventListener('click', function () {
        ...
        submitComment(suggestionId, text)
          ...
          .catch(function (err) {
            showToast(
              sgT('toast_post_comment_failed') + ': ' + (err.message || sgT('unknown_error')),
            );
            btn.disabled = false;
